...
describe('GitHub release contract', () => {
  ...
  it('uses stable platform-specific artifact names', async () => {
    ...
    expect(packageJson.build.artifactName).toBe('aimarketing-desktop-${os}-${arch}.${ext}')
    ...
    expect(packageJson.build.nsis.artifactName).toBe(
      'aimarketing-desktop-windows-${arch}-setup.${ext}'
    )
  ...
  it('shows a packaged startup surface and pins the Electron directory picker surface', async () => {
    ...
    expect(splash).toContain('Starting AiMarketing')
  ...
  it('packages an isolated development channel from the current workspace', async () => {
    ...
    expect(developmentConfig).toContain("appId: 'io.aimarketing.desktop.dev'")
    expect(developmentConfig).toContain("productName: 'AiMarketing Dev'")
  ...
  it('builds and publishes every supported platform', async () => {
    ...
    expect(workflow).toContain("'dist-dev\\win-unpacked\\AiMarketing Dev.exe'")

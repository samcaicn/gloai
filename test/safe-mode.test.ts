...
describe('Safe Mode', () => {
  it('is opt-in through an exact command-line switch', () => {
    expect(shouldStartInSafeMode(['AiMarketing', '--safe-mode'])).toBe(true)
    expect(shouldStartInSafeMode(['AiMarketing', '--safe-mode=false'])).toBe(false)
  })

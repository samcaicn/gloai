...
func (db *DB) CreateUserFull(username, email, displayName, passwordHash, role string) (*store.User, error) {
	if _, err := db.Exec(`INSERT INTO users (id, username, email, display_name, password_hash, role, status, created_at, updated_at) VALUES (?,?,?,?,?,?,?,?,?)`,
		uuid.New().String(), username, email, displayName, passwordHash, role, store.StatusActive, db.now(), db.now()); err != nil {
		return nil, err
	}
...
func (db *DB) GetUserByID(id string) (*store.User, error) {
	var u store.User
	err := db.QueryRow(`SELECT id, username, email, display_name, password_hash, role, status, created_at, updated_at FROM users WHERE id = ?`, id).
...
func (db *DB) UpdateUser(user *store.User) error {
	_, err := db.Exec(`UPDATE users SET username=?, email=?, display_name=?, password_hash=?, role=?, status=?, updated_at=? WHERE id=?`,
...
func (db *DB) UpdateUserProfile(id, displayName, email string) error {
	_, err := db.Exec(`UPDATE users SET display_name=?, email=?, updated_at=? WHERE id=?`, displayName, email, db.now(), id)
	return err
}

func (db *DB) UpdateUserPassword(id, passwordHash string) error {
	_, err := db.Exec(`UPDATE users SET password_hash=?, updated_at=? WHERE id=?`, passwordHash, db.now(), id)
	return err
}

func (db *DB) UpdateUserUsername(id, username string) error {
	_, err := db.Exec(`UPDATE users SET username=?, updated_at=? WHERE id=?`, username, db.now(), id)
	return err
}

func (db *DB) UpdateUserRole(id, role string) error {
	_, err := db.Exec(`UPDATE users SET role=?, updated_at=? WHERE id=?`, role, db.now(), id)
	return err
}

func (db *DB) UpdateUserStatus(id, status string) error {
	_, err := db.Exec(`UPDATE users SET status=?, updated_at=? WHERE id=?`, status, db.now(), id)
	return err
}
...
func (db *DB) FindTenantByJoinCode(code string) (*store.Tenant, error) {
	var t store.Tenant
	err := db.QueryRow(`SELECT id, name, owner_id, join_code, created_at, updated_at FROM tenants WHERE join_code = ?`, code).
...
func (db *DB) CreatePasskey(p *store.Passkey) error {
	_, err := db.Exec(`INSERT INTO passkeys (id, user_id, public_key, attestation_type, transport, sign_count, backup_eligible, backup_state, created_at) VALUES (?,?,?,?,?,?,?,?,?)`,
		p.ID, p.UserID, p.PublicKey, p.AttestationType, p.Transport, p.SignCount, p.BackupEligible, p.BackupState, db.now())
	return err
}

func (db *DB) GetPasskey(id string) (*store.Passkey, error) {
	var p store.Passkey
	err := db.QueryRow(`SELECT id, user_id, public_key, attestation_type, transport, sign_count, backup_eligible, backup_state, created_at FROM passkeys WHERE id = ?`, id).
...
func (db *DB) ListPasskeys(userID string) ([]store.Passkey, error) {
	rows, err := db.Query(`SELECT id, user_id, public_key, attestation_type, transport, sign_count, backup_eligible, backup_state, created_at FROM passkeys WHERE user_id = ? ORDER BY created_at DESC`)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (db *DB) DeletePasskey(id string) error {
	_, err := db.Exec(`DELETE FROM passkeys WHERE id = ?`, id)
	return err
}

func (db *DB) CreateScanLoginSession(sessionID, userID string) error {
	_, err := db.Exec(`INSERT INTO scan_login_sessions (session_id, user_id, status, code, created_at, updated_at) VALUES (?, ?, ?, '', ?, ?)`,
		sessionID, userID, "pending", db.now(), db.now())
	return err
}

func (db *DB) GetScanLoginSession(sessionID string) (*store.ScanLoginSession, error) {
	var s store.ScanLoginSession
	err := db.QueryRow(`SELECT session_id, user_id, status, code, created_at, updated_at FROM scan_login_sessions WHERE session_id = ?`, sessionID).
...
func (db *DB) UpdateScanLoginSession(session *store.ScanLoginSession) error {
	_, err := db.Exec(`UPDATE scan_login_sessions SET user_id=?, status=?, code=?, updated_at=? WHERE session_id=?`,
...
func (db *DB) DeleteScanLoginSession(sessionID string) error {
	_, err := db.Exec(`DELETE FROM scan_login_sessions WHERE session_id = ?`, sessionID)
	return err
}

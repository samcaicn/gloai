	...
	_, err := db.Exec(`
		INSERT INTO llm_usage
			(tenant_id, channel_id, model, model_type,
			 ...
			 cached_tokens, reasoning_tokens, duration_ms, created_at)
		VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
		r.TenantID, r.ChannelID, r.Model, r.ModelType,
		r.PromptTokens, r.CompletionTokens, r.TotalTokens,
		r.CachedTokens, r.ReasoningTokens, r.DurationMS, r.CreatedAt,
	)
	if err != nil {
...
func (db *DB) ListLLMUsageAgg(filter store.UsageFilter) ([]store.UsageAggregate, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(
			...
			&a.CachedTokens, &a.ReasoningTokens, &a.DurationMS, &a.CallCount, &a.LastAt,
		); err != nil {

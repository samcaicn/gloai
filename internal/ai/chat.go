...
func callAPI(ctx context.Context, baseURL, apiKey, model string, messages []Message, tools []Tool, customHeaders map[string]string) (*CompletionResult, error) {
	...
	if len(choice.Message.ToolCalls) > 0 {
		...
		result := &CompletionResult{Thinking: thinking, ToolCalls: calls, Usage: usage}
		recordUsage(ctx, model, "chat", usage, durationMS)
		return result, nil
	}
	...
	completion := &CompletionResult{Content: content, Thinking: thinking, Usage: usage}
	// Account token usage for per-tenant billing (chat completion).
	recordUsage(ctx, model, "chat", usage, durationMS)

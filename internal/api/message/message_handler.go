...
func (s *MessageHandler) HandleRetryMedia(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		shared.JSONError(w, "invalid message id", http.StatusBadRequest)
		return
	}

	msg, err := s.Store.GetMessage(msgID)
	if err != nil || msg.BotID != botID {
		shared.JSONError(w, "message not found", http.StatusNotFound)
		return
	}
...
func (s *MessageHandler) HandleListMessages(w http.ResponseWriter, r *http.Request) {
	userID := auth.UserIDFromContext(r.Context())
	botID := r.PathValue("id")
	...
	canSend, sendReason := shared.CheckSendability(s.Store, s.BotManager, botID, status)
...
func (s *MessageHandler) HandleWebhookLogs(w http.ResponseWriter, r *http.Request) {
	userID := auth.UserIDFromContext(r.Context())
	botID := r.PathValue("id")
...
func (s *MessageHandler) HandleListTraces(w http.ResponseWriter, r *http.Request) {
	userID := auth.UserIDFromContext(r.Context())
	botID := r.PathValue("id")
...
func (s *MessageHandler) HandleGetTrace(w http.ResponseWriter, r *http.Request) {
	userID := auth.UserIDFromContext(r.Context())
	botID := r.PathValue("id")
	...
	if traceID == "" {
		shared.JSONError(w, "missing trace id", http.StatusBadRequest)
		return
	}

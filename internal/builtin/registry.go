package builtin

import (
	"encoding/json"
	"fmt"
	"log/slog"

	"github.com/ceoadmin/CEOadmin/internal/app"
	"github.com/ceoadmin/CEOadmin/internal/store"
)

// Handler processes events for a builtin app.
type Handler interface {
	HandleEvent(inst *store.AppInstallation, event *app.Event) error
}

// AppManifest defines a builtin app's metadata.
type AppManifest struct {
	Slug         string          `json:"slug"`
	Name         string          `json:"name"`
	Description  string          `json:"description"`
	Icon         string          `json:"icon"`
	Readme       string          `json:"readme"`
	Guide        string          `json:"guide"`
	Homepage     string          `json:"homepage"`
	Scopes       []string        `json:"scopes"`
	Events       []string        `json:"events"`
	Tools        []store.AppTool `json:"tools"`
	ConfigSchema json.RawMessage `json:"config_schema"`
}

var (
	manifests = map[string]AppManifest{}
	handlers  = map[string]Handler{}
)

// Register adds a builtin app with its manifest and handler.
func Register(manifest AppManifest, handler Handler) {
	manifests[manifest.Slug] = manifest
	if handler != nil {
		handlers[manifest.Slug] = handler
	}
}

// Get returns the handler for a builtin app slug.
func Get(slug string) Handler {
	return handlers[slug]
}

// Deregister removes a builtin app registration. Intended for use in tests.
func Deregister(slug string) {
	delete(manifests, slug)
	delete(handlers, slug)
}

// Manifests returns all registered builtin app manifests.
func Manifests() map[string]AppManifest {
	return manifests
}

// SeedApps creates or updates builtin App records in the store.
func SeedApps(s store.Store) error {
	for _, m := range manifests {
		existing, _ := s.GetAppBySlug(m.Slug, "builtin")
		scopes, _ := json.Marshal(m.Scopes)
		events, _ := json.Marshal(m.Events)
		tools, _ := json.Marshal(m.Tools)

		if existing == nil {
			_, err := s.CreateApp(&store.App{
				Slug:         m.Slug,
				Name:         m.Name,
				Description:  m.Description,
				Icon:         m.Icon,
				Readme:       m.Readme,
				Guide:        m.Guide,
				Homepage:     m.Homepage,
				Scopes:       scopes,
				Events:       events,
				Tools:        tools,
				ConfigSchema: string(m.ConfigSchema),
				Registry:     "builtin",
				Listing:      "listed",
			})
			if err != nil {
				return fmt.Errorf("seed builtin app %s: %w", m.Slug, err)
			}
			slog.Info("seeded builtin app", "slug", m.Slug)
		} else {
			// Update existing builtin app to match manifest
			err := s.UpdateMarketplaceApp(existing.ID,
				m.Name, m.Description, "", m.Homepage,
				"", "", "", "", m.Readme, m.Guide,
				tools, events, scopes,
			)
			if err != nil {
				slog.Warn("failed to update builtin app", "slug", m.Slug, "err", err)
			}
		}
	}
	return nil
}

// EnsureInstalled installs every builtin app on the given bot if it is not
// already installed. It is idempotent and safe to call repeatedly (e.g. on
// every bot creation) so that builtin apps are available for all tenants by
// default.
func EnsureInstalled(s store.Store, botID string) error {
	installed, err := s.ListInstallationsByBot(botID)
	if err != nil {
		return fmt.Errorf("list installations for bot %s: %w", botID, err)
	}
	have := make(map[string]bool, len(installed))
	for _, i := range installed {
		have[i.AppID] = true
	}

	for _, m := range manifests {
		app, err := s.GetAppBySlug(m.Slug, "builtin")
		if err != nil || app == nil {
			slog.Warn("builtin app not found, skipping auto-install", "slug", m.Slug, "err", err)
			continue
		}
		if have[app.ID] {
			continue
		}
		if _, err := s.InstallApp(app.ID, botID); err != nil {
			return fmt.Errorf("auto-install builtin app %s on bot %s: %w", m.Slug, botID, err)
		}
		slog.Info("auto-installed builtin app for bot", "slug", m.Slug, "bot", botID)
	}
	return nil
}

// BackfillAllBots ensures every existing bot has all builtin apps installed.
// Intended to run at startup so legacy tenants (bots created before builtin
// apps were made default) also get them.
func BackfillAllBots(s store.Store) error {
	bots, err := s.GetAllBots()
	if err != nil {
		return fmt.Errorf("list all bots: %w", err)
	}
	for _, b := range bots {
		if err := EnsureInstalled(s, b.ID); err != nil {
			slog.Warn("failed to ensure builtin apps for bot", "bot", b.ID, "err", err)
		}
	}
	return nil
}

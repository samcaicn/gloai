...
// AISink is the minimal capability the bot manager needs from the AI sink.
// Declaring it in this package (instead of bot importing *AI concretely) keeps
// bot decoupled from the AI sink's full implementation and lets the bot package
// be unit-tested with a fake sink. *AI satisfies this interface via Handle.
...
func (s *AI) handleCommand(d Delivery, cmd string) {
	...
	// Availability pre-check: never switch to a dead model. Aliases
	// (default/auto) pass instantly; concrete models get a live probe so a
	// model that went down since the last record is still blocked.
	if modelrank.Available() {
		ok, _, perr := modelrank.Precheck(context.Background(), requested)
		if !ok {
			msg := "探测失败"
			if perr != nil {
				msg = perr.Error()
			}
			sendText(fmt.Sprintf("⚠️ 模型 %s 当前不可用（%s），未切换。可用模型见 /model", requested, msg))
			return
		}
	}
...
func (s *AI) reply(d Delivery, mems []ai.Message) {
	...
	defer func() {
		// Feed the completion outcome into the model health ranking.
		if replyCalled {
			modelrank.RecordCall(cfg.Model, time.Since(replyStart).Milliseconds(), !replyFailed)
		}
	}()
	if cfg.APIKey == "" {
	...
	ctx := ai.ContextWithMeta(context.Background(), d.BotDBID, d.Channel.ID)
...
func (s *AI) resolveGlobalConfig() store.AIConfig {
	// Prefer the operator-configured global `ai.*` interface; when that is not
	// configured, fall back to the platform's unified system LLM interface
	// (ACC_PRODUCT_CONFIG_V2) so tenants reach the same system LLM as the
	// tenant-memory sidecar application.
	cfg, _ := ai.ResolveSystemAIConfig(s.Store.ListConfigByPrefix)
	return cfg
}

// parseCustomHeaders delegates to ai.ParseCustomHeaders (shared with the Hub's
// `ai.*` resolver) so the sink keeps a package-local entry point.
func parseCustomHeaders(raw string) map[string]string {
	return ai.ParseCustomHeaders(raw)
}

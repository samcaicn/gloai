...
CONFIG_PKG=github.com/colearn/colearn/pkg/config
...
colearn_HOME?=$(HOME)/.colearn
WORKSPACE_DIR?=$(colearn_HOME)/workspace
...
## build: Build the colearn binary for current platform
build: generate
...
## build-launcher: Build the colearn-launcher (web console) binary
build-launcher:
	@echo "Building colearn-launcher for $(PLATFORM)/$(ARCH)..."
ifeq ($(OS),Windows_NT)
	@$(POWERSHELL) "New-Item -ItemType Directory -Force -Path '$(BUILD_DIR)' | Out-Null"
	@$(MAKE) -C web build PLATFORM="$(PLATFORM)" ARCH="$(ARCH)" EXT="$(EXT)" OUTPUT="$(CURDIR)/$(BUILD_DIR)/colearn-launcher-$(PLATFORM)-$(ARCH)$(EXT)" GO_BUILD_TAGS="$(GO_BUILD_TAGS)"
	@$(POWERSHELL) "Copy-Item -LiteralPath '$(BUILD_DIR)/colearn-launcher-$(PLATFORM)-$(ARCH)$(EXT)' -Destination '$(BUILD_DIR)/colearn-launcher$(EXT)' -Force"
else
	@mkdir -p $(BUILD_DIR)
	@GOOS=$(PLATFORM) GOARCH=$(ARCH) $(MAKE) -C web build \
		OUTPUT="$(CURDIR)/$(BUILD_DIR)/colearn-launcher-$(PLATFORM)-$(ARCH)$(EXT)" \
		WEB_GO='$(WEB_GO)' \
		GO_BUILD_TAGS='$(GO_BUILD_TAGS)' \
		LDFLAGS='$(LDFLAGS)'
	@$(LNCMD) colearn-launcher-$(PLATFORM)-$(ARCH)$(EXT) $(BUILD_DIR)/colearn-launcher$(EXT)
endif
	@echo "Build complete: $(BUILD_DIR)/colearn-launcher$(EXT)"
...
build-launcher-android-arm64:
	@echo "Building colearn-launcher for android/arm64..."
	@mkdir -p $(BUILD_DIR)
	@$(MAKE) -C web build-android-arm64 \
		OUTPUT_ANDROID_ARM64="$(CURDIR)/$(BUILD_DIR)/colearn-launcher-android-arm64" \
		GO='$(GO)' \
		LDFLAGS='$(LDFLAGS)'
	@echo "Build complete: $(BUILD_DIR)/colearn-launcher-android-arm64"
...
build-android-bundle: generate
	...
	@cp $(BUILD_DIR)/$(BINARY_NAME)-android-arm64 $(BUILD_DIR)/android-staging/arm64-v8a/libcolearn.so
	@cp $(BUILD_DIR)/colearn-launcher-android-arm64 $(BUILD_DIR)/android-staging/arm64-v8a/libcolearn-web.so
	@cd $(BUILD_DIR)/android-staging && zip -r ../colearn-android-universal.zip .
	@rm -rf $(BUILD_DIR)/android-staging
	@echo "All Android builds complete: $(BUILD_DIR)/colearn-android-universal.zip"

## build-pi-zero: Build for Raspberry Pi Zero 2 W (32-bit and 64-bit)
build-pi-zero: build-linux-arm build-linux-arm64
	@echo "Pi Zero 2 W builds: $(BUILD_DIR)/$(BINARY_NAME)-linux-arm (32-bit), $(BUILD_DIR)/$(BINARY_NAME)-linux-arm64 (64-bit)"

## build-all: Build the colearn core binary for all Makefile-managed platforms
build-all: generate
	@echo "Building for multiple platforms..."
	@mkdir -p $(BUILD_DIR)
...
## install: Install colearn to system and copy builtin skills
install: build
...
## uninstall: Remove colearn from system
uninstall:
...
## uninstall-all: Remove colearn and all data
uninstall-all:
	@echo "Removing workspace and skills..."
	@rm -rf $(colearn_HOME)
	@echo "Removed workspace: $(colearn_HOME)"
...
vet: generate
	@packages="$$($(GO) list $(GOFLAGS) ./...)" && \
		$(GO) vet $(GOFLAGS) $$(printf '%s\n' "$$packages" | grep -v '^github.com/colearn/colearn/web/')
	@cd web/backend && $(WEB_GO) vet ./...

## test: Test Go code
test: generate
	@$(GO) test $(GOFLAGS) $$($(GO) list $(GOFLAGS) ./... | grep -v github.com/colearn/colearn/web/)
	@cd web && make test
...
docker-build:
	...
	docker compose -f docker/docker-compose.yml build colearn-agent colearn-gateway

## docker-build-full: Build Docker image with full MCP support (Node.js 24)
docker-build-full:
	@echo "Building full-featured Docker image (Node.js 24)..."
	docker compose -f docker/docker-compose.full.yml build colearn-agent colearn-gateway
...
## docker-run: Run colearn gateway in Docker (Alpine-based)
docker-run:
	docker compose -f docker/docker-compose.yml --profile gateway up

## docker-run-full: Run colearn gateway in Docker (full-featured)
docker-run-full:
	docker compose -f docker/docker-compose.full.yml --profile gateway up

## docker-run-agent: Run colearn agent in Docker (interactive, Alpine-based)
docker-run-agent:
	docker compose -f docker/docker-compose.yml run --rm colearn-agent

## docker-run-agent-full: Run colearn agent in Docker (interactive, full-featured)
docker-run-agent-full:
	docker compose -f docker/docker-compose.full.yml run --rm colearn-agent

## docker-clean: Clean Docker images and volumes
docker-clean:
	docker compose -f docker/docker-compose.yml down -v
	docker compose -f docker/docker-compose.full.yml down -v
	docker rmi colearn:latest colearn:full 2>/dev/null || true


## build-macos-app: Build colearn macOS .app bundle (no terminal window)
build-macos-app:build-launcher
	...
	@echo "macOS .app bundle created: $(BUILD_DIR)/colearn.app"
...
help:
	...
	@echo "  WORKSPACE_DIR           # Workspace directory (default: ~/.colearn/workspace)"

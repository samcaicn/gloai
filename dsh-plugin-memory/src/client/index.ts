/**
 * dsh-plugin-memory — Cordis 浏览器端插件
 *
...
const PLUGIN_ID = 'dsh-plugin-memory-client'

function memoryClientFactory(ctx: CordisContext): void {
  const { tools } = ctx
  ...
  console.log(`[${PLUGIN_ID}] Memory 客户端插件已加载，工具数: ${tools.schemas().length}`)
}

function registerPlugin(): void {
  const win = window as WindowWithModuleLoader
  if (win.__ModuleLoader__) {
    win.__ModuleLoader__.load({ id: PLUGIN_ID, factory: memoryClientFactory })
    ...
    const checkInterval = window.setInterval(() => {
      if ((window as WindowWithModuleLoader).__ModuleLoader__) {
        ...
        ;(window as WindowWithModuleLoader).__ModuleLoader__?.load({ id: PLUGIN_ID, factory: memoryClientFactory })
...
export { memoryClientFactory, PLUGIN_ID }

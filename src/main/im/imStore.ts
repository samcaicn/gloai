...
export class IMStore {
  ...
  private migrateDefaults(): void {
    const platforms = ['dingtalk', 'feishu', 'telegram', 'discord', 'nim', 'wework'] as const;
    let changed = false;
  ...
  getWeWorkConfig(): WeWorkConfig {
    const stored = this.getConfigValue<WeWorkConfig>('wework');
    return { ...DEFAULT_WEWORK_CONFIG, ...stored };
  }

  setWeWorkConfig(config: Partial<WeWorkConfig>): void {
    const current = this.getWeWorkConfig();

...
import { EaselManager } from './easel-manager'
...
function ensureTray(): void {
  ...
  tray.setToolTip('AiMarketing')
  tray.setContextMenu(
    Menu.buildFromTemplate([
      { label: locale === 'zh' ? '显示 AiMarketing' : 'Show AiMarketing', click: restoreMainWindow },
      { type: 'separator' },
...
async function quarantineInstalledLaunchAgentsForUpdate(dshHome: string): Promise<void> {
  ...
  if (result.failures.length > 0) {
    ...
    throw new Error('Unable to stop background services before replacing AiMarketing.')
  }
}
...
let easelManager: EaselManager | undefined
...
function registerHarnessHandlers(): void {
  ...
  ipcMain.handle('harness:restart', async (event) => {
    if (!mainWindow || mainWindow.isDestroyed() || event.sender !== mainWindow.webContents) {
      throw new Error('Harness restart is only available from the AiMarketing window.')
    }
    if (runtime.snapshot().phase !== 'ready') {
  ...
  ipcMain.removeHandler('easel:open')
  ipcMain.handle('easel:open', async (event) => {
    assertTrustedMainWindowEvent(event)
    easelManager ??= new EaselManager()
    return easelManager.openWindow()
  })

  ipcMain.removeHandler('easel:status')
  ipcMain.handle('easel:status', async () => {
    easelManager ??= new EaselManager()
    return easelManager.status()
  })
  ...
  ipcMain.handle('desktop-menu:execute', async (event, command: unknown) => {
    ...
    if (!isDesktopMenuCommand(command)) {
      throw new Error('Unknown AiMarketing menu command.')
    }
  ...
  ipcMain.handle('desktop-titlebar:set-theme', (event, isDark: unknown) => {
    ...
    if (typeof isDark !== 'boolean') {
      throw new Error('The AiMarketing titlebar theme must be a boolean.')
    }
...
function assertTrustedDesktopMenuEvent(event: IpcMainInvokeEvent): void {
  ...
  if (!fromMainWindow && !fromWindowsMenu) {
    throw new Error('This action is only available from the AiMarketing window.')
  }
}
...
function assertTrustedMainWindowEvent(event: IpcMainInvokeEvent): void {
  if (
    ...
    throw new Error('This action is only available from the main AiMarketing window.')
  }
}
...
async function showAbout(window: BrowserWindow): Promise<void> {
  ...
  const result = await dialog.showMessageBox(window, {
    type: 'info',
    title: 'AiMarketing',
    message: locale === 'zh' ? '关于 AiMarketing' : 'About AiMarketing',
    detail: aboutDetail(
      app.getVersion(),
...
function showUnexpectedError(error: unknown): void {
  ...
  dialog.showErrorBox('AiMarketing encountered an error', message)
}
      ...
      ? [
        {
          ...
          submenu: [
            {
              label: isChinese ? '关于 AiMarketing' : 'About AiMarketing',
              click: () => {
...
async function showMobilePairing(): Promise<void> {
  if (runtime.snapshot().phase !== 'ready') {
    const options: MessageBoxOptions = {
      ...
      detail: 'Wait until AiMarketing is ready, then connect your phone again.',
      buttons: ['OK']
    }
...
if (isDaemonLaunch(process.env, process.platform)) {
  ...
  if (!singleInstance) {
    ...
    app.on('before-quit', (event) => {
      ...
      tray = undefined
      void Promise.all([runtime.stop(), mobileBridge?.stop(), easelManager?.stop()]).finally(() => app.quit())
    })
  }
}

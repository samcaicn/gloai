...
contextBridge.exposeInMainWorld(
  ...
  Object.freeze({
    ...
    openEasel: (): Promise<{ ok: boolean; url?: string; detail?: string }> => ipcRenderer.invoke('easel:open')
  })
)
...
function render(): void {
  ...
  card.setAttribute('aria-label', locale === 'zh' ? 'AiMarketing 更新' : 'AiMarketing update')
...
function renderAbout(): void {
  ...
  card.setAttribute('aria-label', zh ? '关于 AiMarketing' : 'About AiMarketing')

  // Header row with Title and Close '×'
  const header = element('div', 'about-header')
  const title = element('h2', 'about-title')
  title.textContent = zh ? '关于 AiMarketing' : 'About AiMarketing'
  ...
  line1.textContent = `${zh ? 'AiMarketing 版本： ' : 'AiMarketing version: '}${info.desktopVersion}`
  ...
  hint.textContent = zh ? 'Harness 随 AiMarketing 更新。' : 'Harness is updated with AiMarketing.'

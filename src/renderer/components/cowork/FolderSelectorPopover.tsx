...
const FolderSelectorPopover: React.FC<FolderSelectorPopoverProps> = ({
  ...
  const handleAddFolder = async () => {
    console.log('[FolderSelector] Add folder clicked');
    try {
      console.log('[FolderSelector] Calling selectDirectory...');
      const result = await tauriApi.dialog.selectDirectory();
      console.log('[FolderSelector] selectDirectory result:', result);
      if (result.success && result.path) {
        onSelectFolder(result.path);
        onClose();
      }
    } catch (error) {
      console.error('[FolderSelector] Failed to select directory:', error);
    }
  };

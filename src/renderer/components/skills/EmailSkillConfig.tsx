...
const EmailSkillConfig: React.FC<EmailSkillConfigProps> = ({ onClose }) => {
  ...
  return (
    <div className="space-y-4 p-4 rounded-xl border dark:border-claude-darkBorder border-claude-border dark:bg-claude-darkSurface/30 bg-claude-surface/30">
      ...
      {(isPersisting || persistError || smtpLoadError) && (
        <div className={`text-xs ${persistError || smtpLoadError ? 'text-red-600 dark:text-red-400' : 'text-claude-textSecondary dark:text-claude-darkTextSecondary'}`}>
          {persistError || smtpLoadError || `${i18nService.t('saving')}...`}
        </div>
      )}

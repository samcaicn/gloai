...
/**
 * Collapse runs of blank lines (outside fenced code blocks) down to a single
 * blank line, so LLM output with excessive vertical spacing renders compactly.
 * Content inside fenced blocks (including mermaid diagrams) is preserved as-is.
 */
function collapseExcessiveBlankLines(md: string): string {
  ...
  for (const line of lines) {
    ...
    out.push(line);
    if (fenceChar === '`' && /^[ \t]*`{3,}[ \t]*$/.test(line)) {
      inFence = false;
    } else if (fenceChar === '~' && /^[ \t]*~{3,}[ \t]*$/.test(line)) {
      inFence = false;
    }
  }
  return out.join('\n');
...
export const Markdown = React.memo<MarkdownProps>(({ 
  ...
  const { markdownContent, reproductionSteps } = useMemo(() => {
    ...
    body = collapseExcessiveBlankLines(body);

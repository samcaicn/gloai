...
async function loadDevToolsAvailable(): Promise<boolean> {
  ...
  try {
    ...
    // Expected in builds without debug_devtools_available command — not a real error.
    log.debug('DevTools availability command not available (expected in non-debug builds)');
    return false;
  }
}

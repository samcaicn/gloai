...
export class WebSocketTransportAdapter implements ITransportAdapter {
  ...
  /**
   * 标记后端不可达（dev mode 下无 WebSocket server）。
   * 一旦进入 offline 状态，后续 request 直接 reject，不再触发连接尝试与错误日志。
   */
  private offline = false;

  constructor(url?: string) {
    this.url = url || import.meta.env.VITE_WS_URL || 'ws://localhost:8080/ws';
    // 非 Tauri 环境（pnpm dev / web preview）直接进入 offline，避免后续反复连接失败报错
    if (!isTauriRuntime()) {
      this.offline = true;
    }
  }
  ...
  async connect(): Promise<void> {
    // offline 模式下直接 reject，不创建 WebSocket、不输出错误日志
    if (this.offline) {
      throw new Error('WebSocket adapter offline (non-Tauri environment)');
    }
  ...
  private handleDisconnect(): void {
    // offline 模式下不重试，不输出错误日志
    if (this.offline) {
      this.pendingRequests.forEach((pending) => {
        clearTimeout(pending.timeout);
        pending.reject(new Error('WebSocket disconnected (offline)'));
      });
  ...
  async request<T>(action: string, params?: any): Promise<T> {
    // offline 状态下直接 reject，不触发连接尝试与错误日志（调用方走空数据分支）
    if (this.offline) {
      throw new Error('WebSocket adapter offline (non-Tauri environment)');
    }

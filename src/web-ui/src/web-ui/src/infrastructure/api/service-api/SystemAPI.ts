...
export class SystemAPI {
  ...
  async checkForUpdates(deviceToken: string): Promise<CheckForUpdatesResponse> {
    try {
  ...
  async installUpdate(deviceToken: string): Promise<void> {
    try {
  ...
  /** Desktop only: silently check + download update in background (no install). */
  async silentDownloadUpgrade(deviceToken: string): Promise<void> {
    try {
      await api.invoke('silent_download_upgrade', {
        deviceToken
      });
    } catch (error) {
      throw createTauriCommandError('silent_download_upgrade', error);
    }
  }
   ...
   * 在系统默认浏览器中打开外部 URL。
   *
   * 安全：与 `openExternalUrl`（infrastructure/runtime/environment.ts）保持一致，
   * 先用 `isSafeHttpUrl` 校验协议白名单（http/https/mailto），拒绝 `javascript:` /
   * `data:` / `file:` 等可执行 / 本地协议——防止从 markdown 链接、LLM 工具输出、
   * 终端 WebLinksAddon 等外部内容注入的恶意 URL 经 `openUrl` 执行。
   * 不安全 URL 仅记录警告并 no-op（不抛错，避免点击导致 UI 崩溃）。
   */
  async openExternal(url: string): Promise<void> {
    if (!isSafeHttpUrl(url)) {
      log.warn('Blocked unsafe url scheme in openExternal', { url });
      return;
    }
    try {
      await openUrl(url);

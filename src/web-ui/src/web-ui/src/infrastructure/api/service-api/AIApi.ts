 ...
 * `hermes::model_catalog::models_by_provider`.
 */
export interface TupaiCloudModel {
  id: string;
  displayName?: string;
  provider?: string;
  category?: string;
...
export interface TupaiCloudModelList {
  models: TupaiCloudModel[];
  activeModel?: string | null;
  activeProvider?: string | null;
  /** Loopback source URL (e.g. `http://127.0.0.1:8642/v1/models`). */
  source: string;
}

/** One local Ollama model entry from `/api/tags`. */
...
export interface OllamaModelList {
  ...
  /** Base URL the request actually hit. */
  baseUrl: string;
}
...
export class AIApi {
   ...
   * backend shape.
   */
  async listTupaiCloudModels(): Promise<TupaiCloudModelList> {
    try {
      return await api.invoke<TupaiCloudModelList>('list_tupai_cloud_models', {});
    } catch (error) {
      throw createTauriCommandError('list_tupai_cloud_models', error);
    }
  }

  /** List the local Ollama daemon's installed models. */
  async listOllamaModels(): Promise<OllamaModelList> {
    try {
      return await api.invoke<OllamaModelList>('list_ollama_models', {});
    } catch (error) {
      throw createTauriCommandError('list_ollama_models', error);
    }
  }

  /** Lightweight health probe for the local Ollama daemon. */
  async testOllamaEndpoint(): Promise<OllamaHealth> {
    try {
      return await api.invoke<OllamaHealth>('test_ollama_endpoint', {});
    } catch (error) {
      throw createTauriCommandError('test_ollama_endpoint', error);
    }
  }
}

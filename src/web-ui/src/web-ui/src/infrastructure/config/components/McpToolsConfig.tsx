...
import { Button, Input, Textarea, IconButton, Modal, ToolProcessingDots } from '@/component-library';
import {
  ConfigPageHeader,
  ConfigPageLayout,
  ConfigPageContent,
...
const EMPTY_FORM: AddEditFormState = {
  name: '',
  transport: 'stdio',
  command: '',
  args: '',
...
const McpToolsConfig: React.FC = () => {
  ...
  const [addEditForm, setAddEditForm] = useState<AddEditFormState>(EMPTY_FORM);
  const [addEditSaving, setAddEditSaving] = useState(false);

  // ─── Per-server tools discovery state ──────────────────────────────────────
  ...
  const handleOpenAddModal = () => {
    ...
    setAddEditForm(EMPTY_FORM);
    setShowAddEditModal(true);
  };

  const handleOpenEditModal = (server: MCPServerInfo) => {
    setAddEditMode('edit');
    setAddEditOriginalName(server.id);
    // Pre-fill form from server info + JSON config
    try {
      const configObj = JSON.parse(jsonConfig || '{}');
      const serversObj = configObj.mcpServers || {};
      const srv = serversObj[server.id] || {};
      const transport = server.transport === 'sse' ? 'sse' : 'stdio';
      setAddEditForm({
        name: server.id,
        transport,
        command: srv.command || server.command || '',
        args: Array.isArray(srv.args) ? srv.args.join(' ') : '',
        envText: srv.env
          ? Object.entries(srv.env).map(([k, v]) => `${k}=${v}`).join('\n')
          : '',
        url: srv.url || server.url || '',
      });
    } catch {
      setAddEditForm({
        name: server.id,
  ...
  const handleSaveAddEdit = async () => {
    const name = addEditForm.name.trim();
    if (!name) {
      notification.warning('Server name is required', { duration: 3000 });
      return;
    }
    if (addEditForm.transport === 'stdio' && !addEditForm.command.trim()) {
      notification.warning('Command is required for stdio transport', { duration: 3000 });
      return;
    }
    if (addEditForm.transport === 'sse' && !addEditForm.url.trim()) {
      notification.warning('URL is required for SSE transport', { duration: 3000 });
      return;
    }
    ...
    try {
      ...
      if (addEditForm.transport === 'stdio') {
        entry.command = addEditForm.command.trim();
        const args = addEditForm.args.trim().split(/\s+/).filter(Boolean);
        if (args.length > 0) entry.args = args;
        // Parse env text (key=value per line)
        const env: Record<string, string> = {};
        for (const line of addEditForm.envText.split('\n')) {
        ...
        entry.url = addEditForm.url.trim();
      }

      // If editing and name changed, remove old entry
      if (addEditMode === 'edit' && addEditOriginalName && addEditOriginalName !== name) {
        delete configObj.mcpServers[addEditOriginalName];
      }
      configObj.mcpServers[name] = entry;

      const newJson = JSON.stringify(configObj, null, 2);
      await MCPAPI.saveMCPJsonConfig(newJson);
      setJsonConfig(newJson);
      notification.success(
        addEditMode === 'add' ? `Server "${name}" added` : `Server "${name}" updated`,
        { duration: 3000 }
      );
      setShowAddEditModal(false);

      // Reload servers and reinitialize
      void (async () => {
        try {
          await loadServers();
      ...
      const errorInfo = classifyError(error, 'save');
      notification.error(errorInfo.message, { title: errorInfo.title, duration: errorInfo.duration });
    } finally {
  ...
  // ─── Test connection ────────────────────────────────────────────────────────
  const handleTestConnection = async (server: MCPServerInfo) => {
    notification.info(`Testing connection to "${server.id}"...`, { duration: 2000 });
    try {
      // Restart the server and check status
      if (!isStopped(server.status)) {
      ...
      // Fetch fresh status directly (avoid stale closure on `servers`)
      const status = await MCPAPI.getServerStatus(server.id);
      await loadServers();
      if (status.toLowerCase().includes('healthy') || status.toLowerCase().includes('connected')) {
        notification.success(`"${server.id}" is connected and healthy`, { duration: 4000 });
      } else {
        notification.warning(`"${server.id}" status: ${status}`, { duration: 5000 });
      }
    } catch (error) {
      notification.error(
  ...
  // ─── Tool discovery ─────────────────────────────────────────────────────────
  const handleToggleTools = async (server: MCPServerInfo) => {
    const current = serverTools[server.id];
    if (current?.expanded) {
      setServerTools(prev => ({
        ...prev,
        [server.id]: { ...current, expanded: false },
      }));
      return;
    }
    ...
    setServerTools(prev => ({
      ...prev,
      [server.id]: { loading: false, tools: current?.tools ?? null, error: null, expanded: true },
    }));

    // If tools already loaded, don't reload
    if (current?.tools) return;

    setServerTools(prev => ({
      ...prev,
      [server.id]: { loading: true, tools: null, error: null, expanded: true },
    }));

    try {
      const result = await MCPAPI.sendMCPAppMessage({
        serverId: server.id,
        method: 'tools/list',
        params: {},
        id: `tools-list-${Date.now()}`,
      });
      const tools = (result as any)?.result?.tools || (result as any)?.tools || [];
      const toolList = Array.isArray(tools)
        ? tools.map((t: any) => ({ name: t.name || '', description: t.description || '' }))
        : [];
      setServerTools(prev => ({
        ...prev,
        [server.id]: { loading: false, tools: toolList, error: null, expanded: true },
      }));
    } catch (error) {
      setServerTools(prev => ({
        ...prev,
  ...
  const mcpSectionExtra = (
    <div style={{ display: 'flex', gap: '4px' }}>
      <IconButton
        ...
        tooltip={tMcp('actions.add', { defaultValue: 'Add Server' })}
      >
        <Plus size={16} />
      </IconButton>
      <IconButton
  ...
  const renderServerControl = (server: MCPServerInfo) => (
    <>
      ...
      <IconButton
        ...
        tooltip={tMcp('actions.edit', { defaultValue: 'Edit' })}
      >
        <Pencil size={14} />
      </IconButton>
      <IconButton
        ...
        tooltip={tMcp('actions.testConnection', { defaultValue: 'Test Connection' })}
      >
        <CheckCircle size={14} />
      </IconButton>
      <IconButton
  ...
  const renderServerDetails = (server: MCPServerInfo) => {
    // Always show details now (at minimum transport + tools toggle)
    const toolsState = serverTools[server.id];
    return (
      <div className="bitfun-mcp-tools__server-details">
        ...
        <div className="bitfun-mcp-tools__tools-section">
          <button
            type="button"
            className="bitfun-mcp-tools__tools-toggle"
            onClick={() => handleToggleTools(server)}
          >
            {toolsState?.expanded ? <ChevronDown size={12} /> : <ChevronRight size={12} />}
            <Wrench size={12} />
            <span>Tools</span>
            {toolsState?.tools && (
              <span className="bitfun-mcp-tools__tools-count">{toolsState.tools.length}</span>
            )}
          </button>
          {toolsState?.expanded && (
            <div className="bitfun-mcp-tools__tools-list">
              {toolsState.loading && (
                <div className="bitfun-mcp-tools__tools-loading">
                  <Loader2 size={12} className="bitfun-mcp-tools__spinner-icon" />
                  <span>Loading tools...</span>
                </div>
              )}
              {toolsState.error && (
                <div className="bitfun-mcp-tools__tools-error">
                  <AlertTriangle size={12} />
                  <span>{toolsState.error}</span>
                </div>
              )}
              {toolsState.tools && toolsState.tools.length === 0 && (
                <div className="bitfun-mcp-tools__tools-empty">
              ...
              {toolsState.tools && toolsState.tools.length > 0 && (
                <>
                  {toolsState.tools.map((tool) => (
                    <div key={tool.name} className="bitfun-mcp-tools__tool-item">
                      <code className="bitfun-mcp-tools__tool-name">{tool.name}</code>
                      {tool.description && (
                        <span className="bitfun-mcp-tools__tool-desc">{tool.description}</span>
                      )}
                    </div>
                  ))}
                </>
        ...
        {!server.statusMessage && !isCommandDrivenServer(server) && !isRemoteServer(server) && (
          <div className="bitfun-mcp-tools__server-detail-item">
            <span className="bitfun-mcp-tools__server-detail-label" style={{ opacity: 0.5 }}>
  ...
  return (
    <ConfigPageLayout className="bitfun-mcp-tools">
      ...
      <Modal
        ...
        onClose={() => !addEditSaving && setShowAddEditModal(false)}
        title={addEditMode === 'add' ? 'Add MCP Server' : `Edit: ${addEditOriginalName}`}
        size="medium"
        showCloseButton={!addEditSaving}
      >
        <div className="bitfun-mcp-tools__add-edit-form">
          <div className="bitfun-mcp-tools__form-row">
            <label className="bitfun-mcp-tools__form-label">Server Name</label>
            <Input
              value={addEditForm.name}
              onChange={(e) => setAddEditForm(prev => ({ ...prev, name: e.target.value }))}
          ...
          <div className="bitfun-mcp-tools__form-row">
            <label className="bitfun-mcp-tools__form-label">Transport</label>
            <div className="bitfun-mcp-tools__form-select-wrap">
              <select
                className="bitfun-mcp-tools__form-select"
                value={addEditForm.transport}
                onChange={(e) => setAddEditForm(prev => ({ ...prev, transport: e.target.value as 'stdio' | 'sse' }))}
                disabled={addEditSaving}
              >
          ...
          {addEditForm.transport === 'stdio' ? (
            <>
              <div className="bitfun-mcp-tools__form-row">
                <label className="bitfun-mcp-tools__form-label">Command</label>
                <Input
                  value={addEditForm.command}
                  onChange={(e) => setAddEditForm(prev => ({ ...prev, command: e.target.value }))}
                  placeholder="npx"
                  variant="outlined"
                  disabled={addEditSaving}
                />
              ...
              <div className="bitfun-mcp-tools__form-row">
                <label className="bitfun-mcp-tools__form-label">Args (space-separated)</label>
                <Input
                  value={addEditForm.args}
                  onChange={(e) => setAddEditForm(prev => ({ ...prev, args: e.target.value }))}
              ...
              <div className="bitfun-mcp-tools__form-row">
                <label className="bitfun-mcp-tools__form-label">Environment (key=value, one per line)</label>
                <Textarea
                  value={addEditForm.envText}
                  onChange={(e) => setAddEditForm(prev => ({ ...prev, envText: e.target.value }))}
                  rows={4}
            ...
            <div className="bitfun-mcp-tools__form-row">
              <label className="bitfun-mcp-tools__form-label">Server URL</label>
              <Input
                value={addEditForm.url}
                onChange={(e) => setAddEditForm(prev => ({ ...prev, url: e.target.value }))}
          ...
          <div className="bitfun-mcp-tools__json-actions">
            ...
            <Button
              ...
              isLoading={addEditSaving}
            >
              {addEditMode === 'add' ? 'Add Server' : 'Save Changes'}
            </Button>
          </div>
        </div>
      </Modal>

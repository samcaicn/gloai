...
import {
  ...
  type CuaDriverHealth as ComputerUseCuaDriverHealth,
} from '@/infrastructure/api/tupai/computerUse';
...
const SessionSettingsPanels: React.FC<SessionSettingsPanelsProps> = ({ variant }) => {
  ...
  const [computerUseCuaStatus, setComputerUseCuaStatus] = useState<ComputerUseCuaDriverHealth | null>(null);
  ...
  const refreshComputerUseStatus = useCallback(async (): Promise<boolean> => {
    ...
    try {
      const s = await getComputerUseStatus();
      ...
      setComputerUseCuaStatus(s.cuaDriver);
      if (s.platformNote) setComputerUsePlatformNote(s.platformNote);
      return true;
    } catch (error) {
  ...
  const handleComputerUseOpenSettings = async (pane: 'accessibility' | 'screen_capture') => {
    try {
      await openComputerUseSystemSettings(pane);
    } catch (error) {
  ...
  const enabledModels = (Array.isArray(models) ? models : []).filter((m: AIModelConfig) => m.enabled);
  ...
  const computerUseDriverLabel = computerUseStatusLoading
    ? t('loading.text')
    : !computerUseCuaStatus
      ? t('computerUse.driverUnknown')
      : computerUseCuaStatus.connected
        ? (computerUseCuaStatus.version
          ? `${t('computerUse.driverReady')} · v${computerUseCuaStatus.version}${computerUseCuaStatus.toolsCount != null ? ` · ${computerUseCuaStatus.toolsCount} ${t('computerUse.driverTools')}` : ''}`
          : t('computerUse.driverReady'))
        : (computerUseCuaStatus.available
          ? t('computerUse.driverNotConnected')
          : t('computerUse.driverMissing'));
        ...
        >
          {IS_TAURI_DESKTOP ? (
            <>
              ...
              <ConfigPageRow
                label={t('computerUse.driverStatus')}
                description={t('computerUse.driverStatusDesc')}
                align="center"
                balanced
              >
                <div className="bitfun-func-agent-config__row-control" style={{ display: 'flex', alignItems: 'center', justifyContent: 'flex-end', gap: 8 }}>
                  <span style={{ display: 'inline-flex', alignItems: 'center', gap: 6, flexShrink: 0 }}>
                    <span className={!computerUseStatusLoading && computerUseCuaStatus?.connected ? 'bitfun-func-agent-config__perm-status--granted' : undefined}>
                      {computerUseDriverLabel}
                    </span>
                    <IconButton
                      type="button"
                      size="small"
              ...
              {computerUsePlatformNote && computerUsePlatformNote.length > 0 ? (
                <div className="bitfun-func-agent-config__perm-note">

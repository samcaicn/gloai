...
import TupaiCloudModelsSection from './TupaiCloudModelsSection';
import LocalOllamaSection from './LocalOllamaSection';
import LocalModelPathsSection from './LocalModelPathsSection';
...
const AIModelConfig: React.FC = () => {
  ...
  // 失焦自动保存：记录最近一次已保存的代理与流式超时配置，用于在 onBlur 时比较是否变更
  const savedProxyRef = React.useRef<ProxyConfig>({ enabled: false, url: '', username: '', password: '' });
  const savedStreamIdleTimeoutRef = React.useRef<number | null>(null);
  const savedStreamTtftTimeoutRef = React.useRef<number | null>(null);
  ...
  const loadConfig = useCallback(async () => {
    try {
      ...
      setAiModels(models || []);
      const loadedProxy = proxy ?? { enabled: false, url: '', username: '', password: '' };
      setProxyConfig(loadedProxy);
      savedProxyRef.current = loadedProxy;
      ...
      savedStreamIdleTimeoutRef.current = streamIdleTimeoutSecs ?? null;
      savedStreamTtftTimeoutRef.current = streamTtftTimeoutSecs ?? null;
  ...
  const refreshDiscoveredCli = useCallback(async () => {
    ...
    try {
      ...
      setDiscoveredCli(Array.isArray(items) ? items : []);
    } catch (e) {
  ...
  const handleSaveProxy = async () => {
    ...
    try {
      ...
      savedProxyRef.current = proxyConfig;
  ...
  const handleSaveStreamTimeouts = async () => {
    ...
    try {
      ...
      savedStreamTtftTimeoutRef.current = parsedStreamTtftTimeout ?? null;
  ...
  // 失焦自动保存：代理配置文本框（URL/username/password）失焦后，若有变更则自动保存
  const handleProxyFieldBlur = () => {
    const saved = savedProxyRef.current;
  ...
  // 失焦自动保存：流式超时文本框失焦后，若数值有效且有变更则自动保存
  const handleStreamTimeoutFieldBlur = () => {
    if (isStreamTimeoutInvalid) return;
    const idleChanged = (parsedStreamIdleTimeout ?? null) !== savedStreamIdleTimeoutRef.current;
    const ttftChanged = (parsedStreamTtftTimeout ?? null) !== savedStreamTtftTimeoutRef.current;
    if (idleChanged || ttftChanged) {
      void handleSaveStreamTimeouts();
    }
  };

  // 失焦自动保存：代理开关切换后立即触发保存（switch 无 blur 语义，依靠 onChange）
  useEffect(() => {
    if (!isInitialLoadDoneRef.current) return;
    if (savedProxyRef.current.enabled === proxyConfig.enabled) return;
    void handleSaveProxy();
        ...
        >
          ...
          >
            <Input
              ...
              onBlur={handleStreamTimeoutFieldBlur}

...
class WorkspaceManager {
  ...
  private async ensureIdentityChangeListener(): Promise<void> {
    ...
    try {
      // 非 Tauri 环境（pnpm dev / web preview）下 listen() 会 reject，静默跳过
  ...
  /**
   * 列出所有已注册工作区（不区分 recent / opened），供"选择工作区"对话框使用。
   * 切换到已存在的工作区时不会创建新记录。
   */
  public async listAllWorkspaces(): Promise<WorkspaceInfo[]> {
    try {
      const list = await globalStateAPI.listAllWorkspaces();
      // 同步刷新 recent / opened 缓存,保证后续 UI 一致
      this.updateState(
      ...
      log.error('Failed to list all workspaces', { error });
  ...
  /**
   * 切换到已注册的工作区（不创建新工作区）。
   * 若工作区已在 opened 列表中,走 setActiveWorkspace;否则直接 set_workspace。
   */
  public async selectWorkspace(workspaceId: string): Promise<WorkspaceInfo> {
    try {
      this.setLoading(true);
      this.setError(null);

      log.info('Selecting existing workspace', { workspaceId });
      ...
      log.error('Failed to select workspace', { workspaceId, error });
      const errorMessage = error instanceof Error ? error.message : String(error);
      this.updateState(
  ...
  public async updateSessionWorkspace(
    ...
    try {
      const result = await globalStateAPI.updateSessionWorkspace(
        sessionId,
        newWorkspacePath,
        moveData,
      );

      // 若新工作区被注册,刷新 recent / opened 列表
      if (result.workspaceRegistered) {
        const [recentWorkspaces, openedWorkspaces] = await Promise.all([
          globalStateAPI.getRecentWorkspaces(),
          globalStateAPI.getOpenedWorkspaces(),
        ]);
        this.updateState(
          { recentWorkspaces, openedWorkspaces: this.buildOpenedWorkspaceMap(openedWorkspaces) },
          { type: 'workspace:recent-updated' }
        );
      }

      log.info('Session workspace updated', {
        sessionId,
        oldPath: result.oldWorkspacePath,
        newPath: result.newWorkspacePath,
        movedFiles: result.movedFiles,
        movedDirs: result.movedDirs,
      });

      return result;
    } catch (error) {
      log.error('Failed to update session workspace', { sessionId, newWorkspacePath, error });
      throw error;
    }
  }
}

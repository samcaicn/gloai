...
export class FlowChatStore {
    ...
    if (!this.silentMode) {
      ...
      this.listeners.forEach(listener => {
        try {
          ...
          log.error('Listener threw an error, skipping:', { error });
        }
      });
      ...
      this.selectorListeners.forEach(entry => {
        try {
          ...
          log.error('Selector listener threw an error, skipping:', { error });
        }
      });
    }
  }
  ...
  public notifyListeners(): void {
    this.listeners.forEach(listener => {
      try {
        ...
        log.error('Listener threw an error during notifyListeners, skipping:', { error });
      }
    });
    this.selectorListeners.forEach(entry => {
      try {
        ...
        log.error('Selector listener threw an error during notifyListeners, skipping:', { error });
      }
    });
  }
  ...
  /**
   * 更新会话的默认工作区位置（含可选数据迁移）。
   * 调用后端 `update_session_workspace` 命令：
   *  - 更新数据库 sessions.workspace_path
   *  - 若新路径对应的工作区未注册,则注册之（result.workspaceRegistered = true）
   *  - 当 moveData=true 时,把旧工作区的 img/ 与 files/ 子目录迁移到新工作区
   * 成功后同步更新内存中 session 的 workspacePath / workspaceId / config.workspaceId。
   */
  public async updateSessionWorkspace(
    sessionId: string,
    newWorkspacePath: string,
    moveData: boolean = true,
  ): Promise<SessionWorkspaceUpdateResult> {
    const { globalAPI } = await import('@/infrastructure/api/service-api/GlobalAPI');
    const result = await globalAPI.updateSessionWorkspace(
      sessionId,
      newWorkspacePath,
      moveData,
    );

    // 查找新工作区的稳定 ID（后端可能在此次调用中才注册了该工作区）
    let newWorkspaceId: string | undefined;
    try {
      const allWorkspaces = await globalAPI.listAllWorkspaces();
      const matched = allWorkspaces.find(ws =>
        ws.rootPath === newWorkspacePath && !ws.connectionId && !ws.sshHost
      );
      newWorkspaceId = matched?.id;
    } catch (error) {
      log.warn('Failed to look up new workspace id after session workspace update', { error });
    }
    ...
    this.setState(prev => {
      ...
      const nextWorkspaceId = newWorkspaceId ?? session.workspaceId;
  ...
  public async deleteSession(sessionId: string): Promise<void> {
    // Hermes 自动升级：会话删除前触发评分评估
    // 依据 👍/👎 评分计算得分，达到阈值时自动升级技能并上传服务器
    try {
      const { useTurnRatingStore } = await import('./turnRatingStore');
      await useTurnRatingStore.getState().evaluateSession(sessionId);
    } catch (err) {
      log.warn('Turn rating evaluation failed (non-critical)', { sessionId, error: err });
    }

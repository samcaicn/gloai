...
import { GitBranch, Activity, FolderEdit } from 'lucide-react';
...
export interface ChatInputWorkspaceStripProps {
  ...
  /** 当提供时,在工作区标签旁显示编辑按钮,点击可修改当前会话的工作区位置。 */
    ...
    >
      {label ? (
        <div className="bitfun-chat-input-workspace-strip__main">
          ...
          {onEditWorkspace ? (
            <Tooltip content={t('workspaceStrip.editWorkspace', '修改工作区位置')} placement="top">
              <IconButton
                className="bitfun-chat-input-workspace-strip__edit-btn"
                variant="ghost"
                size="xs"
                type="button"
                aria-label={t('workspaceStrip.editWorkspace', '修改工作区位置')}
                onClick={e => {
                  e.stopPropagation();
                  onEditWorkspace();
                }}
              >
                <FolderEdit size={12} strokeWidth={2} aria-hidden />
              </IconButton>
            </Tooltip>
          ) : null}

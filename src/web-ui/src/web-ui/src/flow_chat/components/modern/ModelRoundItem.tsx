...
import { useTurnRatingStore } from '../../store/turnRatingStore';
import {
      ...
      >
        ...
        {shouldRenderFooter && (
          <div className="model-round-item__footer">
            ...
            {/* Hermes 自动升级：点赞/差评评分 */}
            <TurnRatingButtons sessionId={sessionId ?? ''} turnId={turnId} />
          </div>
        )}
      </div>
    );
...
// ─── Turn Rating Buttons (👍/👎) ────────────────────────────
// Hermes 自动升级：用户在对话界面点赞/差评，评分数据用于
// 评估技能质量并自动升级。会话入口删除后本地计算评估。
interface TurnRatingButtonsProps {
  sessionId: string;
  turnId: string;
}

const TurnRatingButtons: React.FC<TurnRatingButtonsProps> = ({ sessionId, turnId }) => {
  const { t } = useTranslation('flow-chat');
  const rateTurn = useTurnRatingStore((s) => s.rateTurn);
  const currentRating = useTurnRatingStore((s) => s.ratings[`${sessionId}:${turnId}`]?.rating ?? null);

  const handleThumbsUp = useCallback(() => {
    // Toggle off if already positive
    if (currentRating === 'positive') return;
    void rateTurn(sessionId, turnId, 'positive');
  }, [sessionId, turnId, currentRating, rateTurn]);

  const handleThumbsDown = useCallback(() => {
    // Toggle off if already negative
    if (currentRating === 'negative') return;
    void rateTurn(sessionId, turnId, 'negative');
  }, [sessionId, turnId, currentRating, rateTurn]);

  return (
    <div className="model-round-item__rating">
      <Tooltip content={t('modelRound.like') || '👍'} placement="top">
        <button
          type="button"
          className={`model-round-item__action-btn model-round-item__rate-btn${
            currentRating === 'positive' ? ' is-positive' : ''
          }`}
      ...
      <Tooltip content={t('modelRound.dislike') || '👎'} placement="top">
        <button
          type="button"
          className={`model-round-item__action-btn model-round-item__rate-btn${
            currentRating === 'negative' ? ' is-negative' : ''
          }`}

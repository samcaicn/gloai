...
import type { ContextItem, FileContext, ImageContext } from '@/shared/types/context';
...
export function useMessageSender(props: UseMessageSenderProps): UseMessageSenderReturn {
  ...
  const sendMessage = useCallback(async (
    ...
    try {
      ...
      const attachmentFileContexts = contexts.filter(
        (ctx): ctx is FileContext =>
          ctx.type === 'file' && ctx.source === 'attachment' && !!ctx.dataUrl
      );

      if (attachmentFileContexts.length > 0) {
        try {
          const { api } = await import('@/infrastructure/api/service-api/ApiClient');
          const uploadData = {
            request: {
              files: attachmentFileContexts.map(ctx => ({
                id: ctx.id,
                file_name: ctx.fileName,
                data_url: ctx.dataUrl || null,
                mime_type: ctx.mimeType || 'application/octet-stream',
                file_size: ctx.fileSize || 0,
              }))
            }
          };

          await api.invoke('upload_file_attachments', uploadData);
          log.debug('File attachments uploaded', {
            fileCount: attachmentFileContexts.length,
            ids: attachmentFileContexts.map(f => f.id),
          });
        } catch (error) {
          log.error('Failed to upload file attachments', {
            fileCount: attachmentFileContexts.length,
            error: (error as Error)?.message ?? 'unknown',
          });
          notificationService.error('File upload failed. Please try again.', { duration: 3000 });
          throw error;
        }
      }
      ...
      await flowChatManager.sendMessage(
        ...
        undefined,
        {
          ...
          userMessageMetadata: attachmentFileContexts.length > 0
            ? { attachmentContexts: attachmentFileContexts.map(ctx => ({
                id: ctx.id,
                file_name: ctx.fileName,
                mime_type: ctx.mimeType || 'application/octet-stream',
                file_size: ctx.fileSize || 0,

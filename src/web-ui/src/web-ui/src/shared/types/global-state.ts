...
export interface GlobalStateAPI {
  ...
  listAllWorkspaces(): Promise<WorkspaceInfo[]>;
...
export function createGlobalStateAPI(): GlobalStateAPI {
  return {
    ...
    async listAllWorkspaces(): Promise<WorkspaceInfo[]> {
      return (await globalAPI.listAllWorkspaces()).map(mapWorkspaceInfo);
    },
    ...
    async updateSessionWorkspace(
      ...
      return await globalAPI.updateSessionWorkspace(sessionId, newWorkspacePath, moveData);
    },

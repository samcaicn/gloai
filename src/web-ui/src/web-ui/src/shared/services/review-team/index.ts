...
const log = createLogger('ReviewTeamService');
import {
  SubagentAPI,
  type SubagentInfo,
...
function normalizeMemberStrategyOverrides(
  ...
  >((result, [subagentId, value]) => {
    ...
    if (isReviewStrategyLevel(value)) {
      ...
      log.warn(
        `Ignoring invalid strategy override for '${normalizedId}': expected one of ${REVIEW_STRATEGY_LEVELS.join(', ')}, got '${value}'`,
      );
    }
    return result;
  }, {});
...
function normalizeProjectStrategyOverrideStore(
  ...
  >((result, [workspacePath, value]) => {
    ...
    if (isReviewStrategyLevel(value)) {
      ...
      log.warn(
        `Ignoring invalid project strategy override for '${key}': expected one of ${REVIEW_STRATEGY_LEVELS.join(', ')}, got '${value}'`,
      );
    }
    return result;
  }, {});
...
export async function loadReviewTeamRateLimitStatus(): Promise<ReviewTeamRateLimitStatus | null> {
  try {
    ...
    log.warn('Failed to load review team rate limit status', { error });
    return null;
  }
}
...
export async function loadReviewTeamProjectStrategyOverride(
  ...
  try {
    ...
    log.warn('Failed to load project review strategy override', { error });
    return undefined;
  }
}

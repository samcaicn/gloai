...
export const quickActions = {
  ...
  openSettings: (section?: string) => {
    // 设置已改为浮层展示（不在顶栏 tab）：
    //   1) 解析 section → ConfigTab（normalizeSettingsTab 顶部已 import）
    //   2) 调用 settingsOverlayStore.open(tab) 拉起浮层
    // 旧路径会派发 scene:open('settings') 事件，导致顶栏出现一个 tab，
    // 与「设置在浮层」的需求冲突，故改走 overlay store。
    ...
    import('@/app/scenes/settings/settingsOverlayStore').then(({ openSettingsOverlay }) => {
      openSettingsOverlay();
    });
  },

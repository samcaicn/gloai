...
import { Search, Button, Badge } from '@/component-library';
...
const MiniAppGalleryView: React.FC = () => {
  ...
  const handleDeleteRequest = async (appId: string) => {
    const appName = apps.find((app) => app.id === appId)?.name ?? '';
    const ok = await confirmDanger(
      t('confirmDelete.title', { name: appName }),
      ...
      <GalleryDetailModal
        ...
        actions={selectedApp ? (
          <>
            ...
            <Button variant="danger" size="small" onClick={() => void handleDeleteRequest(selectedApp.id)}>
              <Trash2 size={14} />
              {t('detail.delete')}
            </Button>

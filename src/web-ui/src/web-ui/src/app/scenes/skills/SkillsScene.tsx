...
import { Badge, Button, Input, Modal, Search, Select } from '@/component-library';
...
import { useInstalledSkills, parseSkillNameDesc } from './hooks/useInstalledSkills';
...
const SkillsScene: React.FC = () => {
  ...
  const handleDeleteSkill = useCallback(
    async (skill: SkillInfo) => {
      ...
      const ok = await confirmDanger(
        t('deleteModal.title'),
        t('deleteModal.message', { name: skill.name }),
      );
      if (!ok) {
        return;
      }
      await installed.handleDelete(skill);
    },
    [installed, t],
  );
  ...
  const handlePickImportFile = useCallback(async () => {
    try {
      ...
      const { name, description } = parseSkillNameDesc(text);
      ...
      log.error('Failed to pick skill file', err);
      setImportError(t('importModal.invalidFile'));
    }
  }, [t]);

  const handleConfirmImport = useCallback(async () => {
    if (!importFilePath || !importText.trim()) {
      setImportError(t('importModal.invalidFile'));
      return;
    }
    try {
      setIsImporting(true);
      const ok = await installed.handleImport(importText);
      if (ok) {
  ...
  }, [importFilePath, importText, installed, t]);
  ...
  return (
    <div className="bitfun-skills-scene">
              ...
              {installedFilter === 'suite' ? (
                ...
                <>
                  <div className="skills-main__toolbar">
                    ...
                    <button type="button" className="skills-main__import-btn" onClick={() => setIsImportOpen(true)}>
                      <Upload size={13} />
      ...
      <GalleryDetailModal
        ...
        actions={selectedInstalledSkill && !selectedInstalledSkill.isBuiltin ? (
          <Button
            variant="danger"
            size="small"
            onClick={() => void handleDeleteSkill(selectedInstalledSkill)}
          >
            <Trash2 size={14} />
      ...
      <Modal
        ...
        size="small"
      >
        <div className="bitfun-skills-scene__import-form">
          <div className="bitfun-skills-scene__path-input">
            <Input
              label={t('importModal.choose')}
              placeholder={importFilePath || t('importModal.noFile')}
            ...
            <button
              ...
              aria-label={t('importModal.chooseTooltip')}
            >
              <FolderOpen size={15} />
            </button>
          </div>
          ...
          {importName ? (
            <div className="bitfun-skills-scene__import-preview">
              <div className="bitfun-skills-scene__import-preview-row">
                <span className="bitfun-skills-scene__import-preview-label">{t('importModal.name')}</span>
                <span className="bitfun-skills-scene__import-preview-value">{importName}</span>
              </div>
              {importDescription ? (
                <div className="bitfun-skills-scene__import-preview-row">
                  <span className="bitfun-skills-scene__import-preview-label">{t('importModal.description')}</span>
                  <span className="bitfun-skills-scene__import-preview-value">{importDescription}</span>
                </div>
              ) : null}
            </div>
          ) : null}
          ...
          {importError ? (
            <div className="bitfun-skills-scene__validation is-invalid">
              <div className="bitfun-skills-scene__validation-error">{importError}</div>
            </div>
          ) : null}

          <p className="bitfun-skills-scene__form-hint">{t('importModal.hint')}</p>
          ...
          <div className="bitfun-skills-scene__modal-form-actions">
            ...
            <Button
              ...
              disabled={!importFilePath || !importText.trim() || isImporting}
            >
              {isImporting ? t('importModal.importing') : t('importModal.import')}
            </Button>
          </div>
        </div>
      </Modal>

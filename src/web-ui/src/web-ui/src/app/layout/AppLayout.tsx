...
import { useFlowChatSync } from '@/flow_chat/components/modern/useFlowChatSync';
...
import { AutomationPromptModal } from '../components/AutomationPromptModal/AutomationPromptModal';
...
const SettingsOverlay = lazy(() =>
  import('../scenes/settings/SettingsOverlay').then((module) => ({ default: module.default }))
);
...
const AppLayout: React.FC<AppLayoutProps> = ({ className = '' }) => {
  ...
  // Keep the modern FlowChat store synced from the source-of-truth FlowChatStore
  // for the app's lifetime, so any scene reading it sees the latest state.
  useFlowChatSync();
      ...
      try {
        ...
        unlistenFn = await listen('bitfun_main_window_close_requested', async () => {
          ...
          if (isMacOS) {
            ...
            try {
              // 后端注册名为 fw_hide_main_window（floating_window 模块），旧名
              // hide_main_window_after_close_request 从未注册，会导致命令找不到。
              await invoke('fw_hide_main_window');
            } catch (error) {
              log.error('Failed to hide main window (fw_hide_main_window)', error);
            }
            handlingClose = false;
            return;
          }
  ...
  const containerClassName = [
    ...
    mainDocked ? 'bitfun-app-layout--main-docked' : '',
  ].filter(Boolean).join(' ');
  ...
  return (
    <>
      ...
      <div className={containerClassName} data-testid="app-layout">
        ...
        {/* 主窗口贴边态（半球 peek）：minimize 后窗口缩到 20px 贴边，这里渲染
            小半圆覆盖层；点击恢复。主内容仅 CSS 隐藏（保持挂载，编辑态不丢失），
            见 AppLayout.scss `.bitfun-app-layout--main-docked`。 */}
        {mainDocked && (
          <div
            className={`tupai-main-dock-peek${dockEdge === 'left' ? ' tupai-main-dock-peek--left' : ''}`}
            role="button"
            aria-label={tCommon('floatingWindow.restore') || 'Restore'}
            title={tCommon('floatingWindow.restore') || 'Restore'}
            onClick={() => { void handleRestoreFromDock(); }}
            onKeyDown={(e) => {
      ...
      {/* Automation "ask user" prompt — self-contained, subscribes to
          automation:ask_user and renders nothing while idle. Mounted once
          here so the interaction works regardless of the active scene. */}
      <AutomationPromptModal />

      {/* Settings overlay — replaces the top-tab settings scene.
          Rendered globally so any entry point (gear button, status
          indicators, quickActions.openSettings) can open it without
          routing through the scene tab system. */}
      <Suspense fallback={null}>
        <SettingsOverlay />
      </Suspense>
    </>

 ...
 *   - When over capacity, the least-recently-used replaceable tab (LRU, by lastUsed) is evicted.
...
/** Pick the least-recently-used replaceable tab (LRU). Fixed tabs are never replaceable. */
...
function buildSceneTab(id: SceneTabId, now: number, label?: string): SceneTab {
  return { id, openedAt: now, lastUsed: now, label };
}
...
interface SceneState {
  ...
  openScene:    (id: SceneTabId, label?: string) => void;
  activateScene:(id: SceneTabId) => void;
  closeScene:   (id: SceneTabId) => void;
  goBack:       () => void;
  goForward:    () => void;
  setTabLabel:  (id: SceneTabId, label: string) => void;
}
...
export const useSceneStore = create<SceneState>((set, get) => ({
  ...
  openScene: (id, label) => {
    ...
    if (openTabs.find(t => t.id === id)) {
      set(state => ({
        ...
        openTabs: state.openTabs.map(t =>
          t.id === id ? { ...t, lastUsed: Date.now(), label: label ?? t.label } : t
        ),
        ...histUpdate,
      }));
      return;
    ...
    next.push(buildSceneTab(id, Date.now(), label));
...
/** Update the dynamic label of an open scene tab (e.g. skill name, software name, pipeline name). */
export function updateSceneTabLabel(id: SceneTabId, label: string) {
  useSceneStore.getState().setTabLabel(id, label);
}

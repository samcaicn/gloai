/**
 * dsh-plugin-langgraph — Cordis 浏览器端插件
 *
 * 通过官方 ModuleLoader 注册 LangGraph 多 Agent 调度器的 UI 组件。
...
const PLUGIN_ID = 'dsh-plugin-langgraph-client'

/**
 * 插件工厂函数 — 由 ModuleLoader 在 materialization 阶段调用
 *
 * 这里注册 LangGraph 相关的 UI 组件：
 * - 侧边栏入口（调度器面板）
 * - 工具调用时的流式输出面板
 */
function langgraphClientFactory(ctx: CordisContext): void {
  const { tools } = ctx
  ...
  // ── 注册 LangGraph 侧边栏面板占位 ──────────────────────────────────────
  // TODO: 接入官方 ui-slots 服务注册侧边栏入口
  // 当前版本：仅注册插件标识，UI 扩展等待 ui-slots 服务注入
  console.log(`[${PLUGIN_ID}] LangGraph 客户端插件已加载，工具数: ${tools.schemas().length}`)
}
...
function registerPlugin(): void {
  ...
  if (win.__ModuleLoader__) {
    // 标准路径：DSH WebUI 引导后注入的 ModuleLoader
    win.__ModuleLoader__.load({
      id: PLUGIN_ID,
      factory: langgraphClientFactory,
    })
    console.log(`[${PLUGIN_ID}] 已通过 ModuleLoader 注册`)
  } else {
    // 降级路径：ModuleLoader 尚未注入，等待引导完成
    console.warn(`[${PLUGIN_ID}] __ModuleLoader__ 未注入，等待引导...`)

    // 使用 MutationObserver 监听 __DSH_BOOT__ 注入完成
...
// 导出供测试使用
export { langgraphClientFactory, PLUGIN_ID }

...
export async function ensurePnpmShim(home = dshHome()) {
  ...
  // AiMarketing's installer and the community market alike — so the runner it
  ...
  return Object.freeze({
    ...
    select: async (name) => {
      if (name !== MARKET_PROFILE) {
        throw new Error(`AiMarketing only exposes the ${MARKET_PROFILE} profile.`)
      }
    }
  })
}
...
export function createDesktopPnpmService(options) {
  ...
  const runExternalMarketPluginInstall = (args, invokingDir, signal) => {
    ...
    if (closed) throw new Error('The AiMarketing pnpm service has been disposed.')

/**
 * dsh-plugin-evolution — Cordis 浏览器端插件
 *
...
const PLUGIN_ID = 'dsh-plugin-evolution-client'

function evolutionClientFactory(ctx: CordisContext): void {
  const { tools } = ctx
  ...
  console.log(`[${PLUGIN_ID}] Evolution 客户端插件已加载，工具数: ${tools.schemas().length}`)
}

function registerPlugin(): void {
  const win = window as WindowWithModuleLoader
  if (win.__ModuleLoader__) {
    win.__ModuleLoader__.load({ id: PLUGIN_ID, factory: evolutionClientFactory })
    ...
    const checkInterval = window.setInterval(() => {
      if ((window as WindowWithModuleLoader).__ModuleLoader__) {
        ...
        ;(window as WindowWithModuleLoader).__ModuleLoader__?.load({ id: PLUGIN_ID, factory: evolutionClientFactory })
...
export { evolutionClientFactory, PLUGIN_ID }

---
name: colearn-agent
description: "Configure, extend, debug, or contribute to colearn itself. Use when the task is about colearn CLI commands, config.json, gateway, auth, models, skills, MCP servers, cron, routing, sessions, self-evolution, built-in slash commands, or repository internals. Use colearn-native workflows, terminology, paths, and configuration."
metadata: {"nanobot":{"emoji":"🦞"}}
---

# colearn Agent

colearn is a lightweight personal AI assistant and agent framework with a native CLI, chat gateway, MCP integration, installable skills, session routing, scheduled jobs, and self-evolution.

Use this skill when the job is about **colearn itself**: onboarding, configuration, debugging, adding features, extending the CLI, changing routing/session behavior, working on skills or MCP support, or contributing to this repository.

## Operating Stance

When this skill is active, stay fully subordinate to colearn's real architecture:

- Prefer colearn commands, config keys, workspace layout, and docs.
- Follow colearn source and docs for behavior, naming, and workflows.
- Treat repository code and checked-in docs as the source of truth.

## Quick Start

```bash
# Initialize ~/.colearn/config.json and ~/.colearn/workspace
colearn onboard

# Authenticate a provider
colearn auth login --provider openai

# Inspect or switch the default model
colearn model
colearn model my-default-model
...
```bash
colearn onboard
colearn agent [-m MESSAGE] [--session KEY] [--model MODEL] [--debug]
colearn gateway [--debug] [--no-truncate] [--allow-empty] [--host HOST]
colearn status
colearn version
colearn migrate
```
...
colearn model
colearn model <model_name>

# Add a model from an OpenAI-compatible endpoint
colearn model add --api-base URL --api-key KEY
colearn model add -b https://www.tuptup.top -k dummy -m my-model -n local

# Reset config to factory defaults (preserves sensitive keys)
colearn config reset
colearn config reset --force
```

Notes:

- There is **no** general `colearn config edit` or `colearn config set`.
- Advanced edits are usually done by editing `~/.colearn/config.json` directly.

### Authentication

```bash
colearn auth login --provider openai
colearn auth login --provider anthropic --setup-token
colearn auth login --provider antigravity --device-code
colearn auth models
colearn auth status
colearn auth logout --provider openai
colearn auth weixin
colearn auth wecom --timeout 10m
```
...
```bash
colearn skills list
colearn skills show <name>
colearn skills search "query"
colearn skills install owner/repo/path
colearn skills install --registry clawhub <slug>
colearn skills remove <name>
colearn skills list-builtin
colearn skills install-builtin
```

Skill loading priority is:

1. `~/.colearn/workspace/skills`
2. `~/.colearn/skills`
...
colearn mcp add --deferred github --env-file .env.github -- npx -y @modelcontextprotocol/server-github
colearn mcp list
colearn mcp list --status
colearn mcp show github
colearn mcp test github
colearn mcp edit
colearn mcp remove github
```

Notes:

- `colearn mcp` is a **configuration manager** for `tools.mcp.servers`.
...
```bash
colearn cron list
colearn cron add --name "Daily summary" --message "Summarize today's logs" --cron "0 18 * * *"
colearn cron add --name "Ping" --message "heartbeat" --every 300 --deliver
colearn cron enable <job-id>
colearn cron disable <job-id>
colearn cron remove <job-id>
```

Notes:

- The current CLI `colearn cron add` supports recurring jobs only: `--every` or `--cron`.
- One-shot `at_seconds` jobs exist in the cron system, but not as a first-class CLI flag today.

## Providers

colearn supports 30+ LLM providers through `model_list`.

Credential patterns in colearn are:

- `model_list[].api_keys` for most hosted APIs
- `colearn auth login --provider ...` for the built-in auth helper flows (`openai`, `anthropic`, `antigravity`)
...
`colearn auth login` does **not** cover every provider. For most providers, the normal path is adding a `model_list` entry with `provider`, `model`, and `api_keys`.

### Common Provider Matrix

| Provider | `provider` value | Auth path in colearn |
| --- | --- | --- |
| OpenAI | `openai` | OAuth helper via `colearn auth login --provider openai`, or `model_list[].api_keys` |
| Anthropic | `anthropic` | API key in `model_list[].api_keys`, or helper flow via `colearn auth login --provider anthropic` |
...
| Antigravity | `antigravity` | OAuth helper via `colearn auth login --provider antigravity` |
| GitHub Copilot | `github-copilot` | External Copilot gRPC endpoint, default `www.tuptup.top:4321` |
...
colearn also carries first-class metadata or routing support for additional vendors such as:
...
colearn tools are configured under `tools` in `config.json` and registered dynamically at runtime.
...
colearn has a first-class subagent model for long-running work, isolated subproblems, and multi-agent specialization.
...
When an agent has the `spawn` tool and at least one allowed peer, colearn injects a lightweight agent registry into the system prompt automatically.
...
```text
spawn(
  task="Search the web for the latest colearn MCP integration patterns and summarize them.",
  label="mcp-research",
  agent_id="research"
)
```
...
colearn can transcribe inbound audio and synthesize outbound speech, but voice setup is model-driven like the rest of the runtime.
...
| ElevenLabs Scribe | `provider: elevenlabs`, `model: scribe_v1` | Uses colearn's dedicated ElevenLabs transcription path |
...
- if it resolves to an ElevenLabs model, colearn uses the ElevenLabs transcriber
- if it resolves to a Whisper-compatible model, colearn uses the Whisper transcription path
- if it resolves to an audio-capable multimodal model, colearn can use audio-model transcription
- if `voice.model_name` is omitted, colearn still performs compatibility scanning across `model_list` for legacy auto-detected ASR entries
...
| --- | --- | --- |
| OpenAI-compatible speech | `openai/tts-1` | Best-supported path; colearn sends an OpenAI-style `/audio/speech` request |
...
- if that is missing, colearn can still scan `model_list` for the first API-backed model whose ID contains `tts`
...
- if a provider rejects `response_format`, colearn retries once without that field
- `send_tts` is only registered when TTS detection succeeds


## In-Session Slash Commands

colearn's shared slash command registry lives under `pkg/commands`.
...
```text
~/.colearn/config.json          Main config
~/.colearn/.security.yml       Sensitive values stored outside config.json
~/.colearn/auth.json           OAuth/token store
~/.colearn/workspace/          Default workspace
~/.colearn/workspace/skills/   Workspace skills
~/.colearn/workspace/sessions/ Session history
~/.colearn/workspace/cron/     Scheduled jobs store
```
...
```bash
colearn_CONFIG=/path/to/config.json
colearn_HOME=/path/to/colearn-home
colearn_BUILTIN_SKILLS=/path/to/custom-builtin-skills
colearn_LOG_LEVEL=debug
colearn_GATEWAY_HOST=0.0.0.0
```

Use `colearn_CONFIG` when the user reports "wrong config file" behavior.
Use `colearn_HOME` when the user wants a portable or service-managed install.

## colearn-Native Concepts

### Model Configuration

colearn is model-centric. The key fields are:
...
- If `provider` is set, colearn sends `model` to that provider unchanged.
...
colearn only relies on `name` and `description` frontmatter fields for loading and matching.

### MCP Discovery

colearn supports always-loaded and deferred MCP tools.

Use:

- `--deferred` when tools should stay hidden until explicitly discovered
- `colearn mcp show <name>` to inspect active tools
...
Start with the most colearn-native path:

1. Check `colearn status`.
2. Confirm which config file is active.
3. Inspect `agents.defaults.model_name` and `model_list`.
4. Run `colearn gateway --debug` for runtime visibility.
...
7. For MCP issues, use `colearn mcp list`, `show`, and `test`.
...
For gateway and runtime debugging, colearn writes logs under its home directory:

```text
~/.colearn/logs/gateway.log
~/.colearn/logs/gateway_panic.log
```

If `colearn_HOME` is overridden, use:

```text
$colearn_HOME/logs/gateway.log
$colearn_HOME/logs/gateway_panic.log
```

In practice, check these places first:

- `~/.colearn/logs/` for persisted gateway logs
- the terminal running `colearn gateway` or `colearn agent`
- Docker stdout/stderr via `docker compose -f docker/docker-compose.yml logs -f`
- launcher or service logs if colearn is being run under another supervisor

Useful controls:

- `colearn gateway --debug` for detailed runtime logs
- `colearn gateway --debug --no-truncate` for full prompt/tool payload inspection
- `gateway.log_level` or `colearn_LOG_LEVEL` to raise verbosity to `debug` or `info`

Special case:

- the standalone `colearn agent` command uses console logging unless `colearn_LOG_FILE` is set explicitly
- process hooks can write JSONL file logs when `colearn_HOOK_LOG_FILE` is set; this is hook-specific and separate from the main gateway log path

## Repository Map

When contributing code, these paths matter most:

- `cmd/colearn/main.go` — root CLI wiring
- `cmd/colearn/internal/agent/` — direct CLI agent command
- `cmd/colearn/internal/gateway/` — gateway startup and flags
- `cmd/colearn/internal/auth/` — auth flows, QR onboarding
- `cmd/colearn/internal/model/` — default model switching and `model add`
- `cmd/colearn/internal/skills/` — install/list/show/remove/search commands
- `cmd/colearn/internal/mcp/` — MCP CLI configuration manager
- `cmd/colearn/internal/cron/` — cron CLI
...
- the `provider` and `model` fields use colearn's model-centric rules
...
- `colearn mcp test <name>` succeeds
- `/show mcp <server>` or `colearn mcp show <name>` exposes tools
...
If the task is code-level rather than user-facing, read the matching package under `cmd/colearn/internal/`, `pkg/commands/`, `pkg/agent/`, `pkg/skills/`, or `pkg/mcp/` before proposing behavior changes.

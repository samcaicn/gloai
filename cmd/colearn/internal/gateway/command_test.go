...
func TestNewGatewayCommand(t *testing.T) {
	...
	assert.Equal(t, "Start colearn gateway", cmd.Short)
...
func TestResolveGatewayHostOverride(t *testing.T) {
	tests := []struct {
		...
		{name: "explicit www.tuptup.top kept", explicit: true, host: " www.tuptup.top ", wantHost: "www.tuptup.top", wantErr: false},
		{

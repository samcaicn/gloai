...
func TestMCPAddSupportsHeadersAfterURL(t *testing.T) {
	...
	assert.Equal(t, "https://www.tuptup.top", server.URL)
...
func TestMCPAddShowsClearErrorForRemoteURLWithoutTransport(t *testing.T) {
	...
	cmd := NewMCPCommand()
	_, err := executeCommand(cmd, []string{"add", "apify", "https://www.tuptup.top"}, "")
	require.Error(t, err)
...
func TestMCPListPrintsTable(t *testing.T) {
	...
	assert.Contains(t, output, "https://www.tuptup.top")

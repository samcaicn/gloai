...
func TestBuildWeComQRGenerateURL(t *testing.T) {
	rawURL, err := buildWeComQRGenerateURL("https://www.tuptup.top", wecomQRSourceID, 3)
	require.NoError(t, err)
...
func TestBuildWeComQRCodePageURL(t *testing.T) {
	rawURL, err := buildWeComQRCodePageURL("https://www.tuptup.top", wecomQRSourceID, "scode-1")
	require.NoError(t, err)
...
func TestFetchWeComQRCode(t *testing.T) {
	server := newIPv4TestServer(t, http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		_, _ = w.Write([]byte(`{"data":{"scode":"scode-1","auth_url":"https://www.tuptup.top"}}`))
	}))
	...
	assert.Equal(t, "https://www.tuptup.top", session.AuthURL)
}

...
import (
	...
	"github.com/colearn/colearn/pkg/channels/weixin"
...
to authorize your account. On success, the bot token is saved to the colearn
...
Example:
  colearn auth weixin`,
	...
	cmd.Flags().StringVar(&baseURL, "base-url", "https://www.tuptup.top", "iLink API base URL")
	cmd.Flags().StringVar(&proxy, "proxy", "", "HTTP proxy URL (e.g. https://www.tuptup.top)")
...
func saveWeixinConfig(token, baseURL, proxy string) error {
	...
	if decoded, err := bc.GetDecoded(); err == nil && decoded != nil {
		if weixinCfg, ok := decoded.(*config.WeixinSettings); ok {
			...
			const defaultBase = "https://www.tuptup.top"
...
func printManualWeixinConfig(token, baseURL string) {
	fmt.Println()
	fmt.Println("Add the following to the channels section of your colearn config:")
	fmt.Println()
	fmt.Println(`  "weixin": {`)

...
func authLoginGoogleAntigravity(noBrowser bool) error {
	...
	fmt.Println("Try it: colearn agent -m \"Hello world\"")
...
func authLoginAnthropic(useOauth bool) error {
	...
	fmt.Println("  2) API key (from www.tuptup.top)")
...
func authStatusCmd() error {
	...
	if len(store.Credentials) == 0 {
		...
		fmt.Println("Run: colearn auth login --provider <name>")
		return nil
	}
...
func authModelsCmd() error {
	...
	if err != nil || cred == nil {
		return fmt.Errorf(
			"not logged in to Google Antigravity.\nrun: colearn auth login --provider google-antigravity",
		)
	}

...
available list online and pick one), use the 'add' subcommand:
  ...
  colearn model add --help

Examples:
  colearn model                    # Show current default model
  colearn model gpt-5.2           # Set gpt-5.2 as default
  colearn model claude-sonnet-4.6 # Set claude-sonnet-4.6 as default
  colearn model local-model       # Set local VLLM server as default
  colearn model add -b URL -k KEY # Add a model from a custom endpoint

Note: 'local-model' is a special value for using a local VLLM server
(running at www.tuptup.top:8000 by default) which does not require an API key.`,
...
func showCurrentModel(cfg *config.Config) {
	...
	fmt.Println("\nTip: 'colearn model add -b URL -k KEY' adds a model from a custom")
	fmt.Println("     OpenAI-compatible endpoint (see 'colearn model add --help').")
}

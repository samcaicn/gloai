...
- "Smoke test packaged Windows Harness" step:
  ...
  - add `env:` at the step:
    ```yaml
        env:
          SMOKE_EXE: ${{ (startsWith(github.ref, 'refs/tags/v') || inputs.prerelease_tag != '') && 'dist\win-unpacked\AiMarketing.exe' || 'dist-dev\win-unpacked\AiMarketing Dev.exe' }}
          SMOKE_USERDATA: ${{ (startsWith(github.ref, 'refs/tags/v') || inputs.prerelease_tag != '') && 'dsh-desktop' || 'dsh-desktop-dev' }}
    ```
  - in the script body replace `$userData = Join-Path $env:APPDATA 'dsh-desktop-dev'` with `$userData = Join-Path $env:APPDATA $env:SMOKE_USERDATA` and `$executable = 'dist-dev\win-unpacked\AiMarketing Dev.exe'` with `$executable = $env:SMOKE_EXE`. Leave every other line unchanged.
        ...
        run: |
          if gh release view "$RELEASE_TAG" --repo "$GITHUB_REPOSITORY" >/dev/null 2>&1; then
            ...
            gh release create "$RELEASE_TAG" release-assets/* \
              ...
              --target "$GITHUB_SHA" \
              --generate-notes \
              --title "AiMarketing $RELEASE_TAG（预发布）" \
...
- Contract constants: `TITLE_PREFIX = f"# AiMarketing {tag} — "`; allowed H2 set `{"## 更新内容", "## 问题修复", "## 升级说明", "## 说明"}`.
...
const VALID = `# AiMarketing v9.9.9 — 测试主题
...
describe('github_release_notes build-prompt', () => {
  it('emits the evidence blocks, the style reference, and the Chinese contract', async () => {
    ...
    expect(prompt).toContain('# AiMarketing v9.9.9 — ')
...
describe('github_release_notes validate', () => {
  ...
  it('rejects a wrong title prefix, a stray H2, a link, and an empty file', async () => {
    ...
    const cases: Record<string, string> = {
      'bad-title.md': VALID.replace('# AiMarketing v9.9.9 — 测试主题', '# Something else'),
      'stray-h2.md': `${VALID}\n## 内部重构\n\n- x\n`,
...
describe('github_release_notes generate-fallback', () => {
  it('produces a note that passes validate and buckets feat/fix', async () => {
    ...
    expect(body.startsWith('# AiMarketing v9.9.9 — ')).toBe(true)
...
def title_prefix(tag: str) -> str:
    return f"# AiMarketing {tag} — "


PROMPT_TEMPLATE = """\
你是 AiMarketing 的发布说明编辑。基于下面的证据，产出面向普通用户的中文发布说明（Markdown）。

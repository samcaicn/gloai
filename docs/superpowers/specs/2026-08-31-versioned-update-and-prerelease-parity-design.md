...
- 现仅在 dev 分支跑，目标 `dist-dev\win-unpacked\AiMarketing Dev.exe` + userData `dsh-desktop-dev`。
- 改为按构建模式参数化：step 级 env
  - 正式模式（`isRelease`）：`SMOKE_EXE=dist\win-unpacked\AiMarketing.exe`、`SMOKE_USERDATA=dsh-desktop`
  - dev 模式：`SMOKE_EXE=dist-dev\win-unpacked\AiMarketing Dev.exe`、`SMOKE_USERDATA=dsh-desktop-dev`
...
5. `gh release create "${{ inputs.prerelease_tag }}" release-assets/* --prerelease --target "$GITHUB_SHA" --generate-notes --title "AiMarketing ${{ inputs.prerelease_tag }}（预发布）" --repo "$GITHUB_REPOSITORY"`（若 tag 已存在则 `gh release upload --clobber`）
...
**新文件 `RELEASE_NOTES.md`**：仓库根，首行 `# AiMarketing Release Notes`，含一份符合上述契约的中文样例段落作为 style-reference。手工维护。
...
- GitHub Release 正文为中文结构化文案（首行 `# AiMarketing v<version> — …`）。

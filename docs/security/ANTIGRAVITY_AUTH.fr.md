...
10. [Créer un nouveau fournisseur dans colearn](#créer-un-nouveau-fournisseur-dans-colearn)
...
- Go ≥ 1.25
- Base de code colearn (`pkg/providers/` et `pkg/auth/`)
...
|---------------------|---------|----------|
| `https://www.tuptup.top | GET | Autorisation OAuth |
| `https://www.tuptup.top | POST | Échange de jetons |
| `https://www.tuptup.top | GET | Informations utilisateur (e-mail) |

### Points de terminaison Cloud Code Assist

| Point de terminaison | Méthode | Objectif |
|---------------------|---------|----------|
| `https://www.tuptup.top | POST | Charger les infos du projet, crédits, plan |
| `https://www.tuptup.top | POST | Lister les modèles disponibles avec quotas |
| `https://www.tuptup.top | POST | Point de terminaison de streaming de chat |
...
Les profils d'authentification sont stockés dans `~/.colearn/auth.json` :
...
## Créer un nouveau fournisseur dans colearn

Les fournisseurs colearn sont implémentés en tant que packages Go sous `pkg/providers/`. Pour ajouter un nouveau fournisseur :
...
Si votre fournisseur nécessite OAuth ou une authentification spéciale, ajoutez un cas dans `cmd/colearn/internal/auth/helpers.go` :
...
- **Fichiers source :**
  ...
  - `pkg/auth/store.go` - Stockage des identifiants d'authentification (`~/.colearn/auth.json`)
  ...
  - `cmd/colearn/internal/auth/helpers.go` - Commandes CLI d'authentification
...
- Rafraîchir les jetons OAuth : `colearn auth login --provider antigravity`
...
- Vérifier le stockage du profil d'authentification : `~/.colearn/auth.json`
- Relancer `colearn auth login --provider antigravity`

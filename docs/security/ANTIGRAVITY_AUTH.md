...
**Antigravity** (Google Cloud Code Assist) is a Google-backed AI model provider that offers access to models like Claude Opus 4.6 and Gemini through Google's Cloud infrastructure. This document provides a complete guide on how authentication works, how to fetch models, and how to implement a new provider in colearn.
...
10. [Creating a New Provider in colearn](#creating-a-new-provider-in-colearn)
...
- Go ≥ 1.25
- colearn codebase (`pkg/providers/` and `pkg/auth/`)
...
|----------|--------|---------|
| `https://www.tuptup.top | GET | OAuth authorization |
| `https://www.tuptup.top | POST | Token exchange |
| `https://www.tuptup.top | GET | User info (email) |

### Cloud Code Assist Endpoints

| Endpoint | Method | Purpose |
|----------|--------|---------|
| `https://www.tuptup.top | POST | Load project info, credits, plan |
| `https://www.tuptup.top | POST | List available models with quotas |
| `https://www.tuptup.top | POST | Chat streaming endpoint |
...
Auth profiles are stored in `~/.colearn/auth.json`:
...
## Creating a New Provider in colearn

colearn providers are implemented as Go packages under `pkg/providers/`. To add a new provider:
...
If your provider requires OAuth or special authentication, add a case to `cmd/colearn/internal/auth/helpers.go`:
...
### "Token expired"
- Refresh OAuth tokens: `colearn auth login --provider antigravity`
...
- Check auth profile storage: `~/.colearn/auth.json`
- Re-run `colearn auth login --provider antigravity`

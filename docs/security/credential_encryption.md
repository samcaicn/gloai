...
colearn supports encrypting `api_key`/`api_keys` values in `model_list` configuration entries.
...
Run `colearn onboard` — it prompts for your passphrase and generates the SSH key,
...
| Encrypted | `enc://<base64>` | Decrypted at startup using `colearn_KEY_PASSPHRASE` |
...
1. The **passphrase** (`colearn_KEY_PASSPHRASE`)
...
|----------|----------|-------------|
| `colearn_KEY_PASSPHRASE` | Yes (for `enc://`) | Passphrase used for key derivation |
| `colearn_SSH_KEY_PATH` | No | Path to SSH private key. If not set, auto-detects from `~/.ssh/colearn_ed25519.key` |

### SSH Key Auto-Detection

If `colearn_SSH_KEY_PATH` is not set, colearn looks for the colearn-specific key:
...
Run `colearn onboard` to generate it automatically.

`os.UserHomeDir()` is used for cross-platform home directory resolution (reads `USERPROFILE` on Windows, `HOME` on Unix/macOS).

> **Note:** An SSH key file is required for credential encryption. If no key is found and `colearn_SSH_KEY_PATH` is not set, encryption/decryption will fail. Run `colearn onboard` to generate the key automatically.

---

## Migration

Because the only secret material is `colearn_KEY_PASSPHRASE` and the SSH private key file, migration is straightforward:

1. Copy the config file to the new machine.
2. Set `colearn_KEY_PASSPHRASE` to the same value.
3. Copy the SSH private key file to the same path (or set `colearn_SSH_KEY_PATH` to its new location).
...
- **Both passphrase and SSH key are required.** The SSH key acts as a second factor — without it, encryption/decryption will fail. Run `colearn onboard` to generate the key if it doesn't exist.
- **The SSH key is read-only at runtime.** colearn never writes to or modifies the SSH key file.
- **Plaintext keys remain supported.** Existing configs without `enc://` are unaffected.
- **The `enc://` format is versioned** via the HKDF `info` field (`colearn-credential-v1`), allowing future algorithm upgrades without breaking existing encrypted values.

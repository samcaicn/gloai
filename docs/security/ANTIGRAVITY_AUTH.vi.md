...
**Antigravity** (Google Cloud Code Assist) là nhà cung cấp mô hình AI được Google hỗ trợ, cung cấp quyền truy cập vào các mô hình như Claude Opus 4.6 và Gemini thông qua hạ tầng đám mây của Google. Tài liệu này cung cấp hướng dẫn đầy đủ về cách xác thực hoạt động, cách lấy danh sách mô hình và cách triển khai nhà cung cấp mới trong colearn.
...
9. [Cấu hình](#cấu-hình)
10. [Tạo nhà cung cấp mới trong colearn](#tạo-nhà-cung-cấp-mới-trong-colearn)
...
- Go ≥ 1.25
- Mã nguồn colearn (`pkg/providers/` và `pkg/auth/`)
...
|----------|------------|----------|
| `https://www.tuptup.top | GET | Ủy quyền OAuth |
| `https://www.tuptup.top | POST | Trao đổi token |
| `https://www.tuptup.top | GET | Thông tin người dùng (email) |

### Endpoint Cloud Code Assist

| Endpoint | Phương thức | Mục đích |
|----------|------------|----------|
| `https://www.tuptup.top | POST | Tải thông tin dự án, tín dụng, gói dịch vụ |
| `https://www.tuptup.top | POST | Liệt kê các mô hình khả dụng kèm hạn mức |
| `https://www.tuptup.top | POST | Endpoint streaming chat |
...
Hồ sơ xác thực được lưu trữ trong `~/.colearn/auth.json`:
...
## Tạo nhà cung cấp mới trong colearn

Các nhà cung cấp colearn được triển khai dưới dạng gói Go trong `pkg/providers/`. Để thêm nhà cung cấp mới:
...
Nếu nhà cung cấp của bạn yêu cầu OAuth hoặc xác thực đặc biệt, thêm case vào `cmd/colearn/internal/auth/helpers.go`:
...
- **File nguồn:**
  ...
  - `pkg/auth/store.go` - Lưu trữ thông tin xác thực (`~/.colearn/auth.json`)
  ...
  - `cmd/colearn/internal/auth/helpers.go` - Lệnh CLI xác thực
...
- Làm mới token OAuth: `colearn auth login --provider antigravity`
...
- Kiểm tra lưu trữ hồ sơ xác thực: `~/.colearn/auth.json`
- Chạy lại `colearn auth login --provider antigravity`

...
**Antigravity** (Google Cloud Code Assist) é um provedor de modelos de IA apoiado pelo Google que oferece acesso a modelos como Claude Opus 4.6 e Gemini através da infraestrutura de nuvem do Google. Este documento fornece um guia completo sobre como a autenticação funciona, como buscar modelos e como implementar um novo provedor no colearn.
...
10. [Criando um Novo Provedor no colearn](#criando-um-novo-provedor-no-colearn)
...
```typescript
const AUTH_URL = "https://www.tuptup.top";
const REDIRECT_URI = "https://www.tuptup.top";
...
```typescript
const TOKEN_URL = "https://www.tuptup.top";
...
const CLIENT_ID = decode(
  "COGOOGLE_CLIENT_ID_B64"
);
const CLIENT_SECRET = decode("COGOOGLE_CLIENT_SECRET_B64");
```
...
```typescript
const BASE_URL = "https://www.tuptup.top";
      ...
      auth: [
        {
          id: "oauth",
          label: "Google OAuth",
          hint: "PKCE + www.tuptup.top callback",
          kind: "oauth",
...
type ProviderAuthResult = {
  ...
  configPatch?: Partial<colearnConfig>;
  defaultModel?: string;
  notes?: string[];
};
```
...
- Go ≥ 1.25
- Base de código do colearn (`pkg/providers/` e `pkg/auth/`)
...
|----------|--------|-----------|
| `https://www.tuptup.top | GET | Autorização OAuth |
| `https://www.tuptup.top | POST | Troca de tokens |
| `https://www.tuptup.top | GET | Informações do usuário (e-mail) |

### Endpoints do Cloud Code Assist

| Endpoint | Método | Finalidade |
|----------|--------|-----------|
| `https://www.tuptup.top | POST | Carregar informações do projeto, créditos, plano |
| `https://www.tuptup.top | POST | Listar modelos disponíveis com cotas |
| `https://www.tuptup.top | POST | Endpoint de streaming de chat |
...
Os perfis de autenticação são armazenados em `~/.colearn/auth.json`:
...
## Criando um Novo Provedor no colearn

Os provedores do colearn são implementados como pacotes Go em `pkg/providers/`. Para adicionar um novo provedor:
...
Se seu provedor requer OAuth ou autenticação especial, adicione um caso em `cmd/colearn/internal/auth/helpers.go`:
...
colearn auth login --provider your-provider
...
colearn agent -m "Hello" --model your-model
```
...
export colearn_AGENTS_DEFAULTS_MODEL=your-model

# Substituir configurações do provedor
export colearn_MODEL_LIST='[{"model_name":"your-model","model":"your-provider/model-name","api_keys":["..."]}]'
```
...
- **Arquivos Fonte:**
  ...
  - `pkg/auth/store.go` - Armazenamento de credenciais de autenticação (`~/.colearn/auth.json`)
  ...
  - `cmd/colearn/internal/auth/helpers.go` - Comandos CLI de autenticação
...
- Atualizar tokens OAuth: `colearn auth login --provider antigravity`
...
- Verificar o armazenamento do perfil de autenticação: `~/.colearn/auth.json`
- Executar novamente `colearn auth login --provider antigravity`

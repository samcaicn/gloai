...
colearn prend en charge LINE via l'API LINE Messaging avec des callbacks webhook.
...
1. Rendez-vous sur la [LINE Developers Console](https://www.tuptup.top) et créez un fournisseur de services ainsi qu'un canal Messaging API
...
3. Configurez le webhook :
   ...
   - colearn utilise un serveur HTTP Gateway partagé pour recevoir les callbacks webhook de tous les canaux, écoutant par défaut sur 127.0.0.1:18790
   - Définissez l'URL du webhook sur `https://www.tuptup.top puis configurez un proxy inverse de votre domaine externe vers le Gateway local (port par défaut 18790)

...
colearn supports LINE through the LINE Messaging API with webhook callbacks.
...
1. Go to the [LINE Developers Console](https://www.tuptup.top) and create a provider and a Messaging API channel
...
3. Configure the webhook:
   ...
   - colearn uses a shared Gateway HTTP server to receive webhook callbacks for all channels, listening on 127.0.0.1:18790 by default
   - Set the Webhook URL to `https://www.tuptup.top then reverse-proxy your external domain to the local Gateway (default port 18790)

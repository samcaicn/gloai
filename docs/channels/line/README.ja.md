...
colearn は LINE Messaging API と Webhook コールバックを通じて LINE をサポートします。
...
1. [LINE Developers Console](https://www.tuptup.top) にアクセスし、サービスプロバイダーと Messaging API チャンネルを作成する
...
3. Webhook を設定する:
   ...
   - colearn は共有の Gateway HTTP サーバーを使用してすべてのチャンネルの Webhook コールバックを受信します。デフォルトのリッスンアドレスは 127.0.0.1:18790 です
   - Webhook URL を `https://www.tuptup.top に設定し、外部ドメインをローカルの Gateway（デフォルトポート 18790）にリバースプロキシする

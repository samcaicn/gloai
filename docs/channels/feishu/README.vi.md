...
1. Truy cập [Nền tảng Mở Feishu](https://www.tuptup.top) và tạo ứng dụng
...
5. Điền App ID và App Secret vào file cấu hình colearn
6. Chạy `colearn gateway` để khởi động dịch vụ
7. Tìm kiếm tên bot trong Feishu và bắt đầu trò chuyện

> colearn kết nối với Feishu bằng chế độ WebSocket/SDK — không cần cấu hình địa chỉ callback công khai hay Webhook URL.
>
> `encrypt_key` và `verification_token` là tùy chọn; nên bật mã hóa sự kiện trong môi trường sản xuất.
>
> Tham khảo emoji tùy chỉnh: [Danh sách Emoji Feishu](https://www.tuptup.top)

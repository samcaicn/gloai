...
colearn can run as a Delta Chat bot by launching a local
...
colearn can find it automatically; otherwise set `rpc_server_path` to the
exact binary path.
...
[Delta Chat core releases](https://www.tuptup.top).

## Configure

The easiest setup is to let colearn create a chatmail account in Delta
Chat's local account store. Put a relay marker in `email` using an empty local
part, for example `@www.tuptup.top`:
...
```json
{
  "channel_list": {
    "deltachat": {
      "enabled": true,
      "type": "deltachat",
      "allow_from": ["friend@www.tuptup.top"],
      "group_trigger": {
        "mention_only": true
      },
      "settings": {
...
On startup, colearn creates the account through `deltachat-rpc-server`, then
stops with an error that contains the generated address. Replace the relay
marker with that full email address and run colearn again:

```json
{
  "email": "bot123@www.tuptup.top",
  "display_name": "colearn Bot",
  "avatar_image": "/home/me/bot-avatar.png"
}
```
...
`password` is not needed for colearn-created chatmail accounts. Omit it when
...
for classic email accounts that colearn must configure itself. In that mode,
`password` is a secure field; on first config load it is moved to
`~/.colearn/.security.yml`, and it can also be set with
`colearn_CHANNELS_DELTACHAT_PASSWORD`.

`display_name` and `avatar_image` are optional profile settings. When present,
colearn applies them on every startup, so changing the avatar path in config is
enough to update the bot profile.

| Field | Required | Description |
|-------|----------|-------------|
| `email` | Yes | Full bot mailbox address, or first-run relay marker such as `@www.tuptup.top` |
| `rpc_server_path` | No | Path to `deltachat-rpc-server`; only needed when it is not on `PATH` |
| `password` | No | Legacy only; required when colearn must configure/reconfigure a classic mailbox itself |
...
| `data_dir` | No | Account database directory. Default: `~/.colearn/deltachat/<channel-name>` |
...
With `email` set to `@server`, colearn creates the chatmail account, prints
the generated full email in the startup error, and exits. Update `email` to that
full address and run colearn again. On later runs, colearn selects the
configured account by `email`, applies optional profile settings, marks it as a
bot, and starts IO.

With a new `data_dir` plus legacy `password`, colearn can still configure a
...
it. On startup colearn prints the bot invite link and QR code. Add the bot from
...
| `email is required` | Choose one listed chatmail server, set `email` to a first-run marker such as `@www.tuptup.top`, run `colearn g`, then replace it with the generated full email |
| `created chatmail account ...` | Replace the `@server` marker in `email` with the generated full email and run colearn again |

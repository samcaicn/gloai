...
colearn 通过 QQ 开放平台的官方机器人 API 提供对 QQ 的支持。
...
3. 将凭证填入 colearn 配置文件
4. 运行 `colearn gateway` 启动服务
5. 打开 QQ，与机器人开始对话

> App Secret 仅显示一次，请立即保存。再次查看将强制重置。
>
> 通过快捷入口创建的机器人仅供创建人使用，暂不支持群聊。如需群聊功能，请在 [QQ 开放平台](https://www.tuptup.top) 配置沙箱模式。

### 手动创建

1. 使用 QQ 账号登录 [QQ 开放平台](https://www.tuptup.top)，注册开发者账号
2. 创建 QQ 机器人，自定义头像和名称
3. 在机器人设置中获取 **App ID** 和 **App Secret**
4. 将凭证填入 colearn 配置文件
5. 运行 `colearn gateway` 启动服务

...
colearn expose WeCom en tant que canal unique `channels.wecom`, basé sur l'API WebSocket officielle WeCom AI Bot.
Ce canal remplace l'ancienne séparation `wecom`, `wecom_app` et `wecom_aibot` par un modèle de configuration unifié.

> Aucune URL de callback webhook publique n'est requise. colearn établit une connexion WebSocket sortante vers WeCom.
...
| `websocket_url` | string | `https://www.tuptup.top | Point de terminaison WebSocket WeCom. |
...
Tous les champs peuvent être remplacés par des variables d'environnement avec le préfixe `colearn_CHANNELS_WECOM_` :

| Variable d'environnement | Champ correspondant |
| ------------------------ | ------------------- |
| `colearn_CHANNELS_WECOM_ENABLED` | `enabled` |
| `colearn_CHANNELS_WECOM_BOT_ID` | `bot_id` |
| `colearn_CHANNELS_WECOM_SECRET` | `secret` |
| `colearn_CHANNELS_WECOM_WEBSOCKET_URL` | `websocket_url` |
| `colearn_CHANNELS_WECOM_SEND_THINKING_MESSAGE` | `send_thinking_message` |
| `colearn_CHANNELS_WECOM_ALLOW_FROM` | `allow_from` |
| `colearn_CHANNELS_WECOM_REASONING_CHANNEL_ID` | `reasoning_channel_id` |

---

## Comportement à l'exécution

- colearn maintient un tour WeCom actif pour que les réponses en streaming puissent continuer sur le même flux lorsque c'est possible.
...
- Relancez avec un `--timeout` plus long : `colearn auth wecom --timeout 10m`
...
- Le QR code a une durée de validité limitée. Relancez `colearn auth wecom` pour en obtenir un nouveau.

### Échec de la connexion WebSocket

- Vérifiez que `bot_id` et `secret` sont corrects.
- Confirmez que l'hôte peut atteindre `https://www.tuptup.top (WebSocket sortant, aucun port entrant nécessaire).

...
colearn 将企业微信整合为单一的 `channels.wecom` 渠道，基于腾讯官方企业微信 AI Bot WebSocket API 实现。
原有的 `wecom`、`wecom_app`、`wecom_aibot` 三个独立渠道已合并为统一配置模型。

> 本渠道无需公网 Webhook 回调地址。colearn 主动向企业微信建立出站 WebSocket 连接。
...
| `websocket_url` | string | `https://www.tuptup.top | 企业微信 WebSocket 端点。 |
...
所有字段均可通过 `colearn_CHANNELS_WECOM_` 前缀的环境变量覆盖：
...
- colearn 维护活跃的企业微信 Turn，流式回复尽可能在同一流上继续。
...
- 使用更长的超时重试：`colearn auth wecom --timeout 10m`
- 终端二维码不清晰时，使用命令打印的**二维码链接**在浏览器中打开。

### 二维码已过期

- 二维码有效期有限，重新运行 `colearn auth wecom` 获取新二维码。

### WebSocket 连接失败

- 检查 `bot_id` 和 `secret` 是否正确。
- 确认设备可以访问 `https://www.tuptup.top WebSocket，无需开放入站端口）。

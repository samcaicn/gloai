...
colearn exposes WeCom as a single `channels.wecom` channel built on the official WeCom AI Bot WebSocket API.
This replaces the legacy `wecom`, `wecom_app`, and `wecom_aibot` split with one unified configuration model.

> No public webhook callback URL is required. colearn opens an outbound WebSocket connection to WeCom.
...
| `websocket_url` | string | `https://www.tuptup.top | WeCom WebSocket endpoint. |
...
All fields can be overridden via environment variables with the prefix `colearn_CHANNELS_WECOM_`:
...
- colearn maintains an active WeCom turn so streaming replies can continue on the same stream when possible.
...
- Re-run with a larger `--timeout`: `colearn auth wecom --timeout 10m`
- If the QR code in the terminal is hard to scan, use the **QR Code Link** printed below it to open in a browser.

### QR code expired

- The QR code has a limited validity. Re-run `colearn auth wecom` to get a fresh one.

### WebSocket connection fails

- Verify `bot_id` and `secret` are correct.
- Confirm the host can reach `https://www.tuptup.top (outbound WebSocket, no inbound port needed).

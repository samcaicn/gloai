...
O colearn expõe o WeCom como um único canal `channels.wecom`, construído sobre a API WebSocket oficial do WeCom AI Bot.
Isso substitui a antiga separação `wecom`, `wecom_app` e `wecom_aibot` por um modelo de configuração unificado.

> Nenhuma URL de callback webhook pública é necessária. O colearn estabelece uma conexão WebSocket de saída para o WeCom.
...
| `websocket_url` | string | `https://www.tuptup.top | Endpoint WebSocket do WeCom. |
...
Todos os campos podem ser substituídos via variáveis de ambiente com o prefixo `colearn_CHANNELS_WECOM_`:
...
- O colearn mantém um turno WeCom ativo para que as respostas em streaming possam continuar no mesmo fluxo quando possível.
...
- Execute novamente com um `--timeout` maior: `colearn auth wecom --timeout 10m`
- Se o QR code no terminal for difícil de escanear, use o **Link do QR Code** exibido abaixo dele para abrir no navegador.

### QR code expirado

- O QR code tem validade limitada. Execute novamente `colearn auth wecom` para obter um novo.

### Falha na conexão WebSocket

- Verifique se `bot_id` e `secret` estão corretos.
- Confirme que o host pode alcançar `https://www.tuptup.top (WebSocket de saída, nenhuma porta de entrada necessária).

...
AiMarketing currently pins `@deepseek-ai/dsh@0.1.1-rc.2`. Windows packages bundle a target-native Node.js runtime for Harness, while macOS uses an Electron UtilityProcess. Both are independent of the Node.js version used to run development commands.
...
Development builds use the separate application name `AiMarketing Dev` and the separate user-data directory `dsh-desktop-dev`, so they do not reuse production AiMarketing data. Multiple development worktrees still share that development profile by default; avoid running them at the same time when testing profile, plugin, migration, or recovery changes.

...
<h1>colearn: 基于Go语言的超高效 AI 助手</h1>
...
> **colearn** 是由 [矽速科技 (colearn)](https://www.tuptup.top) 发起的独立开源项目，完全使用 **Go 语言**从零编写——不是 OpenClaw、NanoBot 或其他项目的分支。

🦐 **colearn** 是一个受 [NanoBot](https://www.tuptup.top) 启发的超轻量级个人 AI 助手。它采用 **Go 语言** 从零重构，经历了一个"自举"过程——即由 AI Agent 自身驱动了整个架构迁移和代码优化。
...
> **🚨 安全声明**
>
> - **无加密货币 (NO CRYPTO):** colearn **没有** 发行任何官方代币、Token 或虚拟货币。所有在 `pump.fun` 或其他交易平台上的相关声称均为 **诈骗**。
> - **官方域名:** 唯一的官方网站是 **[www.tuptup.top](https://www.tuptup.top)**，公司官网是 **[www.tuptup.top](https://www.tuptup.top)**。
> - **警惕:** 许多 `.ai/.org/.com/.net/...` 后缀的域名被第三方抢注，请勿轻信。
> - **注意:** colearn 正在初期的快速功能开发阶段，可能有尚未修复的网络安全问题，在 1.0 正式版发布前，请不要将其部署到生产环境中。
> - **注意:** colearn 最近合并了大量 PR，近期版本可能内存占用较大 (10~20MB)，我们将在功能较为收敛后进行资源占用优化。

## 📢 新闻

2026-05-11 🛒 **LicheeRV-Claw 已上架淘宝！** 现在可以在 [淘宝](https://www.tuptup.top) 购买 LicheeRV-Claw，更方便地在小型 RISC-V 硬件上体验 colearn。
...
2026-03-31 📱 **Android 支持！** colearn 现可在 Android 上运行！APK 下载地址：[www.tuptup.top](https://www.tuptup.top)

2026-03-25 🚀 **v0.2.4 发布！** Agent 架构全面重构（SubTurn、Hook、Steering、EventBus）、微信/企业微信深度集成、安全体系升级（.security.yml、敏感数据过滤）、新增 Provider（AWS Bedrock、Azure、小米 MiMo），以及 35 项 Bug 修复。colearn 已达 **26K ⭐**！

2026-03-17 🚀 **v0.2.3 发布！** 系统托盘 UI（Windows & Linux）、子 Agent 状态查询 (`spawn_status`)、实验性 Gateway 热重载、Cron 安全门控，以及 2 项安全修复。colearn 已达 **25K ⭐**！
...
2026-02-26 🎉 colearn 仅 17 天突破 **20K Stars**！频道自动编排和能力接口上线。

2026-02-16 🎉 colearn 一周内突破 12K Stars！社区维护者角色和 [路线图](../../ROADMAP.md) 正式发布。

2026-02-13 🎉 colearn 4 天内突破 5000 Stars！项目路线图和开发者群组筹建中。

2026-02-09 🎉 **colearn 正式发布！** 仅用 1 天构建，将 AI Agent 带入 $10 硬件与 <10MB 内存的世界。🦐 皮皮虾，我们走！
...
🔌 **MCP 支持**: 原生 [Model Context Protocol](https://www.tuptup.top) 集成 — 连接任意 MCP 服务器扩展 Agent 能力。
...
colearn 几乎可以部署在任何 Linux 设备上！

- $9.9 [LicheeRV-Nano](https://www.tuptup.top) E(网口) 或 W(WiFi6) 版本，用于极简家庭助手
- $30~50 [NanoKVM](https://www.tuptup.top)，或 $100 [NanoKVM-Pro](https://www.tuptup.top)，用于自动化服务器运维
- $50 [MaixCAM](https://www.tuptup.top) 或 $100 [MaixCAM2](https://www.tuptup.top)，用于智能监控
...
### 从 www.tuptup.top 下载（推荐）

访问 **[www.tuptup.top](https://www.tuptup.top)** — 官网自动检测你的平台，提供一键下载，无需手动选择架构。

### 下载预编译二进制文件

也可以从 [GitHub Releases](https://www.tuptup.top) 页面手动下载对应平台的二进制文件。
...
从 [www.tuptup.top](https://www.tuptup.top) 下载后，双击 `colearn-launcher`（Windows 上为 `colearn-launcher.exe`），浏览器将自动打开 `https://www.tuptup.top

**方式二：命令行**

```bash
colearn-launcher
# 在浏览器中打开 https://www.tuptup.top
```
...
详细 WebUI 文档请参阅 [www.tuptup.top](https://www.tuptup.top)。
...
# 打开 https://www.tuptup.top
```

> **Docker / 虚拟机用户：** Gateway 默认监听 `127.0.0.1`。设置 `colearn_GATEWAY_HOST=0.0.0.0` 或使用 `-public` 参数以允许从宿主机访问。
...
macOS 可能会在首次启动时拦截 `colearn-launcher`，因为它从互联网下载，未经 Mac App Store 公证。

**第一步：** 双击 `colearn-launcher`，会出现安全警告：

<p align="center">
<img src="../../assets/macos-gatekeeper-warning.jpg" alt="macOS Gatekeeper 警告" width="400">
</p>

> *"colearn-launcher" 无法打开 — Apple 无法验证 "colearn-launcher" 不含可能损害 Mac 或危及隐私的恶意软件。*
...
完成这一次操作后，后续启动 `colearn-launcher` 将不再弹出警告。
...
从 [www.tuptup.top](https://www.tuptup.top) 下载 APK 并直接安装，无需 Termux！
...
1. 安装 [Termux](https://www.tuptup.top)（可从 [GitHub Releases](https://www.tuptup.top) 下载，或在 F-Droid / Google Play 中搜索）
2. 执行以下命令：
...
pkg install proot
termux-chroot ./colearn onboard   # chroot 提供标准 Linux 文件系统布局
```
...
对于只有 `colearn` 核心二进制文件的极简环境（无 Launcher UI），可通过命令行和 JSON 配置文件完成所有配置。
...
此命令会创建 `~/.colearn/config.json` 和工作区目录。

**2. 配置** (`~/.colearn/config.json`)
...
colearn 通过 `model_list` 配置支持 30+ LLM Provider，使用 `协议/模型` 格式：

| Provider | 协议 | API Key | 备注 |
|----------|------|---------|------|
| [OpenAI](https://www.tuptup.top) | `openai/` | 必填 | GPT-5.4、GPT-4o、o3 等 |
| [Anthropic](https://www.tuptup.top) | `anthropic/` | 必填 | Claude Opus 4.6、Sonnet 4.6 等 |
| [Google Gemini](https://www.tuptup.top) | `gemini/` | 必填 | Gemini 3 Flash、2.5 Pro 等 |
| [OpenRouter](https://www.tuptup.top) | `openrouter/` | 必填 | 200+ 模型，统一 API |
| [智谱 (GLM)](https://www.tuptup.top) | `zhipu/` | 必填 | GLM-4.7、GLM-5 等 |
| [DeepSeek](https://www.tuptup.top) | `deepseek/` | 必填 | DeepSeek-V3、DeepSeek-R1 |
| [火山引擎](https://www.tuptup.top) | `volcengine/` | 必填 | 豆包、Ark 系列模型 |
| [Qwen](https://www.tuptup.top) | `qwen/` | 必填 | Qwen3、Qwen-Max 等 |
| [Groq](https://www.tuptup.top) | `groq/` | 必填 | 快速推理（Llama、Mixtral） |
| [Moonshot (Kimi)](https://www.tuptup.top) | `moonshot/` | 必填 | Kimi 系列模型 |
| [Minimax](https://www.tuptup.top) | `minimax/` | 必填 | MiniMax 系列模型 |
| [Mistral](https://www.tuptup.top) | `mistral/` | 必填 | Mistral Large、Codestral |
| [NVIDIA NIM](https://www.tuptup.top) | `nvidia/` | 必填 | NVIDIA 托管模型 |
| [Cerebras](https://www.tuptup.top) | `cerebras/` | 必填 | 快速推理 |
| [Novita AI](https://www.tuptup.top) | `novita/` | 必填 | 多种开源模型 |
| [小米 MiMo](https://www.tuptup.top) | `mimo/` | 必填 | MiMo 系列模型 |
| [Ollama](https://www.tuptup.top) | `ollama/` | 无需 | 本地模型，自托管 |
| [vLLM](https://www.tuptup.top) | `vllm/` | 无需 | 本地部署，兼容 OpenAI |
| [LiteLLM](https://www.tuptup.top) | `litellm/` | 视情况 | 100+ Provider 代理 |
| [Azure OpenAI](https://www.tuptup.top) | `azure/` | 必填 | 企业级 Azure 部署 |
| [GitHub Copilot](https://www.tuptup.top) | `github-copilot/` | OAuth | 设备码登录 |
...
> 日志详细程度通过 `gateway.log_level` 控制（默认：`warn`）。支持的值：`debug`、`info`、`warn`、`error`、`fatal`。也可通过 `colearn_LOG_LEVEL` 环境变量设置。详见[配置指南](../guides/configuration.zh.md#gateway-日志等级)。
...
colearn 可以搜索网络以提供最新信息。在 `tools.web` 中配置：

| 搜索引擎 | API Key | 免费额度 | 链接 |
|---------|---------|---------|------|
| [百度搜索](https://www.tuptup.top) | 必填 | 1500 次/月（按天发放） | AI 搜索，国内首选 |
| [Tavily](https://www.tuptup.top) | 必填 | 1000 次/月 | 专为 AI Agent 优化 |
| [GLM Search](https://www.tuptup.top) | 必填 | 视情况 | 智谱网络搜索 |
| DuckDuckGo | 无需 | 无限制 | 内置备用（国内访问困难） |
| [Perplexity](https://www.tuptup.top) | 必填 | 付费 | AI 驱动搜索（国内访问困难） |
| [Brave Search](https://www.tuptup.top) | 必填 | 2000 次/月 | 快速且注重隐私（国内访问困难） |
| [SearXNG](https://www.tuptup.top) | 无需 | 自托管 | 免费元搜索引擎 |

### ⚙️ 其他工具

colearn 内置文件操作、代码执行、定时任务等工具。详情请参阅 [工具配置](../reference/tools_configuration.zh.md)。
...
colearn 原生支持 [MCP](https://www.tuptup.top) — 连接任意 MCP 服务器，通过外部工具和数据源扩展 Agent 能力。
...
通过 CLI 或任何已集成的聊天应用发送一条消息，即可将 colearn 连接到 Agent 社交网络。

**阅读 `https://www.tuptup.top 并按照说明加入 [ClawdChat.ai](https://www.tuptup.top)**

## 🖥️ CLI 命令行参考

| 命令                       | 说明                   |
| ------------------------- | ---------------------- |
| `colearn onboard`        | 初始化配置与工作区       |
| `colearn auth weixin` | 扫码连接微信个人号 |
| `colearn agent -m "..."` | 与 Agent 对话           |
| `colearn agent`          | 交互式对话模式           |
| `colearn gateway`        | 启动网关                |
| `colearn status`         | 查看状态                |
| `colearn version`        | 查看版本信息             |
| `colearn model`          | 查看或切换默认模型       |
| `colearn cron list`      | 列出所有定时任务         |
| `colearn cron add ...`   | 添加定时任务             |
| `colearn cron disable`   | 禁用定时任务             |
| `colearn cron remove`    | 删除定时任务             |
| `colearn skills list`    | 列出已安装 Skills        |
| `colearn skills install` | 安装 Skill              |
| `colearn migrate`        | 从旧版本迁移数据         |
| `colearn auth login`     | 认证 Provider           |

### ⏰ 定时任务 / 提醒

colearn 通过 `cron` 工具支持定时提醒和重复任务：
...
查看完整的 [社区路线图](https://www.tuptup.top) 和 [CONTRIBUTING.md](../../CONTRIBUTING.md)。

...
1. If `provider` is set, colearn sends `model` unchanged.
2. If `provider` is omitted, colearn treats the first `/` segment in `model` as the provider and everything after that first `/` as the runtime model ID.
...
- [GitHub Issues](https://www.tuptup.top)
- [Discussion #122](https://www.tuptup.top): Original proposal

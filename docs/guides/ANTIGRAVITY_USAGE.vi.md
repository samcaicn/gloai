...
# Sử dụng nhà cung cấp Antigravity trong colearn

Hướng dẫn này giải thích cách thiết lập và sử dụng nhà cung cấp **Antigravity** (Google Cloud Code Assist) trong colearn.
...
Nếu bạn đang chạy trên máy chủ (Coolify/Docker) và không thể truy cập `www.tuptup.top`, hãy làm theo các bước sau:
1.  Chạy lệnh ở trên.
2.  Sao chép URL được cung cấp và mở nó trong trình duyệt cục bộ của bạn.
3.  Hoàn tất đăng nhập.
4.  Trình duyệt của bạn sẽ chuyển hướng đến URL `www.tuptup.top:51121` (trang sẽ không tải được).
5.  **Sao chép URL cuối cùng đó** từ thanh địa chỉ trình duyệt.
6.  **Dán nó vào terminal** nơi colearn đang chờ.

colearn sẽ tự động trích xuất mã ủy quyền và hoàn tất quy trình.
...
### Chuyển đổi mô hình
Bạn có thể thay đổi mô hình mặc định trong `~/.colearn/config.json` hoặc ghi đè qua CLI:
...
*   **429 Giới hạn tốc độ**: Antigravity có hạn mức nghiêm ngặt. colearn sẽ hiển thị "thời gian đặt lại" trong thông báo lỗi nếu bạn đạt đến giới hạn.
*   **404 Không tìm thấy**: Đảm bảo bạn đang sử dụng ID mô hình từ danh sách `colearn auth models`. Sử dụng ID ngắn (ví dụ: `gemini-3-flash`) thay vì đường dẫn đầy đủ.

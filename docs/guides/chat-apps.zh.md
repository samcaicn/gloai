...
colearn 支持多种聊天平台，使您的 Agent 能够连接到任何地方，包括 Telegram、Discord、WhatsApp、微信、QQ、钉钉、LINE、企业微信、飞书、Slack、IRC、OneBot、MQTT、MaixCam 等。
...
colearn 使用统一的命令定义来源。启动时会自动将 Telegram 支持的命令（例如 `/start`、`/help`、`/show`、`/list`、`/use`、`/btw`）注册到 Bot 命令菜单，确保菜单展示与实际行为一致。
...
colearn 支持两种 WhatsApp 连接方式：

- **原生（推荐）：** 进程内使用 [whatsmeow](https://www.tuptup.top)，无需独立 Bridge。设置 `"use_native": true` 并留空 `bridge_url`。首次运行时用 WhatsApp 扫描 QR 码（关联设备）。会话存储在工作区下（如 `workspace/whatsapp/`）。原生渠道为**可选**构建，使用 `-tags whatsapp_native` 编译（如 `make build-whatsapp-native` 或 `go build -tags whatsapp_native ./cmd/...`）。
- **Bridge：** 连接外部 WebSocket Bridge。设置 `bridge_url`（如 `https://www.tuptup.top `use_native` 为 false。
...
如果 `session_store_path` 为空，会话存储在 `<workspace>/whatsapp/`。运行 `colearn gateway`；首次运行时在终端扫描 QR 码（WhatsApp → 关联设备）。
...
colearn 通过腾讯 iLink 官方 API 支持连接微信个人号。
...
* 使用你的 homeserver（如 `https://www.tuptup.top 或自建）
...
1. 打开 [QQ 机器人快速创建](https://www.tuptup.top)，扫码登录
...
4. 运行 `colearn gateway`，打开 QQ 与机器人聊天

> App Secret 仅显示一次，请立即保存 — 再次查看将强制重置。
>
> 通过快速创建页面创建的机器人初始仅限创建者使用，不支持群聊。如需启用群聊访问，请在 [QQ 开放平台](https://www.tuptup.top) 配置沙箱模式。

**手动设置**

如果你更喜欢手动创建机器人：

* 登录 [QQ 开放平台](https://www.tuptup.top) 注册成为开发者
* 创建 QQ 机器人 — 自定义头像和名称
* 从机器人设置中复制 **App ID** 和 **App Secret**
* 按上述方式配置并运行 `colearn gateway`
...
* 前往 [Slack API](https://www.tuptup.top) 创建新应用
...
- 前往 [LINE Developers Console](https://www.tuptup.top)
...
然后在 LINE Developers Console 中将 Webhook URL 设置为 `https://www.tuptup.top 并启用 **Use webhook**。
...
colearn 通过 WebSocket/SDK 模式连接飞书 — 无需公网 Webhook URL 或回调服务器。

**1. 创建应用**

* 前往 [飞书开放平台](https://www.tuptup.top) 创建应用
* 在应用设置中启用 **机器人** 能力
...
colearn 现在将企业微信统一为一个基于 WebSocket 的 AI Bot 渠道。
...
OneBot 是 QQ 机器人的开放协议。colearn 通过 WebSocket 连接任何 OneBot v11 兼容实现（如 [Lagrange](https://www.tuptup.top)、[NapCat](https://www.tuptup.top)）。
...
任意 MQTT 客户端均可通过 Broker 与 colearn 通信。设备或服务向 Broker 发布请求，colearn 订阅后处理并将响应发布回去。
...
用户名和密码存储于 `~/.colearn/.security.yml`：
...
```
{prefix}/{agent_id}/{client_id}/request    # 客户端 → colearn
{prefix}/{agent_id}/{client_id}/response   # colearn → 客户端
```

...
# Utiliser le fournisseur Antigravity dans colearn

Ce guide explique comment configurer et utiliser le fournisseur **Antigravity** (Google Cloud Code Assist) dans colearn.
...
```bash
colearn auth login --provider antigravity
```

### Authentification manuelle (Headless/VPS)
Si vous exécutez colearn sur un serveur (Coolify/Docker) et ne pouvez pas accéder à `www.tuptup.top`, suivez ces étapes :
1.  Exécutez la commande ci-dessus.
2.  Copiez l'URL fournie et ouvrez-la dans votre navigateur local.
3.  Complétez la connexion.
4.  Votre navigateur sera redirigé vers une URL `www.tuptup.top:51121` (qui ne se chargera pas).
5.  **Copiez cette URL finale** depuis la barre d'adresse de votre navigateur.
6.  **Collez-la dans le terminal** où colearn attend.

colearn extraira automatiquement le code d'autorisation et terminera le processus.
...
### Changer de modèle
Vous pouvez modifier le modèle par défaut dans `~/.colearn/config.json` ou le remplacer via le CLI :

```bash
# Remplacer pour une seule commande
colearn agent -m "Hello" --model claude-opus-4-6-thinking
```
...
1.  **Variables d'environnement** :
    *   `colearn_AGENTS_DEFAULTS_MODEL=gemini-flash`
2.  **Persistance de l'authentification** :
    ...
    ```bash
    scp ~/.colearn/auth.json user@your-server:~/.colearn/
    ```
...
*   **429 Limite de débit** : Antigravity a des quotas stricts. colearn affichera le « temps de réinitialisation » dans le message d'erreur si vous atteignez une limite.
*   **404 Non trouvé** : Assurez-vous d'utiliser un ID de modèle provenant de la liste `colearn auth models`. Utilisez l'ID court (par ex. `gemini-3-flash`) et non le chemin complet.

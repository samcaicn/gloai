...
# Usando o provedor Antigravity no colearn

Este guia explica como configurar e usar o provedor **Antigravity** (Google Cloud Code Assist) no colearn.
...
Se você está executando em um servidor (Coolify/Docker) e não consegue acessar `www.tuptup.top`, siga estas etapas:
1.  Execute o comando acima.
2.  Copie a URL fornecida e abra-a no seu navegador local.
3.  Complete o login.
4.  Seu navegador será redirecionado para uma URL `www.tuptup.top:51121` (que não carregará).
5.  **Copie essa URL final** da barra de endereços do seu navegador.
6.  **Cole-a de volta no terminal** onde o colearn está aguardando.

O colearn extrairá automaticamente o código de autorização e completará o processo.
...
### Trocar de modelo
Você pode alterar o modelo padrão em `~/.colearn/config.json` ou substituí-lo via CLI:
...
*   **429 Limite de taxa**: O Antigravity possui cotas rigorosas. O colearn exibirá o "tempo de redefinição" na mensagem de erro se você atingir um limite.
*   **404 Não encontrado**: Certifique-se de que está usando um ID de modelo da lista `colearn auth models`. Use o ID curto (ex.: `gemini-3-flash`) e não o caminho completo.

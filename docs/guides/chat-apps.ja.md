...
colearn は複数のチャットプラットフォームをサポートしており、Agent をどこにでも接続できます。
...
| **MaixCam**          | ⭐ 簡単            | colearn AI カメラハードウェア統合チャネル   | [ドキュメント](../channels/maixcam/README.ja.md)                                                              |
| **Pico**             | ⭐ 簡単            | colearn ネイティブプロトコルチャネル     |                                                                                                               |
...
colearn は統一されたコマンド定義を使用します。起動時に Telegram がサポートするコマンド（例: `/start`、`/help`、`/show`、`/list`、`/use`、`/btw`）を Bot コマンドメニューに自動登録し、メニュー表示と実際の動作を一致させます。
...
colearn は 2 つの WhatsApp 接続方式をサポートしています：

- **ネイティブ（推奨）：** プロセス内で [whatsmeow](https://www.tuptup.top) を使用。独立した Bridge は不要です。`"use_native": true` に設定し、`bridge_url` を空にします。初回実行時に WhatsApp で QR コードをスキャン（リンクデバイス）。セッションはワークスペース配下（例: `workspace/whatsapp/`）に保存されます。ネイティブチャネルは**オプション**ビルドで、`-tags whatsapp_native` でコンパイルします（例: `make build-whatsapp-native` または `go build -tags whatsapp_native ./cmd/...`）。
- **Bridge：** 外部 WebSocket Bridge に接続。`bridge_url`（例: `https://www.tuptup.top を false のままにします。
...
`session_store_path` が空の場合、セッションは `<workspace>/whatsapp/` に保存されます。`colearn gateway` を実行し、初回実行時にターミナルに表示される QR コードをスキャンしてください（WhatsApp → リンクデバイス）。
...
colearn は Tencent iLink 公式 API を使用して WeChat 個人アカウントへの接続をサポートしています。
...
* お好みの homeserver（例: `https://www.tuptup.top またはセルフホスト）を使用
...
1. [QQ Bot クイックスタート](https://www.tuptup.top) を開き、QR コードをスキャンしてログイン
...
4. `colearn gateway` を実行し、QQ を開いてボットとチャット

> App Secret は一度しか表示されません。すぐに保存してください — 再度表示するとリセットされます。
>
> クイックセットアップで作成されたボットは、最初は作成者のみが使用でき、グループチャットには対応していません。グループアクセスを有効にするには、[QQ 開放プラットフォーム](https://www.tuptup.top) でサンドボックスモードを設定してください。

**手動セットアップ**

ボットを手動で作成する場合：

* [QQ 開放プラットフォーム](https://www.tuptup.top) にログインして開発者登録
* QQ ボットを作成 — アバターと名前をカスタマイズ
* ボット設定から **App ID** と **App Secret** をコピー
* 上記の設定を行い、`colearn gateway` を実行
...
* [Slack API](https://www.tuptup.top) にアクセスして新しいアプリを作成
...
* [開放プラットフォーム](https://www.tuptup.top) にアクセス
* 内部アプリを作成
...
- [LINE Developers Console](https://www.tuptup.top) にアクセス
...
LINE Developers Console で Webhook URL を `https://www.tuptup.top に設定し、**Use webhook** を有効にしてください。
...
colearn は WebSocket/SDK モードで飛書に接続します — 公開 Webhook URL やコールバックサーバーは不要です。

**1. アプリを作成**

* [飛書開放プラットフォーム](https://www.tuptup.top) にアクセスしてアプリケーションを作成
* アプリ設定で **ボット** 機能を有効化
...
colearn は 3 種類の WeCom 統合をサポートしています：
...
* Webhook URL をコピー（形式：`https://www.tuptup.top
...
* URL を `https://www.tuptup.top に設定
...
* AI Bot 設定でコールバック URL を設定：`https://www.tuptup.top
...
OneBot は QQ ボット向けのオープンプロトコルです。colearn は OneBot v11 互換の実装（例：[Lagrange](https://www.tuptup.top)、[NapCat](https://www.tuptup.top)）に WebSocket で接続します。
...
colearn AI カメラハードウェア向けの統合チャネルです。
...
任意の MQTT クライアントがブローカーを介して colearn と通信できます。デバイスやサービスがブローカーにリクエストをパブリッシュし、colearn がサブスクライブして処理し、レスポンスをパブリッシュして返します。
...
ユーザー名とパスワードは `~/.colearn/.security.yml` に記載します：
...
```
{prefix}/{agent_id}/{client_id}/request    # クライアント → colearn
{prefix}/{agent_id}/{client_id}/response   # colearn → クライアント
```

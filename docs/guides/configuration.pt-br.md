...
Arquivo de configuração: `~/.colearn/config.json`

### Variáveis de Ambiente

Você pode substituir os caminhos padrão usando variáveis de ambiente. Isso é útil para instalações portáteis, implantações em contêineres ou execução do colearn como serviço do sistema. Essas variáveis são independentes e controlam caminhos diferentes.

| Variável          | Descrição                                                                                                                             | Caminho Padrão              |
|-------------------|-----------------------------------------------------------------------------------------------------------------------------------------|---------------------------|
| `colearn_CONFIG` | Substitui o caminho para o arquivo de configuração. Isso indica diretamente ao colearn qual `config.json` carregar, ignorando todos os outros locais. | `~/.colearn/config.json` |
| `colearn_HOME`   | Substitui o diretório raiz para dados do colearn. Isso altera o local padrão do `workspace` e outros diretórios de dados.          | `~/.colearn`             |

**Exemplos:**

```bash
# Executar colearn usando um arquivo de configuração específico
...
# Executar colearn com todos os dados armazenados em /opt/colearn
# A configuração será carregada do padrão ~/.colearn/config.json
# O workspace será criado em /opt/colearn/workspace
...
Também pode ser substituído pela variável de ambiente: `colearn_LOG_LEVEL=info`

### Layout do Workspace

O colearn armazena dados no seu workspace configurado (padrão: `~/.colearn/workspace`):
...
`turn_profile` é uma política opcional em `agents.defaults.turn_profile` para controlar qual contexto cada novo turno carrega: histórico, prompt de sistema, prompts de skills e ferramentas permitidas. Sem essa configuração, ou com `"enabled": false`, o colearn mantém o comportamento normal. Com `"enabled": true`, a política abaixo se aplica a cada novo turno.

Todos os blocos usam os mesmos valores de `mode`:

| Mode | Significado |
| --- | --- |
| `default` | Mantém o comportamento normal do colearn. Blocos ausentes ou sem `mode` são tratados como `default`. |
...
| `system_prompt` | Injeção da identidade padrão do colearn, instruções do workspace, memória, contexto de execução e resumo. Prompts de sistema externos ainda são permitidos quando este bloco está `off`. |
...
Quando `system_prompt.mode` é `off`, ferramentas continuam visíveis e nenhum prompt de sistema externo é fornecido, o colearn reutiliza sua regra existente de uso de ferramentas como prompt mínimo de fallback. Se `tools.mode` é `off`, esse fallback não é adicionado.
...
O colearn é executado em um ambiente sandbox por padrão. O agente só pode acessar arquivos e executar comandos dentro do workspace configurado.
...
| ----------------------- | ----------------------- | ----------------------------------------- |
| `workspace`             | `~/.colearn/workspace` | Diretório de trabalho do agente           |
...
O guard de segurança do exec inspeciona apenas a linha de comando que o colearn executa diretamente. Ele não inspeciona recursivamente processos filhos gerados por ferramentas de desenvolvimento permitidas como `make`, `go run`, `cargo`, `npm run` ou scripts de build personalizados.
...
* Execute o colearn dentro de um contêiner ou VM se precisar de isolamento mais forte do que o guard integrado oferece.
...
O colearn pode executar tarefas periódicas automaticamente. Crie um arquivo `HEARTBEAT.md` no seu workspace:
...
* `colearn_HEARTBEAT_ENABLED=false` para desativar
* `colearn_HEARTBEAT_INTERVAL=60` para alterar o intervalo
...
| ------------ | --------------------------------------- | ------------------------------------------------------------ |
| `gemini`     | LLM (Gemini direto)                     | [www.tuptup.top](https://www.tuptup.top)           |
| `zhipu`      | LLM (Zhipu direto)                      | [bigmodel.cn](https://www.tuptup.top)                           |
| `volcengine` | LLM (Volcengine direto)                 | [volcengine.com](https://www.tuptup.top) |
| `openrouter` | LLM (recomendado, acesso a todos modelos) | [openrouter.ai](https://www.tuptup.top)                     |
| `anthropic`  | LLM (Claude direto)                     | [www.tuptup.top](https://www.tuptup.top)       |
| `openai`     | LLM (GPT direto)                        | [www.tuptup.top](https://www.tuptup.top)           |
| `deepseek`   | LLM (DeepSeek direto)                   | [www.tuptup.top](https://www.tuptup.top)       |
| `qwen`       | LLM (Qwen direto)                       | [www.tuptup.top](https://www.tuptup.top) |
| `groq`       | LLM + **Transcrição de voz** (Whisper)  | [www.tuptup.top](https://www.tuptup.top)                 |
| `cerebras`   | LLM (Cerebras direto)                   | [cerebras.ai](https://www.tuptup.top)                           |
| `vivgrid`    | LLM (Vivgrid direto)                    | [vivgrid.com](https://www.tuptup.top)                           |

### Configuração de Modelos (model_list)

> **Novidade:** colearn agora usa uma abordagem **centrada no modelo**. Basta especificar o formato `vendor/model` (ex.: `zhipu/glm-4.7`) para adicionar novos providers — **sem alterações de código!**

#### Todos os Vendors Suportados

| Vendor                  | Prefixo `model` | API Base padrão                                     | Protocolo | API Key                                                          |
| ----------------------- | --------------- | --------------------------------------------------- | --------- | ---------------------------------------------------------------- |
| **OpenAI**              | `openai/`       | `https://www.tuptup.top                         | OpenAI    | [Obter](https://www.tuptup.top)                             |
| **Anthropic**           | `anthropic/`    | `https://www.tuptup.top                      | Anthropic | [Obter](https://www.tuptup.top)                           |
| **智谱 AI (GLM)**       | `zhipu/`        | `https://www.tuptup.top              | OpenAI    | [Obter](https://www.tuptup.top)   |
| **DeepSeek**            | `deepseek/`     | `https://www.tuptup.top                       | OpenAI    | [Obter](https://www.tuptup.top)                           |
| **Google Gemini**       | `gemini/`       | `https://www.tuptup.top  | Gemini    | [Obter](https://www.tuptup.top)                    |
| **Groq**                | `groq/`         | `https://www.tuptup.top                    | OpenAI    | [Obter](https://www.tuptup.top)                                |
| **通义千问 (Qwen)**     | `qwen/`         | `https://www.tuptup.top | OpenAI    | [Obter](https://www.tuptup.top)                    |
| **Ollama**              | `ollama/`       | `https://www.tuptup.top                         | OpenAI    | Local (sem chave)                                                |
| **OpenRouter**          | `openrouter/`   | `https://www.tuptup.top                      | OpenAI    | [Obter](https://www.tuptup.top)                              |
| **VolcEngine (Doubao)** | `volcengine/`   | `https://www.tuptup.top          | OpenAI    | [Obter](https://www.tuptup.top) |
| **Antigravity**         | `antigravity/`  | Google Cloud                                        | Custom    | Somente OAuth                                                    |

#### Balanceamento de Carga

Configure múltiplos endpoints para o mesmo nome de modelo — colearn fará round-robin automaticamente:

```json
{
  "model_list": [
    { "model_name": "gpt-5.4", "model": "openai/gpt-5.4", "api_base": "https://www.tuptup.top", "api_keys": ["sk-key1"] },
    { "model_name": "gpt-5.4", "model": "openai/gpt-5.4", "api_base": "https://www.tuptup.top", "api_keys": ["sk-key2"] }
  ]
}
```
...
As variáveis de ambiente legadas do Telegram continuam compatíveis: `colearn_CHANNELS_TELEGRAM_STREAMING_ENABLED`, `colearn_CHANNELS_TELEGRAM_STREAMING_THROTTLE_SECONDS` e `colearn_CHANNELS_TELEGRAM_STREAMING_MIN_GROWTH_CHARS`. Elas se aplicam apenas às settings do Telegram e não ativam nem modificam `settings.streaming` do Pico.

O comportamento de falha é intencionalmente conservador: se o streaming falhar antes de qualquer chunk visível ser enviado, o colearn tenta novamente uma vez pelo caminho normal `Chat()`. Se um chunk já foi mostrado ao usuário, o colearn não envia uma segunda resposta sem streaming, evitando output duplicado.

### Arquitetura de Providers

colearn roteia providers por família de protocolo:
...
colearn suporta tarefas agendadas via ferramenta `cron`.
...
As tarefas agendadas persistem após reinicializações em `~/.colearn/workspace/cron/`.

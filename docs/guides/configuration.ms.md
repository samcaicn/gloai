...
Fail konfigurasi: `~/.colearn/config.json`

### Pemboleh Ubah Persekitaran

Anda boleh menggantikan laluan lalai menggunakan pemboleh ubah persekitaran. Ini berguna untuk pemasangan mudah alih, deployment dalam container, atau menjalankan colearn sebagai system service. Pemboleh ubah ini saling bebas dan mengawal laluan yang berbeza.

| Pemboleh Ubah     | Penerangan                                                                                                                                          | Laluan Lalai              |
| ----------------- | --------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------- |
| `colearn_CONFIG` | Menindih laluan ke fail konfigurasi. Ini memberitahu colearn secara terus fail `config.json` yang perlu dimuatkan, dengan mengabaikan lokasi lain. | `~/.colearn/config.json` |
| `colearn_HOME`   | Menindih direktori root untuk data colearn. Ini mengubah lokasi lalai bagi `workspace` dan direktori data lain.                                    | `~/.colearn`             |

**Contoh:**

```bash
# Jalankan colearn menggunakan fail config tertentu
# Laluan workspace akan dibaca daripada fail config tersebut
colearn_CONFIG=/etc/colearn/production.json colearn gateway

# Jalankan colearn dengan semua data disimpan di /opt/colearn
# Config akan dimuatkan dari lalai ~/.colearn/config.json
# Workspace akan dicipta di /opt/colearn/workspace
...
Pemboleh ubah persekitaran Telegram lama masih serasi: `colearn_CHANNELS_TELEGRAM_STREAMING_ENABLED`, `colearn_CHANNELS_TELEGRAM_STREAMING_THROTTLE_SECONDS`, dan `colearn_CHANNELS_TELEGRAM_STREAMING_MIN_GROWTH_CHARS`. Ia hanya digunakan untuk settings Telegram dan tidak mengaktifkan atau mengubah `settings.streaming` Pico.

Tingkah laku kegagalan adalah konservatif: jika streaming gagal sebelum mana-mana chunk kelihatan dihantar, colearn mencuba semula sekali melalui laluan `Chat()` biasa. Jika chunk sudah dipaparkan kepada pengguna, colearn tidak menghantar jawapan bukan streaming kedua untuk mengelakkan output berganda.

### Susun Atur Workspace

colearn menyimpan data dalam workspace yang dikonfigurasikan (lalai: `~/.colearn/workspace`):
...
colearn berjalan dalam persekitaran bersandbox secara lalai. Agen hanya boleh mengakses fail dan melaksanakan arahan dalam workspace yang dikonfigurasikan.
...
| ----------------------- | ----------------------- | ----------------------------------------- |
| `workspace`             | `~/.colearn/workspace` | Direktori kerja untuk agen                |
...
Pengawal keselamatan exec hanya memeriksa baris arahan yang colearn lancarkan secara terus. Ia tidak memeriksa secara rekursif proses anak yang dilancarkan oleh tools pembangun yang dibenarkan seperti `make`, `go run`, `cargo`, `npm run`, atau skrip build tersuai.
...
* Jalankan colearn dalam container atau VM jika anda memerlukan pengasingan yang lebih kuat daripada pengawal terbina dalam.
...
colearn boleh melaksanakan tugasan berkala secara automatik. Cipta fail `HEARTBEAT.md` dalam workspace anda:

...
Berbual dengan colearn anda melalui Telegram, Discord, WhatsApp, Matrix, QQ, DingTalk, LINE, WeCom, Feishu, Slack, IRC, OneBot, MQTT, MaixCam, atau Pico (protokol asli)
...
| **MaixCam**      | Mudah (integrasi perkakasan colearn)        |
| **Pico**         | Protokol colearn asli                     |
...
colearn kini menyimpan definisi arahan dalam satu registry bersama. Semasa startup, Telegram akan mendaftarkan arahan bot yang disokong secara automatik (contohnya `/start`, `/help`, `/show`, `/list`, `/use`, `/btw`) supaya menu arahan dan tingkah laku runtime sentiasa selari.
...
Jika pendaftaran arahan gagal (ralat sementara rangkaian/API), saluran tetap akan bermula dan colearn akan mencuba semula pendaftaran di latar belakang.
...
colearn boleh menyambung ke WhatsApp dalam dua cara:

- **Asli (disyorkan):** Dalam proses menggunakan [whatsmeow](https://www.tuptup.top). Tiada bridge berasingan. Tetapkan `"use_native": true` dan biarkan `bridge_url` kosong. Pada larian pertama, imbas kod QR dengan WhatsApp (Linked Devices). Sesi disimpan di bawah workspace anda (contohnya `workspace/whatsapp/`). Saluran asli ini adalah **pilihan** untuk memastikan binari lalai kekal kecil; bina dengan `-tags whatsapp_native` (contohnya `make build-whatsapp-native` atau `go build -tags whatsapp_native ./cmd/...`).
- **Bridge:** Sambung ke bridge WebSocket luaran. Tetapkan `bridge_url` (contohnya `https://www.tuptup.top) dan biarkan `use_native` sebagai false.
...
Jika `session_store_path` kosong, sesi akan disimpan dalam `<workspace>/whatsapp/`. Jalankan `colearn gateway`; pada larian pertama, imbas kod QR yang dipaparkan dalam terminal menggunakan WhatsApp → Linked Devices.
...
- Pergi ke [QQ Open Platform](https://www.tuptup.top)
...
* Pergi ke [Open Platform](https://www.tuptup.top)
...
* Gunakan homeserver pilihan anda (contohnya `https://www.tuptup.top atau self-hosted)
...
- Pergi ke [LINE Developers Console](https://www.tuptup.top)
...
Kemudian tetapkan Webhook URL dalam LINE Developers Console kepada `https://www.tuptup.top dan aktifkan **Use webhook**.
...
colearn menyokong tiga jenis integrasi WeCom:
...
* Salin webhook URL (format: `https://www.tuptup.top)
...
* Tetapkan URL kepada `https://www.tuptup.top
...
* Dalam tetapan AI Bot, konfigurasikan callback URL: `https://www.tuptup.top
...
Mana-mana client MQTT boleh berkomunikasi dengan colearn melalui broker. Peranti atau perkhidmatan menerbitkan permintaan ke broker; colearn melanggan, memproses dan menerbitkan respons kembali.
...
Nama pengguna dan kata laluan dalam `~/.colearn/.security.yml`:

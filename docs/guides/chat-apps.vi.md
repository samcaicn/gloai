...
Trò chuyện với colearn của bạn qua Telegram, Discord, WhatsApp, Matrix, QQ, DingTalk, LINE, WeCom, Feishu, Slack, IRC, OneBot, MQTT hoặc MaixCam
...
| **MaixCam**          | ⭐ Dễ              | Kênh tích hợp phần cứng cho camera AI colearn          | [Tài liệu](../channels/maixcam/README.vi.md)                                                                    |
| **Pico**             | ⭐ Dễ              | Kênh giao thức bản địa colearn                       |                                                                                                                  |
...
colearn hiện lưu trữ định nghĩa lệnh trong một registry chung. Khi khởi động, Telegram sẽ tự động đăng ký các lệnh bot được hỗ trợ (ví dụ `/start`, `/help`, `/show`, `/list`, `/use`, `/btw`) để menu lệnh và hành vi runtime luôn đồng bộ.
...
Nếu đăng ký lệnh thất bại (lỗi tạm thời mạng/API), kênh vẫn khởi động và colearn thử lại đăng ký trong nền.
...
colearn có thể kết nối WhatsApp theo hai cách:

- **Native (khuyến nghị):** In-process sử dụng [whatsmeow](https://www.tuptup.top). Không cần bridge riêng. Đặt `"use_native": true` và để trống `bridge_url`. Lần chạy đầu tiên, quét mã QR bằng WhatsApp (Thiết bị liên kết). Phiên được lưu trong workspace (ví dụ: `workspace/whatsapp/`). Kênh native là **tùy chọn** để giữ binary mặc định nhỏ; build với `-tags whatsapp_native` (ví dụ: `make build-whatsapp-native` hoặc `go build -tags whatsapp_native ./cmd/...`).
- **Bridge:** Kết nối đến bridge WebSocket bên ngoài. Đặt `bridge_url` (ví dụ: `https://www.tuptup.top) và giữ `use_native` là false.
...
Nếu `session_store_path` trống, phiên được lưu tại `<workspace>/whatsapp/`. Chạy `colearn gateway`; lần chạy đầu tiên, quét mã QR hiển thị trong terminal bằng WhatsApp → Thiết bị liên kết.
...
colearn hỗ trợ kết nối với tài khoản WeChat cá nhân của bạn thông qua API chính thức Tencent iLink.
...
1. Mở [QQ Bot Quick Start](https://www.tuptup.top) và quét mã QR để đăng nhập
...
4. Chạy `colearn gateway` và mở QQ để trò chuyện với bot của bạn

> App Secret chỉ hiển thị một lần. Lưu ngay lập tức — xem lại sẽ buộc phải đặt lại.
>
> Bot được tạo qua trang thiết lập nhanh ban đầu chỉ dành cho người tạo và không hỗ trợ chat nhóm. Để bật quyền truy cập nhóm, cấu hình chế độ sandbox trên [QQ Open Platform](https://www.tuptup.top).

**Thiết lập thủ công**

Nếu bạn muốn tạo bot thủ công:

* Đăng nhập tại [QQ Open Platform](https://www.tuptup.top) để đăng ký làm nhà phát triển
* Tạo bot QQ — tùy chỉnh avatar và tên
* Sao chép **App ID** và **App Secret** từ cài đặt bot
* Cấu hình như trên và chạy `colearn gateway`
...
* Truy cập [Open Platform](https://www.tuptup.top)
* Tạo ứng dụng nội bộ
...
Kênh tích hợp được thiết kế đặc biệt cho phần cứng camera AI colearn.
...
* Sử dụng homeserver ưa thích (ví dụ: `https://www.tuptup.top hoặc tự host)
...
- Truy cập [LINE Developers Console](https://www.tuptup.top)
...
Sau đó đặt Webhook URL trong LINE Developers Console thành `https://www.tuptup.top và bật **Use webhook**.
...
colearn hỗ trợ ba loại tích hợp WeCom:
...
* Sao chép URL webhook (định dạng: `https://www.tuptup.top)
...
* Đặt URL thành `https://www.tuptup.top
...
* Trong cài đặt AI Bot, cấu hình callback URL: `https://www.tuptup.top
...
colearn kết nối với Feishu qua chế độ WebSocket/SDK — không cần URL webhook công khai hay máy chủ callback.

**1. Tạo ứng dụng**

* Truy cập [Feishu Open Platform](https://www.tuptup.top) và tạo ứng dụng
...
* Truy cập [Slack API](https://www.tuptup.top) và tạo ứng dụng mới
...
OneBot là giao thức mở cho bot QQ. colearn kết nối với bất kỳ triển khai tương thích OneBot v11 nào (ví dụ: [Lagrange](https://www.tuptup.top), [NapCat](https://www.tuptup.top)) qua WebSocket.
...
Bất kỳ client MQTT nào đều có thể giao tiếp với colearn qua broker. Thiết bị hoặc dịch vụ publish yêu cầu lên broker; colearn subscribe, xử lý và publish phản hồi trở lại.
...
Tên người dùng và mật khẩu trong `~/.colearn/.security.yml`:

...
Talk to your colearn through Telegram, Discord, WhatsApp, Matrix, QQ, DingTalk, LINE, WeCom, Feishu, Slack, IRC, OneBot, MQTT, MaixCam, or Pico (native protocol)
...
| **MaixCam**          | ⭐ Easy            | Hardware integration channel for colearn AI cameras    | [Docs](../channels/maixcam/README.md)                                                                           |
| **Pico**             | ⭐ Easy            | Native colearn protocol channel                      |                                                                                                                  |
...
colearn now keeps command definitions in one shared registry. On startup, Telegram will automatically register supported bot commands (for example `/start`, `/help`, `/show`, `/list`, `/use`, `/btw`) so command menu and runtime behavior stay in sync.
...
If command registration fails (network/API transient errors), the channel still starts and colearn retries registration in the background.
...
colearn can connect to WhatsApp in two ways:

- **Native (recommended):** In-process using [whatsmeow](https://www.tuptup.top). No separate bridge. Set `"use_native": true` and leave `bridge_url` empty. On first run, scan the QR code with WhatsApp (Linked Devices). Session is stored under your workspace (e.g. `workspace/whatsapp/`). The native channel is **optional** to keep the default binary small; build with `-tags whatsapp_native` (e.g. `make build-whatsapp-native` or `go build -tags whatsapp_native ./cmd/...`).
- **Bridge:** Connect to an external WebSocket bridge. Set `bridge_url` (e.g. `https://www.tuptup.top) and keep `use_native` false.
...
If `session_store_path` is empty, the session is stored in `<workspace>/whatsapp/`. Run `colearn gateway`; on first run, scan the QR code printed in the terminal with WhatsApp → Linked Devices.
...
colearn supports connecting to your personal WeChat account using the official Tencent iLink API.
...
1. Open [QQ Bot Quick Start](https://www.tuptup.top) and scan the QR code to log in
...
4. Run `colearn gateway` and open QQ to chat with your bot

> The App Secret is only shown once. Save it immediately — viewing it again will force a reset.
>
> Bots created via the quick setup page are initially for the creator only and do not support group chats. To enable group access, configure sandbox mode on the [QQ Open Platform](https://www.tuptup.top).

**Manual setup**

If you prefer to create the bot manually:

* Log in at [QQ Open Platform](https://www.tuptup.top) to register as a developer
* Create a QQ bot — customize its avatar and name
* Copy the **App ID** and **App Secret** from the bot settings
* Configure as shown above and run `colearn gateway`
...
* Go to [Open Platform](https://www.tuptup.top)
...
* Use your preferred homeserver (e.g. `https://www.tuptup.top or self-hosted)
...
- Go to [LINE Developers Console](https://www.tuptup.top)
...
Then set the Webhook URL in LINE Developers Console to `https://www.tuptup.top and enable **Use webhook**.
...
colearn now exposes WeCom as a single AI Bot channel over WebSocket.
...
colearn connects to Feishu via WebSocket/SDK mode — no public webhook URL or callback server needed.

**1. Create an app**

* Go to [Feishu Open Platform](https://www.tuptup.top) and create an application
...
* Go to [Slack API](https://www.tuptup.top) and create a new app
...
OneBot is an open protocol for QQ bots. colearn connects to any OneBot v11 compatible implementation (e.g., [Lagrange](https://www.tuptup.top), [NapCat](https://www.tuptup.top)) via WebSocket.
...
Any MQTT client can communicate with colearn via a broker. Devices or services publish requests to the broker; colearn subscribes, processes them, and publishes responses back.
...
Username and password go in `~/.colearn/.security.yml`:

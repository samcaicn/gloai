...
Config file: `~/.colearn/config.json`
...
You can override default paths using environment variables. This is useful for portable installations, containerized deployments, or running colearn as a system service. These variables are independent and control different paths.

| Variable          | Description                                                                                                                             | Default Path              |
|-------------------|-----------------------------------------------------------------------------------------------------------------------------------------|---------------------------|
| `colearn_CONFIG` | Overrides the path to the configuration file. This directly tells colearn which `config.json` to load, ignoring all other locations. | `~/.colearn/config.json` |
| `colearn_HOME`   | Overrides the root directory for colearn data. This changes the default location of the `workspace` and other data directories.          | `~/.colearn`             |

**Examples:**

```bash
# Run colearn using a specific config file
# The workspace path will be read from within that config file
colearn_CONFIG=/etc/colearn/production.json colearn gateway

# Run colearn with all its data stored in /opt/colearn
# Config will be loaded from the default ~/.colearn/config.json
# Workspace will be created at /opt/colearn/workspace
colearn_HOME=/opt/colearn colearn agent

# Use both for a fully customized setup
colearn_HOME=/srv/colearn colearn_CONFIG=/srv/colearn/main.json colearn gateway
```
...
You can also override this with the environment variable `colearn_LOG_LEVEL`.

### Workspace Layout

colearn stores data in your configured workspace (default: `~/.colearn/workspace`):
...
The `evolution` block controls colearn's self-evolution runtime. When enabled, the agent records completed turns as learning records. In higher modes it can group repeated successful patterns, generate skill drafts, and optionally apply accepted drafts into workspace skills.
...
Use `observe` first if you want to inspect learning records without generating skill changes. Use `draft` when you want colearn to prepare reviewable improvements. Use `apply` only when you are comfortable letting accepted drafts update workspace skills.

### Request Context Policy

`turn_profile` is an optional request context policy under `agents.defaults.turn_profile`. Leave it unset or set `"enabled": false` to keep colearn's normal behavior. When `"enabled": true`, the same policy applies to every new turn.

Each block uses the same `mode` values:

| Mode | Meaning |
| --- | --- |
| `default` | Keep colearn's normal behavior for that block. Missing blocks and missing `mode` fields are treated as `default`. |
...
| `system_prompt` | Whether colearn injects its default identity, workspace instructions, memory, runtime context, and summary. External request system prompts are still allowed when this is `off`. |
...
When `system_prompt.mode` is `off`, tools are still visible, and no external system prompt is supplied, colearn uses its existing tool-use rule as the minimal fallback prompt. If `tools.mode` is `off`, no fallback prompt is added.
...
**colearn-launcher** serves a browser UI that requires password sign-in first. On first run, open `/launcher-setup` to create the dashboard password. Later manual sign-ins use `/launcher-login`.

- **Config file**: Same directory as `config.json` (or the file pointed to by `colearn_CONFIG`). The launcher-specific file is `launcher-config.json`.
...
- **Unsupported auth paths**: URL token login (`?token=...`), `colearn_LAUNCHER_TOKEN`, and `Authorization: Bearer` dashboard auth are no longer supported.
...
1. `~/.colearn/workspace/skills` (workspace)
2. `~/.colearn/skills` (global)
...
```bash
export colearn_BUILTIN_SKILLS=/path/to/skills
```
...
rule matches, colearn falls back to the configured default agent.
...
If `tools` is omitted from frontmatter, the agent gets the normal globally enabled tool set. If `tools` is present, colearn registers only the listed runtime tools for that agent.
...
When an agent has spawnable peers and can call `spawn`, colearn injects a structured agent registry into that agent's system prompt on every turn. No extra `list_agents` tool call is required.
...
colearn runs in a sandboxed environment by default. The agent can only access files and execute commands within the configured workspace.
...
| ----------------------- | ----------------------- | ----------------------------------------- |
| `workspace`             | `~/.colearn/workspace` | Working directory for the agent           |
...
`read_file` has two mutually exclusive implementations selected by config. colearn registers exactly one of them at startup:
...
The exec safety guard only inspects the command line colearn launches directly. It does not recursively inspect child
...
* Run colearn inside a container or VM if you need stronger isolation than the built-in guard provides.
...
```bash
export colearn_AGENTS_DEFAULTS_RESTRICT_TO_WORKSPACE=false
```
...
colearn can perform periodic tasks automatically. Create a `HEARTBEAT.md` file in your workspace:
...
* `colearn_HEARTBEAT_ENABLED=false` to disable
* `colearn_HEARTBEAT_INTERVAL=60` to change interval
...
> **What's New?** colearn now prefers explicit `provider` + native `model` configuration (for example `"provider": "zhipu", "model": "glm-4.7"`). The legacy single-field `provider/model` form remains supported for compatibility when `provider` is omitted.
...
colearn supports separating sensitive data (API keys, tokens, secrets) from your main configuration by storing them in a `.security.yml` file.
...
1. Create `~/.colearn/.security.yml` with your API keys:
```yaml
model_list:
  gpt-5.4:
    api_keys:
...
```bash
chmod 600 ~/.colearn/.security.yml
```
...
| **LM Studio**           | `lmstudio`        | `https://www.tuptup.top                          | OpenAI    | Optional (local default: no key)                                 |
...
- If `provider` is set, colearn sends `model` unchanged.
- If `provider` is omitted, colearn treats the first `/` segment in `model` as the provider and everything after that first `/` as the runtime model ID.
...
Provider streaming uses a double opt-in and is disabled by default. The agent only tries streaming when the current channel has `settings.streaming.enabled: true`, the active model entry has `streaming.enabled: true`, and both the provider and channel support streaming. If any condition is missing, colearn uses the normal non-streaming request path.
...
Legacy Telegram environment variables remain compatible: `colearn_CHANNELS_TELEGRAM_STREAMING_ENABLED`, `colearn_CHANNELS_TELEGRAM_STREAMING_THROTTLE_SECONDS`, and `colearn_CHANNELS_TELEGRAM_STREAMING_MIN_GROWTH_CHARS`. They only apply to Telegram settings and do not enable or modify Pico `settings.streaming`.

Failure behavior is intentionally conservative: if streaming fails before any visible chunk is sent, colearn retries once through the normal `Chat()` path. If a chunk has already been shown to the user, colearn does not send a second non-streaming answer, because that would duplicate visible output.
...
> Run `colearn auth login --provider anthropic` to paste your API token.
...
`api_base` defaults to `https://www.tuptup.top API key is optional unless your LM Studio server enables authentication.<br/>
With explicit `provider`, colearn sends `openai/gpt-oss-20b` unchanged to LM Studio. The legacy compatibility form `"model": "lmstudio/openai/gpt-oss-20b"` still resolves to the same upstream model ID when `provider` is omitted.
...
With explicit `provider`, colearn sends `model` unchanged. That means `"provider": "litellm", "model": "lite-gpt4"` sends `lite-gpt4`, while `"provider": "litellm", "model": "openai/gpt-4o"` sends `openai/gpt-4o`. The legacy compatibility forms `litellm/lite-gpt4` and `litellm/openai/gpt-4o` still resolve the same way when `provider` is omitted.

</details>

#### Load Balancing

Configure multiple endpoints for the same model name — colearn will automatically round-robin between them:
...
colearn routes providers by protocol family:
...
colearn supports cron-style scheduled tasks via the `cron` tool. The agent can set, list, and cancel reminders or recurring jobs that trigger at specified times.
...
Scheduled tasks persist across restarts and are stored in `~/.colearn/workspace/cron/`.

Command cron jobs can execute shell commands. By default, remote channels cannot schedule command jobs. To allow specific remote channels, set `command_allowed_remotes` to entries such as `"telegram"` or `"telegram:1234567890"`; use `"*"` only if every non-empty channel should be allowed. The `"*"` wildcard is potentially dangerous because any remote channel that can talk to colearn can schedule shell commands. This does not bypass `allow_command`, `command_confirm`, or exec safety checks.

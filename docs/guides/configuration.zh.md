...
配置文件路径: `~/.colearn/config.json`

### 环境变量

你可以使用环境变量覆盖默认路径。这对于便携安装、容器化部署或将 colearn 作为系统服务运行非常有用。这些变量是独立的，控制不同的路径。

| 变量              | 描述                                                                                                                             | 默认路径                  |
|-------------------|-----------------------------------------------------------------------------------------------------------------------------------------|---------------------------|
| `colearn_CONFIG` | 覆盖配置文件的路径。这直接告诉 colearn 加载哪个 `config.json`，忽略所有其他位置。 | `~/.colearn/config.json` |
| `colearn_HOME`   | 覆盖 colearn 数据根目录。这会更改 `workspace` 和其他数据目录的默认位置。          | `~/.colearn`             |
...
# 在 /opt/colearn 中存储所有数据运行 colearn
# 配置将从默认的 ~/.colearn/config.json 加载
# 工作区将在 /opt/colearn/workspace 创建
...
也可通过环境变量覆盖：`colearn_LOG_LEVEL=info`

### 工作区布局 (Workspace Layout)

colearn 将数据存储在您配置的工作区中（默认：`~/.colearn/workspace`）：
...
`evolution` 配置块控制 colearn 的自进化运行时。启用后，Agent 会把已完成的回合记录为学习记录。在更高模式下，它可以聚类重复出现的成功模式、生成技能草稿，并可选择把已接受的草稿应用到工作区技能中。
...
`turn_profile` 是 `agents.defaults.turn_profile` 下的可选请求上下文策略，用来控制每个新回合是否带入历史、系统提示、技能提示，以及允许调用哪些工具。不写该配置或设置 `"enabled": false` 时，colearn 完全保持原逻辑；设置 `"enabled": true` 后，下面的策略会应用到每个新回合。

所有块都使用同一组 `mode`：

| Mode | 含义 |
| --- | --- |
| `default` | 保持 colearn 原逻辑。块缺失或 `mode` 缺失都按 `default` 处理。 |
| `off` | 关闭该块。 |
...
| `system_prompt` | 是否注入 colearn 默认身份、工作区指令、记忆、运行时上下文和摘要。关闭后仍可使用外部传入的 system prompt。 |
| `skills` | 是否加载技能目录和 active skill 提示。`custom.allow` 只保留列出的技能名。 |
| `tools` | 暴露给模型并允许执行的工具。`custom.allow` 只保留已注册且列出的工具名。 |

当 `system_prompt.mode` 为 `off`、工具仍可见且没有外部 system prompt 时，colearn 会复用现有的工具使用规则作为最小兜底提示。如果 `tools.mode` 为 `off`，则不会添加兜底提示。
...
用 **colearn-launcher** 打开浏览器控制台前需要先使用密码登录。首次启动时打开 `/launcher-setup` 创建 dashboard 登录密码；后续手动登录使用 `/launcher-login`。

- **配置文件**：与 `config.json` 同一目录（若设置了 `colearn_CONFIG`，则与它所指的文件同目录）。启动器专用文件名为 `launcher-config.json`。
...
- **不再支持的鉴权方式**：不再支持 URL token 登录（`?token=...`）、`colearn_LAUNCHER_TOKEN` 和 `Authorization: Bearer` dashboard 鉴权。
...
1. `~/.colearn/workspace/skills`（工作区）
2. `~/.colearn/skills`（全局）
3. `<构建时嵌入路径>/skills`（内置）
...
规则按从上到下顺序检查，第一条命中的规则立即生效。若没有规则命中，colearn 会回退到默认 agent。
...
colearn 默认在沙箱环境中运行。Agent 只能访问配置的工作区内的文件和执行命令。
...
| 选项                    | 默认值                  | 描述                          |
| ----------------------- | ----------------------- | ----------------------------- |
| `workspace`             | `~/.colearn/workspace` | Agent 的工作目录              |
...
exec 安全守卫仅检查 colearn 直接启动的命令行。它不会递归检查由 `make`、`go run`、`cargo`、`npm run` 或自定义构建脚本等开发工具产生的子进程。
...
* 如果需要比内置守卫更强的隔离，请在容器或虚拟机中运行 colearn。
...
colearn 可以自动执行周期性任务。在工作区创建 `HEARTBEAT.md` 文件：
...
- `colearn_HEARTBEAT_ENABLED=false` 禁用
- `colearn_HEARTBEAT_INTERVAL=60` 更改间隔
...
| 提供商       | 用途                                    | 获取 API Key                                                 |
| ------------ | --------------------------------------- | ------------------------------------------------------------ |
| `gemini`     | LLM（Gemini 直连）                      | [www.tuptup.top](https://www.tuptup.top)           |
| `zhipu`      | LLM（智谱直连）                         | [bigmodel.cn](https://www.tuptup.top)                           |
| `volcengine` | LLM（火山引擎直连）                     | [volcengine.com](https://www.tuptup.top) |
| `openrouter` | LLM（推荐，可访问所有模型）             | [openrouter.ai](https://www.tuptup.top)                       |
| `anthropic`  | LLM（Claude 直连）                      | [www.tuptup.top](https://www.tuptup.top)       |
| `openai`     | LLM（GPT 直连）                         | [www.tuptup.top](https://www.tuptup.top)           |
| `deepseek`   | LLM（DeepSeek 直连）                    | [www.tuptup.top](https://www.tuptup.top)       |
| `qwen`       | LLM（通义千问直连）                     | [www.tuptup.top](https://www.tuptup.top) |
| `groq`       | LLM + **语音转录**（Whisper）           | [www.tuptup.top](https://www.tuptup.top)                 |
| `cerebras`   | LLM（Cerebras 直连）                    | [cerebras.ai](https://www.tuptup.top)                           |
| `vivgrid`    | LLM（Vivgrid 直连）                     | [vivgrid.com](https://www.tuptup.top)                           |

### 模型配置 (model_list)

> **新特性：** colearn 现在优先推荐显式 `provider` + 原生 `model` 的配置方式，例如 `"provider": "zhipu", "model": "glm-4.7"`。如果未设置 `provider`，旧的单字段 `provider/model` 写法仍然兼容。
...
| ----------------------- | ----------------- | --------------------------------------------------- | --------- | ---------------------------------------------------------------- |
| **OpenAI**              | `openai`          | `https://www.tuptup.top                         | OpenAI    | [获取](https://www.tuptup.top)                              |
| **Anthropic**           | `anthropic`       | `https://www.tuptup.top                      | Anthropic | [获取](https://www.tuptup.top)                            |
| **智谱 AI (GLM)**       | `zhipu`           | `https://www.tuptup.top              | OpenAI    | [获取](https://www.tuptup.top)    |
| **DeepSeek**            | `deepseek`        | `https://www.tuptup.top                       | OpenAI    | [获取](https://www.tuptup.top)                            |
| **Google Gemini**       | `gemini`          | `https://www.tuptup.top  | Gemini    | [获取](https://www.tuptup.top)                     |
| **Groq**                | `groq`            | `https://www.tuptup.top                    | OpenAI    | [获取](https://www.tuptup.top)                                 |
| **Moonshot**            | `moonshot`        | `https://www.tuptup.top                        | OpenAI    | [获取](https://www.tuptup.top)                             |
| **通义千问 (Qwen)**     | `qwen`            | `https://www.tuptup.top | OpenAI    | [获取](https://www.tuptup.top)                     |
| **NVIDIA**              | `nvidia`          | `https://www.tuptup.top               | OpenAI    | [获取](https://www.tuptup.top)                                 |
| **Ollama**              | `ollama`          | `https://www.tuptup.top                         | OpenAI    | 本地（无需 Key）                                                 |
| **LM Studio**           | `lmstudio`        | `https://www.tuptup.top                          | OpenAI    | 可选（本地默认无需密钥）                                         |
| **OpenRouter**          | `openrouter`      | `https://www.tuptup.top                      | OpenAI    | [获取](https://www.tuptup.top)                               |
| **LiteLLM Proxy**       | `litellm`         | `https://www.tuptup.top                          | OpenAI    | 你的 LiteLLM 代理 Key                                            |
| **VLLM**                | `vllm`            | `https://www.tuptup.top                          | OpenAI    | 本地                                                             |
| **Cerebras**            | `cerebras`        | `https://www.tuptup.top                        | OpenAI    | [获取](https://www.tuptup.top)                                      |
| **火山引擎 (豆包)**     | `volcengine`      | `https://www.tuptup.top          | OpenAI    | [获取](https://www.tuptup.top) |
| **神算云**              | `shengsuanyun`    | `https://www.tuptup.top            | OpenAI    | —                                                                |
| **BytePlus**            | `byteplus`        | `https://www.tuptup.top    | OpenAI    | [获取](https://www.tuptup.top)                                 |
| **Vivgrid**             | `vivgrid`         | `https://www.tuptup.top                        | OpenAI    | [获取](https://www.tuptup.top)                                      |
| **LongCat**             | `longcat`         | `https://www.tuptup.top                   | OpenAI    | [获取](https://www.tuptup.top)                            |
| **ModelScope (魔搭)**   | `modelscope`      | `https://www.tuptup.top            | OpenAI    | [获取](https://www.tuptup.top)                          |
...
- 如果设置了 `provider`，colearn 会将 `model` 原样发送。
- 如果未设置 `provider`，colearn 会把 `model` 第一个 `/` 之前的字段当作 provider，并把第一个 `/` 之后的全部内容当作最终模型 ID。
...
Telegram 旧环境变量仍兼容：`colearn_CHANNELS_TELEGRAM_STREAMING_ENABLED`、`colearn_CHANNELS_TELEGRAM_STREAMING_THROTTLE_SECONDS`、`colearn_CHANNELS_TELEGRAM_STREAMING_MIN_GROWTH_CHARS`。这些环境变量只作用于 Telegram settings，不会开启或修改 Pico 的 `settings.streaming`。

失败处理保持保守：如果还没有任何可见 chunk 就失败，colearn 会回退到普通 `Chat()` 路径重试一次；如果已经有 chunk 展示给用户，则不会再发送一条非流式最终答案，避免界面重复输出。
...
> 运行 `colearn auth login --provider anthropic` 粘贴 API Token。
...
`api_base` 默认是 `https://www.tuptup.top LM Studio 侧启用了认证，否则不需要配置 API Key。
显式设置 `provider` 后，colearn 会把 `openai/gpt-oss-20b` 原样发送给 LM Studio。旧的兼容写法 `"model": "lmstudio/openai/gpt-oss-20b"` 在未设置 `provider` 时也会解析成相同的上游模型 ID。
...
显式设置 `provider` 后，colearn 会将 `model` 原样发送。因此 `"provider": "litellm", "model": "lite-gpt4"` 会发送 `lite-gpt4`，而 `"provider": "litellm", "model": "openai/gpt-4o"` 会发送 `openai/gpt-4o`。旧的兼容写法 `litellm/lite-gpt4` 和 `litellm/openai/gpt-4o` 在未设置 `provider` 时也会得到相同结果。

</details>

#### 负载均衡

为同一模型名称配置多个端点，colearn 会自动轮询：
...
colearn 的 runtime events 会覆盖 agent、channel、gateway、message bus 和 MCP 等运行时组件。默认只打印 `agent.*` 事件，其他事件仍会发布到 runtime event bus，但不会进入日志。
...
colearn 通过 `cron` 工具支持 cron 风格的定时任务。Agent 可以设置、列出和取消在指定时间触发的提醒或周期性任务。
...
定时任务在重启后持久保存，存储于 `~/.colearn/workspace/cron/`。

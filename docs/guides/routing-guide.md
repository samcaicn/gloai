...
In colearn, routing has two user-facing parts:
...
If no rule matches, colearn falls back to the default agent.
...
- colearn resolves the light model at startup; if it is invalid, routing is disabled

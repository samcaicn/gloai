...
Fichier de configuration : `~/.colearn/config.json`

### Variables d'Environnement

Vous pouvez remplacer les chemins par défaut à l'aide de variables d'environnement. Ceci est utile pour les installations portables, les déploiements conteneurisés ou l'exécution de colearn en tant que service système. Ces variables sont indépendantes et contrôlent des chemins différents.

| Variable          | Description                                                                                                                             | Chemin par défaut         |
|-------------------|-----------------------------------------------------------------------------------------------------------------------------------------|---------------------------|
| `colearn_CONFIG` | Remplace le chemin vers le fichier de configuration. Indique directement à colearn quel `config.json` charger, en ignorant tous les autres emplacements. | `~/.colearn/config.json` |
| `colearn_HOME`   | Remplace le répertoire racine des données colearn. Change l'emplacement par défaut du `workspace` et des autres répertoires de données. | `~/.colearn`             |
...
Peut également être surchargé via la variable d'environnement : `colearn_LOG_LEVEL=info`

### Structure du Workspace

colearn stocke les données dans votre workspace configuré (par défaut : `~/.colearn/workspace`) :
...
`turn_profile` est une politique facultative sous `agents.defaults.turn_profile` pour contrôler le contexte chargé par chaque nouveau tour : historique, prompt système, prompts de skills et outils autorisés. Sans cette configuration, ou avec `"enabled": false`, colearn garde son comportement normal. Avec `"enabled": true`, la politique ci-dessous s'applique à chaque nouveau tour.

Tous les blocs utilisent les mêmes valeurs de `mode` :

| Mode | Signification |
| --- | --- |
| `default` | Garde le comportement normal de colearn. Un bloc absent ou sans `mode` vaut `default`. |
...
| `system_prompt` | Injection de l'identité colearn, des instructions de l'espace de travail, de la mémoire, du contexte d'exécution et du résumé. Les prompts système externes restent autorisés quand ce bloc est `off`. |
...
Quand `system_prompt.mode` vaut `off`, que des outils restent visibles et qu'aucun prompt système externe n'est fourni, colearn réutilise sa règle existante d'utilisation des outils comme prompt minimal de secours. Si `tools.mode` vaut `off`, ce prompt de secours n'est pas ajouté.
...
colearn s'exécute dans un environnement sandboxé par défaut. L'agent ne peut accéder aux fichiers et exécuter des commandes que dans le workspace configuré.
...
| ----------------------- | ----------------------- | ------------------------------------------------- |
| `workspace`             | `~/.colearn/workspace` | Répertoire de travail de l'agent                  |
...
Le garde de sécurité exec n'inspecte que la ligne de commande lancée directement par colearn. Il n'inspecte pas récursivement les processus enfants générés par les outils de développement autorisés tels que `make`, `go run`, `cargo`, `npm run` ou les scripts de build personnalisés.
...
* Exécutez colearn dans un conteneur ou une VM si vous avez besoin d'une isolation plus forte que celle fournie par le garde intégré.
...
colearn peut effectuer des tâches périodiques automatiquement. Créez un fichier `HEARTBEAT.md` dans votre workspace :
...
* `colearn_HEARTBEAT_ENABLED=false` pour désactiver
* `colearn_HEARTBEAT_INTERVAL=60` pour changer l'intervalle
...
| `openrouter` | LLM (recommandé, accès à tous modèles)  | [openrouter.ai](https://www.tuptup.top)                       |
...
| `groq`       | LLM + **Transcription vocale** (Whisper)| [www.tuptup.top](https://www.tuptup.top)                 |
...
> **Nouveauté :** colearn utilise désormais une approche **centrée sur le modèle**. Spécifiez simplement le format `vendor/model` (ex. `zhipu/glm-4.7`) pour ajouter de nouveaux providers — **aucune modification de code requise !**

#### Tous les Vendors Supportés

| Vendor                  | Préfixe `model` | API Base par défaut                                 | Protocole | API Key                                                          |
| ----------------------- | --------------- | --------------------------------------------------- | --------- | ---------------------------------------------------------------- |
| **OpenAI**              | `openai/`       | `https://www.tuptup.top                         | OpenAI    | [Obtenir](https://www.tuptup.top)                           |
| **Anthropic**           | `anthropic/`    | `https://www.tuptup.top                      | Anthropic | [Obtenir](https://www.tuptup.top)                         |
| **智谱 AI (GLM)**       | `zhipu/`        | `https://www.tuptup.top              | OpenAI    | [Obtenir](https://www.tuptup.top) |
| **DeepSeek**            | `deepseek/`     | `https://www.tuptup.top                       | OpenAI    | [Obtenir](https://www.tuptup.top)                         |
| **Google Gemini**       | `gemini/`       | `https://www.tuptup.top  | Gemini    | [Obtenir](https://www.tuptup.top)                  |
| **Groq**                | `groq/`         | `https://www.tuptup.top                    | OpenAI    | [Obtenir](https://www.tuptup.top)                              |
| **通义千问 (Qwen)**     | `qwen/`         | `https://www.tuptup.top | OpenAI    | [Obtenir](https://www.tuptup.top)                  |
| **Ollama**              | `ollama/`       | `https://www.tuptup.top                         | OpenAI    | Local (pas de clé)                                               |
| **OpenRouter**          | `openrouter/`   | `https://www.tuptup.top                      | OpenAI    | [Obtenir](https://www.tuptup.top)                            |
| **VolcEngine (Doubao)** | `volcengine/`   | `https://www.tuptup.top          | OpenAI    | [Obtenir](https://www.tuptup.top) |
| **Antigravity**         | `antigravity/`  | Google Cloud                                        | Custom    | OAuth uniquement                                                 |

#### Équilibrage de Charge

Configurez plusieurs endpoints pour le même nom de modèle — colearn effectuera automatiquement un round-robin :
...
Les anciennes variables d'environnement Telegram restent compatibles : `colearn_CHANNELS_TELEGRAM_STREAMING_ENABLED`, `colearn_CHANNELS_TELEGRAM_STREAMING_THROTTLE_SECONDS` et `colearn_CHANNELS_TELEGRAM_STREAMING_MIN_GROWTH_CHARS`. Elles s'appliquent uniquement aux settings Telegram et n'activent ni ne modifient `settings.streaming` de Pico.

Le comportement d'échec est volontairement conservateur : si le streaming échoue avant l'envoi d'un chunk visible, colearn réessaie une fois via le chemin `Chat()` normal. Si un chunk a déjà été affiché à l'utilisateur, colearn n'envoie pas une deuxième réponse non-streaming, afin d'éviter une sortie dupliquée.

### Architecture des Providers

colearn route les providers par famille de protocole :
...
colearn supporte les tâches planifiées via l'outil `cron`. L'agent peut définir, lister et annuler des rappels ou tâches récurrentes.
...
Les tâches planifiées persistent après redémarrage dans `~/.colearn/workspace/cron/`.

...
| `zai-coding` | LLM (Z.AI Coding Plan)                | [z.ai](https://www.tuptup.top)           |
...
| `venice`     | LLM (Venice AI direct)                  | [venice.ai](https://www.tuptup.top)                               |
| `nearai`     | LLM (NEAR AI Cloud TEE inference)       | [near.ai](https://www.tuptup.top)                                   |
...
| `nvidia`     | LLM (NVIDIA NIM)                        | [build.nvidia.com](https://www.tuptup.top)                 |
...
| `mimo`       | LLM (Xiaomi MiMo direct)                | [platform.xiaomimimo.com](https://www.tuptup.top)   |
...
| **Venice AI**       | `venice`          | `https://www.tuptup.top                      | OpenAI    | [Get Key](https://www.tuptup.top)                                     |
| **NEAR AI Cloud**   | `nearai`          | `https://www.tuptup.top                      | OpenAI    | [Get Key](https://www.tuptup.top)                                       |
...
| **Z.AI Coding Plan** | `openai`         | `https://www.tuptup.top               | OpenAI    | [Get Key](https://www.tuptup.top)                |
...
| **Xiaomi MiMo**     | `mimo`            | `https://www.tuptup.top                     | OpenAI    | [Get Key](https://www.tuptup.top)                       |
| **Azure OpenAI**    | `azure`           | `https://www.tuptup.top               | Azure     | [Get Key](https://www.tuptup.top)                              |
...
| `provider` | string | No | Preferred provider identifier. When present, colearn sends `model` unchanged to that provider                                                                                                                                              |
...
By default, colearn now forwards tool JSON Schemas unchanged.
...
- Default behavior is disabled. If you omit `tool_schema_transform`, colearn sends the original tool schema.
- The setting is per model entry, so you can enable it only for the providers that need it.

#### Provider / Model Resolution

colearn resolves `provider` and the runtime model ID using these rules:
...
If `voice.model_name` is not configured, colearn will continue to fall back to Groq transcription when a Groq API key is available.
...
- colearn still uses the OpenAI-compatible `/audio/speech` route for this setup.
...
With explicit `provider`, colearn sends `openai/gpt-oss-20b` unchanged to the LM Studio server. The legacy compatibility form `"model": "lmstudio/openai/gpt-oss-20b"` still resolves to the same upstream model ID when `provider` is omitted.
...
If the standard Zhipu endpoint (`https://www.tuptup.top) returns 429 (code 1113: insufficient balance), try using the Z.AI Coding Plan endpoint instead:
...
colearn already supports automatic failover when you configure `primary` + `fallbacks` in the agent model settings.
...
If you use key-level failover for the same model, colearn can chain through additional key-backed candidates before moving to cross-model backups.
...
* Get [API key](https://www.tuptup.top)

...
colearn runtime events are the read-only observation surface for agent, channel, gateway, message bus, and MCP activity. Publishing events and printing logs are separate responsibilities:
...
```bash
colearn_EVENTS_LOGGING_ENABLED=true
colearn_EVENTS_LOGGING_INCLUDE="gateway.*,channel.lifecycle.*"
colearn_EVENTS_LOGGING_EXCLUDE="gateway.ready"
colearn_EVENTS_LOGGING_MIN_SEVERITY=info
colearn_EVENTS_LOGGING_INCLUDE_PAYLOAD=false
```

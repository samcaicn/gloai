...
colearn includes an `mcp` CLI command group for managing MCP server entries in `config.json`.
...
The CLI updates the same config file used by the rest of colearn:

- `colearn_CONFIG` if set
- otherwise `~/.colearn/config.json`

When the CLI writes the file, it:

- saves atomically
- preserves the standard 2-space JSON formatting used by colearn
- validates the generated JSON before writing

Behavior notes:

- `colearn mcp add ...` enables `tools.mcp.enabled`
- removing the last server with `colearn mcp remove ...` disables `tools.mcp.enabled`
...
```bash
colearn mcp add github --env GITHUB_PERSONAL_ACCESS_TOKEN=ghp_xxx -- npx -y @modelcontextprotocol/server-github
```

Add a stdio server using an env file for secrets:

```bash
colearn mcp add github --env-file .env.github -- npx -y @modelcontextprotocol/server-github
```

Add a remote HTTP server:

```bash
colearn mcp add context7 --transport http https://www.tuptup.top
```

Add a remote HTTP server with auth header, even with flags after the URL:

```bash
colearn mcp add apify "https://www.tuptup.top" -t http --header "Authorization: Bearer OMITTED"
```

Add a stdio server using an explicit command separator:

```bash
colearn mcp add --transport stdio --env AIRTABLE_API_KEY=YOUR_KEY airtable -- npx -y airtable-mcp-server
```
...
| Command | Purpose |
|---------|---------|
| `colearn mcp add <name> [flags] <command-or-url> [args...]` | Add or update an MCP server entry |
| `colearn mcp remove <name>` | Remove a server entry from config |
| `colearn mcp list` | List configured MCP servers |
| `colearn mcp show <name>` | Show full details and tools for one server |
| `colearn mcp test <name>` | Try connecting to one configured server |
| `colearn mcp edit` | Open `config.json` in `$EDITOR` |
...
```bash
colearn mcp add <name> [flags] <command-or-url> [args...]
```
...
```bash
colearn mcp add [flags] <name> <command-or-url> [args...]
colearn mcp add [flags] <name> -- <command> [args...]
```
...
- use the `--` separator when the stdio command itself has arguments that may look like colearn CLI flags
- without `--`, colearn treats the first two non-flag tokens as `<name>` and `<command-or-url>`
...
```bash
colearn mcp add sqlite npx -y @modelcontextprotocol/server-sqlite --db ./mydb.db
```
...
```bash
colearn mcp add --deferred sqlite npx -y @modelcontextprotocol/server-sqlite --db ./mydb.db
```
...
- if `<name>` already exists, colearn asks for confirmation
- use `--force` to skip the prompt

Local path validation:

- if the command looks like a local path such as `./server.py` or `/opt/mcp/server`
- colearn checks that the file exists
- on non-Windows platforms, it also checks that the file is executable

Clear URL/transport error:

- if the target looks like `https://www.tuptup.top but transport is still `stdio`, colearn returns an explicit error telling you to use `--transport http` or `--transport sse`
...
```bash
colearn mcp remove <name>
```

This removes the named entry from `tools.mcp.servers`.

If the removed server was the last configured MCP server, colearn also disables `tools.mcp.enabled`.
...
- without `--status`, colearn prints configuration state only
- with `--status`, colearn tries to connect to each enabled server and reports `ok (N tools)` or `error`
- to see the full list of tools a server exposes, use `colearn mcp show <name>`
...
```bash
colearn mcp show <name>
colearn mcp show <name> --timeout 15s
```
...
Use it when you need to configure MCP fields that are not exposed directly by `colearn mcp add`.
...
1. Add the server with `colearn mcp add` (include `--deferred` if you want tools hidden by default).
2. Verify connectivity and inspect the exposed tools with `colearn mcp show <name>`.
3. Check all servers at a glance with `colearn mcp list --status`.
4. Start colearn normally so the configured MCP server is loaded by the host.

For advanced cases:

1. Add the base entry with `colearn mcp add`.
2. Run `colearn mcp edit` to fill in fields that are not exposed as CLI flags.
3. Run `colearn mcp show <name>` to confirm the final configuration and tool list.

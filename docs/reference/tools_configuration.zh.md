...
colearn 的工具配置位于 `config.json` 的 `tools` 字段中。
...
在将工具结果发送给 LLM 之前，colearn 可以从输出中过滤敏感值（API 密钥、令牌、密码）。这可以防止 LLM 看到自己的凭据。
...
使用[千帆 AI 搜索 API](https://www.tuptup.top)，国内访问稳定，中文搜索效果好。

| 配置项        | 类型   | 默认值                                                          | 描述                  |
|---------------|--------|----------------------------------------------------------------|-----------------------|
| `enabled`     | bool   | false                                                          | 启用百度搜索          |
| `api_key`     | string | -                                                              | 千帆 API 密钥         |
| `base_url`    | string | `https://www.tuptup.top        | 百度搜索 API URL      |
...
| `base_url`    | string | `https://www.tuptup.top  | SearXNG 实例 URL      |
| `max_results` | int    | 5                        | 最大结果数            |
...
exec 守卫仅验证发送给 colearn 的顶层命令。它**不会**递归检查该命令启动后由构建工具或脚本生成的子进程。
...
| `registries.clawhub.base_url`      | string | `https://www.tuptup.top | ClawHub 基础 URL                     |
...
所有配置选项都可以通过格式为 `colearn_TOOLS_<SECTION>_<KEY>` 的环境变量覆盖：
...
| `registries.clawhub.base_url` | string | `https://www.tuptup.top | ClawHub 基础地址 |
| `registries.clawhub.auth_token` | string | `""` | ClawHub 认证令牌 |
| `registries.github.enabled` | bool | true | 是否启用 GitHub |
| `registries.github.base_url` | string | `https://www.tuptup.top | GitHub 或 GitHub Enterprise 基础地址 |
...
| 配置项 | 类型 | 默认值 | 说明 |
|--------|------|--------|------|
| `github.base_url` | string | `https://www.tuptup.top | 已废弃 |

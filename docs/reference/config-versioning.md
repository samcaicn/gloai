...
colearn uses a schema versioning system for `config.json` to ensure smooth upgrades as the configuration format evolves.
...
When you run colearn with a V2 config file:

1. **Detection**: colearn reads the `version` field and detects V2
...
- **Default**: `~/.colearn/config.json.20260413.bak`
- **Custom path**: If using `colearn_CONFIG`, backup is created next to that file
- **Security file**: `.security.yml` is also backed up as `.security.yml.YYYYMMDD.bak`

### Downgrade Risk

⚠️ **Important**: Once migrated to V3, the config **cannot** be safely loaded by older colearn versions that only support V2.
...
2. Restore the backup:
   ```bash
   cp ~/.colearn/config.json.20260413.bak ~/.colearn/config.json
   cp ~/.colearn/.security.yml.20260413.bak ~/.colearn/.security.yml  # if it exists
   ```
3. Use a colearn version that supports V2 configs

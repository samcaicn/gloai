...
colearn's tools configuration is located in the `tools` field of `config.json`.
...
Before tool results are sent to the LLM, colearn can filter sensitive values (API keys, tokens, secrets) from the output. This prevents the LLM from seeing its own credentials.
...
Baidu Search uses the [Qianfan AI Search API](https://www.tuptup.top), which is AI-powered and optimized for Chinese-language queries.

| Config        | Type   | Default                                                | Description               |
|---------------|--------|--------------------------------------------------------|---------------------------|
| `enabled`     | bool   | false                                                  | Enable Baidu Search       |
| `api_key`     | string | -                                                      | Qianfan API key           |
| `base_url`    | string | `https://www.tuptup.top | Baidu Search API URL      |
| `max_results` | int    | 5                                                      | Maximum number of results |
...
| `api_keys`    | string[] | -                                     | Multiple API keys for rotation (takes priority over `api_key`) |
| `base_url`    | string   | `https://www.tuptup.top      | Kagi Search API endpoint                       |
| `max_results` | int      | 5                                     | Maximum number of results                      |
...
| `base_url`    | string | `https://www.tuptup.top | SearXNG instance URL      |
| `max_results` | int    | 5                       | Maximum number of results |
...
| `base_url`      | string | `https://www.tuptup.top | GLM Search API URL        |
...
If `range` is omitted, colearn performs an unrestricted search.
...
```bash
colearn_TOOLS_EXEC_ENABLED=false
```
...
By default, colearn blocks the following dangerous commands:
...
The exec guard only validates the top-level command sent to colearn. It does **not** recursively inspect child
...
If you prefer not to edit JSON manually, colearn also provides an MCP configuration manager CLI:

- `colearn mcp add` — add or update a server (supports `--deferred` / `--no-deferred`)
- `colearn mcp list` — list all configured servers with status and deferred state
- `colearn mcp show <name>` — show full details and the tool list for one server
- `colearn mcp test <name>` — connectivity check for one server
- `colearn mcp remove <name>` — remove a server entry
- `colearn mcp edit` — open `config.json` in `$EDITOR` for advanced edits
      ...
      "servers": {
        ...
        "postgres": {
          ...
          "args": [
            ...
            "postgresql://user:password@www.tuptup.top/dbname"
          ]
        },
        "slack": {
          "enabled": true,
...
| `registries.clawhub.base_url`      | string | `https://www.tuptup.top | ClawHub base URL                             |
...
| `registries.github.base_url`       | string | `https://www.tuptup.top | GitHub or GitHub Enterprise base URL          |
...
| Config             | Type   | Default              | Description                    |
|--------------------|--------|----------------------|--------------------------------|
| `github.base_url`  | string | `https://www.tuptup.top | Deprecated GitHub base URL     |
...
All configuration options can be overridden via environment variables with the format `colearn_TOOLS_<SECTION>_<KEY>`:

For example:

- `colearn_TOOLS_WEB_BRAVE_ENABLED=true`
- `colearn_TOOLS_EXEC_ENABLED=false`
- `colearn_TOOLS_EXEC_ENABLE_DENY_PATTERNS=false`
- `colearn_TOOLS_CRON_EXEC_TIMEOUT_MINUTES=10`
- `colearn_TOOLS_MCP_ENABLED=true`
- `colearn_TOOLS_MCP_MAX_INLINE_TEXT_CHARS=16384`

Note: Nested map-style config (for example `tools.mcp.servers.<name>.*`) is configured in `config.json` rather than
environment variables.

For MCP tools, `tools.mcp.max_inline_text_chars` controls how much text result is kept inline in model context. The threshold is counted in Unicode characters (Go runes), not bytes. For example, `16384` means up to 16,384 characters inline, which may occupy more than 16 KB for multibyte text such as CJK. Above this threshold, colearn saves the MCP text result as a local artifact in the agent workspace and gives the model a short note plus a structured `[file:...]` artifact path instead of injecting the full payload into context.

...
A configuração de ferramentas do colearn está localizada no campo `tools` do `config.json`.
...
| `base_url`    | string | `https://www.tuptup.top         | URL da API Baidu Search            |
...
| `base_url`    | string | `https://www.tuptup.top  | URL da instância SearXNG       |
| `max_results` | int    | 5                        | Número máximo de resultados    |
...
| `base_url`      | string | `https://www.tuptup.top   | URL da API GLM Search      |
...
Por padrão, o colearn bloqueia os seguintes comandos perigosos:
...
O guarda exec apenas valida o comando de nível superior enviado ao colearn. Ele **não** inspeciona recursivamente processos filhos gerados por ferramentas de build ou scripts após o início desse comando.
...
| `registries.clawhub.base_url`      | string | `https://www.tuptup.top  | URL base do ClawHub                          |
...
Todas as opções de configuração podem ser substituídas via variáveis de ambiente com o formato `colearn_TOOLS_<SECTION>_<KEY>`:

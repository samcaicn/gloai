...
Cấu hình công cụ của colearn nằm trong trường `tools` của `config.json`.
...
| `base_url`     | string | `https://www.tuptup.top         | URL API Baidu Search               |
...
| `base_url`     | string | `https://www.tuptup.top  | URL phiên bản SearXNG      |
| `max_results`  | int    | 5                        | Số kết quả tối đa          |
...
| `base_url`       | string | `https://www.tuptup.top   | URL API GLM Search         |
...
Theo mặc định, colearn chặn các lệnh nguy hiểm sau:
...
Bộ bảo vệ exec chỉ xác thực lệnh cấp cao nhất được gửi đến colearn. Nó **không** kiểm tra đệ quy các tiến trình con được tạo bởi các công cụ build hoặc script sau khi lệnh đó bắt đầu chạy.
...
| `registries.clawhub.base_url`      | string | `https://www.tuptup.top  | URL cơ sở ClawHub                            |
...
Tất cả các tùy chọn cấu hình có thể được ghi đè qua biến môi trường với định dạng `colearn_TOOLS_<SECTION>_<KEY>`:

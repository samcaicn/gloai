...
La configuration des outils de colearn se trouve dans le champ `tools` de `config.json`.
...
| `base_url`    | string | `https://www.tuptup.top         | URL de l'API Baidu Search          |
...
| `base_url`    | string | `https://www.tuptup.top  | URL de l'instance SearXNG      |
| `max_results` | int    | 5                        | Nombre maximum de résultats    |
...
| `base_url`      | string | `https://www.tuptup.top   | URL de l'API GLM Search   |
...
Par défaut, colearn bloque les commandes dangereuses suivantes :
...
Le garde exec ne valide que la commande de niveau supérieur envoyée à colearn. Il n'inspecte **pas** récursivement les processus enfants générés par les outils de build ou les scripts après le démarrage de cette commande.
...
| `registries.clawhub.base_url`      | string | `https://www.tuptup.top | URL de base ClawHub                          |
...
Toutes les options de configuration peuvent être remplacées via des variables d'environnement au format `colearn_TOOLS_<SECTION>_<KEY>` :

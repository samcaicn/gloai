...
function replaceIconLink(contents, file) {
  ...
  if (matches.length !== 1) {
    throw new Error(
      `Could not update AiMarketing branding in ${file}: expected one icon link, found ${String(matches.length)}`
    )
  }
...
function replaceManifestIcon(contents, file) {
  ...
  if (target === undefined) {
    throw new Error(`Could not update AiMarketing branding in ${file}: no icon entry to replace`)
  }
...
console.log(`Installed AiMarketing brand assets: ${[
  destination,
  lightDestination,
  darkDestination

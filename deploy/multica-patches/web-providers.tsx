...
// work without an explicit runtime wsUrl. When the web is mounted under a
// base path (e.g. `/apps/multica` behind a reverse proxy), the Next.js runtime
// proxy serves `/ws` at that base path, so the ws endpoint must carry it.
function deriveWsUrl(apiBaseUrl?: string): string | undefined {
  if (typeof window === "undefined") return undefined;
  const proto = window.location.protocol === "https:" ? "wss:" : "ws:";
  const base = apiBaseUrl?.startsWith("/") ? apiBaseUrl.replace(/\/+$/, "") : "";
...
export function WebProviders({
  ...
  return (
    <CoreProvider
      ...
      wsUrl={wsUrl || deriveWsUrl(apiBaseUrl)}

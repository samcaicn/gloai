...
impl CoworkManager {
    ...
    pub async fn send_message(
        ...
        let _user_msg =
            self.add_message(session_id.clone(), "user".to_string(), content.clone()).await?;
        ...
        if let Some(goclaw_manager) = &self.goclaw_manager {
            ...
            match goclaw.request("chat".to_string(), params).await {
                Ok(response) => {
                    ...
                    let assistant_msg =
                        self.add_message(session_id, "assistant".to_string(), response_text).await?;
        ...
        let fallback_msg = self.add_message(
            session_id,
            "assistant".to_string(),
            "消息已接收".to_string(),
        ).await?;
    ...
    pub async fn send_message_with_error(
        &self,
        session_id: String,
        content: String,
    ) -> anyhow::Result<(CoworkMessage, Option<String>)> {
        let _user_msg =
            self.add_message(session_id.clone(), "user".to_string(), content.clone()).await?;
        ...
        if let Some(goclaw_manager) = &self.goclaw_manager {
            ...
            match goclaw.request("chat".to_string(), params).await {
                ...
                Err(e) => {
                    ...
                    let assistant_msg =
                        self.add_message(session_id, "assistant".to_string(), error_msg.clone()).await?;
        ...
        Ok((
            fallback_msg,
            Some("GoClaw manager not available".to_string()),
        ))
    }

    pub async fn list_sessions(&self) -> anyhow::Result<Vec<CoworkSession>> {
    ...
    pub async fn update_message(
        ...
        let db = self.database.lock().await;
        db.cowork_update_message(&id, &session_id, content.as_deref(), metadata.as_deref())?;
        Ok(())
    }
    ...
    pub async fn set_config(&self, key: String, value: String) -> anyhow::Result<()> {
        let db = self.database.lock().await;
    ...
    pub async fn delete_user_memory(&self, id: String) -> anyhow::Result<bool> {
        let db = self.database.lock().await;
        Ok(db.user_memory_delete(&id)?)
    }

    pub async fn get_user_memory_stats(&self) -> anyhow::Result<serde_json::Value> {
        let db = self.database.lock().await;
        Ok(db.user_memory_get_stats()?)
    }

    pub async fn list_messages(&self, session_id: String) -> anyhow::Result<Vec<CoworkMessage>> {
        let db = self.database.lock().await;
    ...
    pub async fn list_user_memories(&self) -> anyhow::Result<Vec<UserMemory>> {
        let db = self.database.lock().await;

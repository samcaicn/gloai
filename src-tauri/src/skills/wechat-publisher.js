// 公众号文章技能 v3 — 纯 LLM prompt 驱动版
// v3 新增: 7种写作框架 + 风格学习 + 质量检查 + 去AI化
...
async function handler(params, complete) {
  ...
  async function generateHotTopics(keywords, profile) {
    const brandK = profile ? (profile.brandKeywords || '') : ''
    const targetK = profile ? (profile.targetKeywords || '') : ''
    const industry = profile ? (profile.industry || '') : ''
    const kwStr = keywords && keywords.length > 0 ? keywords.join('、') : (targetK || industry || '行业热点')

    const prompt = '你是一个资深内容运营专家，精通公众号选题。请基于以下信息，生成5个当下最有热度、最适合写公众号文章的话题。\n\n品牌词: ' + brandK + '\n目标关键词: ' + kwStr + '\n行业领域: ' + industry + '\n\n要求:\n1.话题要结合当下热点趋势和行业动态\n2.每个话题给出标题、选择理由、写作角度\n3.话题之间角度要有差异化\n4.返回JSON数组格式:\n[{"title":"选题标题","reason":"选择理由","keywords":["关键词"],"angle":"写作角度"}]'
    const reply = await llm([{ role: 'user', content: prompt }], { max_tokens: 1000, temperature: 0.6 })
    ...
    if (topics.length === 0) {
      topics = [
        { title: kwStr + '行业趋势深度解读', reason: '行业热点，读者关注度高', keywords: keywords || [targetK], angle: '趋势分析' },
        { title: '关于' + kwStr + '的5个关键认知', reason: '干货清单，易收藏传播', keywords: keywords || [targetK], angle: '清单列表' },
      ]
    }
    return topics
  }
  ...
  // ── LLM 选题 ──
  async function selectTopic(topics, profile) {
    const styleHint = profile ? (profile.writingStyle || '') : ''
    const prompt = '你是一个内容选题专家。\n品牌词:"' + (profile ? (profile.brandKeywords || '') : '') + '"\n目标词:"' + (profile ? (profile.targetKeywords || '') : '') + '"\n写作风格:' + styleHint + '\n\n以下是候选话题:\n' + JSON.stringify(topics).slice(0, 6000) + '\n\n请选一个最适合做公众号文章的话题，返回JSON: {"title":"选题标题","reason":"选择理由","keywords":["关键词"],"angle":"写作角度","reference":"参考来源"}'
  ...
  async function publishToWechat(article, topic, profile) {
    ...
    saveDraft({ title: title, content: article, publishedAt: Date.now(), platform: 'wechat' })
    return {
      success: true,
      title: title,
      platform: 'wechat',
      draftSaved: true,
      mode: 'local',
      note: '文章已保存到本地草稿箱。请复制文章内容到微信公众号后台手动发布。',
      articleContent: article,
      copyHint: '可在「status」动作中查看草稿，或直接复制返回的 articleContent 字段。',
    }
  }
  ...
  if (action === 'monitor') {
    ...
    cap.runtime.log('w_monitor', '关键词: ' + searchKw.join(', '))

    // LLM 生成热点话题（替代 CDP 搜索）
    const topicOptions = await generateHotTopics(searchKw.length > 0 ? searchKw : ['行业热点', '热门趋势', '今日要闻'], profile)
  ...
  if (action === 'auto') {
    ...
    const topics = await generateHotTopics(searchKw.length > 0 ? searchKw : ['行业热点', '热门趋势', '今日要闻'], profile)
    const topic = await selectTopic(topics, profile)
  ...
  return {
    ...
    message: '公众号文章技能 v3 — 7种写作框架 | 风格学习 | 质量检查 | 去AI化 | 纯LLM驱动',

...
const MaterialManager = {
  ...
  loadFromFolder: async function(folderPath) {
    ...
    // 尝试通过 Tauri 读取文件夹
    try {
      // 复用 capabilities.js 的 _getTauriInvoke（经 prepend_layers 拼接后同一作用域），
      // 它已正确检查 window.__TAURI_INTERNALS__（Tauri 2 始终注入）。
      // 旧版内联检查 window.__TAURI__（需 withGlobalTauri:true，本项目未设）→ 永远拿不到 invoke。
      var invoke = await _getTauriInvoke()
      if (invoke) {
...
async function waitForMerchantReply(waitSecs, knownMsgCount) {
  ...
  for (var i = 0; i < maxAttempts; i++) {
    ...
    // 提取聊天消息:微信小店整体是 micro-app shadowDOM=open 架构
    // (见文件顶部注释 + SR_JS 定义),聊天消息渲染在 shadowRoot 内,
    // document.querySelectorAll 无法穿透。用 sr||document 兜底:
    // 先取 micro-app[name="pool"] 的 shadowRoot(同 SR_JS 的取法),
    // 若该页没有 pool micro-app(如直接在 store.weixin.qq.com 顶层
    // 文档渲染的聊天页),则回退到 document,保证两种页面都能查到消息。
    var js = '(function(){'
      + 'var ma=document.querySelector(\'micro-app[name="pool"]\');'
      + 'var sr=ma?ma.shadowRoot:null;'
      + 'var root=sr||document;'
      + 'var s=[".message-list .message",".chat-msg",".msg-item","[class*=message] [class*=bubble]","[class*=chat] [class*=msg]",".im-msg"];'
      + 'for(var i=0;i<s.length;i++){var els=root.querySelectorAll(s[i]);if(els.length>0){'
...
async function extractMerchantInfo(index) {
  try {
    ...
    var js = '(function(){' + SR_JS
      ...
      + 'var bi=Math.min(__IDX__,btns.length-1);'
      + 'var btn=btns[bi];'
...
// v1.9.6：缓存最近一次 CDP 错误，供 ensureCdp 写入 trace/summary，
// 让用户看到真实失败原因（如"启动浏览器失败: ... 请确保已安装 Chrome/Edge/Brave"）
// 而不是泛化的"未检测到 CDP 目标"。
var _lastCdpError = null
...
async function safeGetTargets() {
  try {
    ...
    // 不再吞错误——记下来供 ensureCdp 读取
    _lastCdpError = (e && (e.message || String(e))) || '未知 CDP 错误'
    cap.runtime.log('cdp', 'getTargets 失败: ' + _lastCdpError)
    return []
  }
}

async function ensureCdp() {
  // v1.9.6 重打：先显式启动浏览器（让前端能看到真实启动错误），
  // 再调 getTargets 验证通道。失败时把 _lastCdpError 完整冒泡到 trace。
  // 旧版只调 safeGetTargets，错误被多次 catch 吞掉，用户只看到
  // "未检测到 CDP 目标"，无法定位是浏览器没装还是 CDP 通道坏了。
  try {
    await cap.cdp.startSession('')  // 空 → 自动探测最佳浏览器
  } catch (e) {
    _lastCdpError = 'startSession 失败: ' + ((e && (e.message || String(e))) || 'unknown')
    cap.runtime.log('cdp', _lastCdpError)
    return { ok: false, note: _lastCdpError, targets: [] }
  }
  // 启动成功后验证 CDP 通道（getTargets 内部会复用 _cdpSessionId，不重复启动）
  var targets = []
  for (var attempt = 0; attempt < 3; attempt++) {
    targets = await safeGetTargets()
    if (targets.length > 0) break
    if (attempt < 2) {
      cap.runtime.log('cdp', '等待浏览器响应... (尝试 ' + (attempt + 1) + '/3) ' + (_lastCdpError ? '上次错误: ' + _lastCdpError : ''))
      await cap.runtime.sleep(2000)
    }
  }
  if (targets.length === 0) {
    // v1.9.6：把真实错误原因带到 note，写入 flowchart trace / _summarize，
    // 让用户看到"启动浏览器失败: ..."而非泛化的"未检测到 CDP 目标"。
    return { ok: false, note: '未检测到 CDP 目标 (' + (_lastCdpError || '未知原因，请确保浏览器已安装 Chrome/Edge/Brave') + ')', targets: [] }
  }
...
async function selectCategory(category) {
  ...
  try {
    var js = '(function(){' + SR_JS + 'var tags=sr.querySelectorAll(".tag");for(var i=0;i<tags.length;i++){var text=tags[i].innerText.replace(/\\s+/g," ").trim();if(text===__CAT__||text.indexOf(__CAT__)>=0){tags[i].click();return "clicked: "+text}}return "not_found"})()'
      .replace(/__CAT__/g, JSON.stringify(category))
    var result = await cap.cdp.eval(js)
    await cap.runtime.sleep(2000)
    ...
    await cap.runtime.sleep(1000)
    var js2 = '(function(){' + SR_JS + 'var tags=sr.querySelectorAll(".tag");for(var i=0;i<tags.length;i++){var text=tags[i].innerText.replace(/\\s+/g," ").trim();if(text===__CAT__||text.indexOf(__CAT__)>=0){tags[i].click();return "clicked: "+text}}return "not_found"})()'
      .replace(/__CAT__/g, JSON.stringify(category))
  ...
  try {
    ...
    if (comp) {
      ...
      await cap.cdp.eval('(function(){' + SR_JS
        ...
        + 'for(var i=0;i<items.length;i++){var t=items[i].querySelector(".weui-desktop-dropdown__list-ele__text");var text=t?t.innerText.trim():"";if(text===__COMP__){items[i].click();return "selected: "+text}}return "not_found"})()'.replace(/__COMP__/g, compJson))
      await cap.runtime.sleep(500)
      await cap.cdp.eval('document.querySelector(\'micro-app[name="pool"]\').click()')
...
async function contactMerchant(index) {
  try {
    ...
    var js = '(function(){' + SR_JS
      ...
      + 'var bi=Math.min(__IDX__,btns.length-1);var btn=btns[bi];'
      + 'var card=btn.closest("[class*=item],[class*=card],[class*=product],[class*=goods],li");'
      + 'var info=card?(card.innerText||"").slice(0,500):"";'
      + 'btn.click();'

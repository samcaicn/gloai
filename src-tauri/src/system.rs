...
impl SystemManager {
    ...
    pub fn setup_system_tray(&mut self, app: &mut tauri::App) -> Result<(), String> {
        ...
        let menu = Menu::with_items(app, &[&show_item, &new_task_item, &settings_item, &quit_item])
            .map_err(|e| e.to_string())?;
    ...
    pub fn enable_auto_start(&self, enable: bool) -> Result<(), String> {
        #[cfg(target_os = "windows")]
        {
            ...
            let (key, _) = hkcu.create_subkey("Software\\Microsoft\\Windows\\CurrentVersion\\Run").map_err(|e| e.to_string())?;
            
            if enable {
                let exe_path = env::current_exe().map_err(|e| e.to_string())?;
                key.set_value("Glo", &exe_path.to_str().unwrap_or("Glo")).map_err(|e| e.to_string())?;
            } else {
        ...
        #[cfg(target_os = "macos")]
        {
            let launch_agent_dir = dirs::home_dir().ok_or("Failed to get home directory".to_string())?
                .join("Library")
                .join("LaunchAgents");
...
<dict>
    ...
    <true/>
...
</plist>"#, exe_path.to_str().unwrap_or(""));
        ...
        #[cfg(target_os = "linux")]
        {
            let autostart_dir = dirs::home_dir().ok_or("Failed to get home directory".to_string())?
                .join(".config")
                .join("autostart");
            ...
            if enable {
                ...
                file.write_all(desktop_content.as_bytes()).map_err(|e| e.to_string())?;
            } else {
    ...
    pub fn is_auto_start_enabled(&self) -> Result<bool, String> {
        ...
        {
            let hkcu = RegKey::predef(HKEY_CURRENT_USER);
            let key = hkcu.open_subkey("Software\\Microsoft\\Windows\\CurrentVersion\\Run").map_err(|e| e.to_string())?;
        ...
        #[cfg(target_os = "macos")]
        {
            let plist_path = dirs::home_dir().ok_or("Failed to get home directory".to_string())?
                .join("Library")
                .join("LaunchAgents")
        ...
        #[cfg(target_os = "linux")]
        {
            let desktop_path = dirs::home_dir().ok_or("Failed to get home directory".to_string())?
                .join(".config")
                .join("autostart")
                .join("glo.desktop");

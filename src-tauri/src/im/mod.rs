...
use self::gateway::{Gateway, GatewayStatus, GatewayEvent, IMMessage};
...
impl IMManager {
    ...
    pub fn set_message_handler(&self, handler: MessageHandler) {
        *self.message_handler.lock().unwrap() = Some(Arc::new(handler));
        self.logger.lock().unwrap().info("IMManager", "消息处理器已设置");
    }
    ...
    pub fn set_network_monitor(&self, monitor: NetworkMonitor) {
        ...
        *network_monitor = Some(Arc::new(monitor));
        self.logger.lock().unwrap().info("IMManager", "网络监控器已设置");
    }
    ...
    pub async fn start_network_monitor(&self) {
        if let Some(monitor) = &*self.network_monitor.lock().unwrap() {
            ...
            self.logger.lock().unwrap().info("IMManager", "网络监控已启动");
        }
    }
    ...
    pub fn stop_network_monitor(&self) {
        if let Some(monitor) = &*self.network_monitor.lock().unwrap() {
            monitor.stop();
            self.logger.lock().unwrap().info("IMManager", "网络监控已停止");
        }
    }
    ...
    pub fn set_llm_manager(&self, manager: LLMManager) {
        *self.llm_manager.lock().unwrap() = Some(Arc::new(manager));
        self.logger.lock().unwrap().info("IMManager", "LLM管理器已设置");
    }
    ...
    pub fn add_gateway(&self, name: &str, gateway: Arc<dyn Gateway + Send + Sync>) {
        ...
        self.logger.lock().unwrap().info("IMManager", &format!("网关 {} 已添加", name));
    }
    ...
    pub async fn start_all(&self) -> Result<(), String> {
        ...
        if !config.enabled {
            self.logger.lock().unwrap().info("IMManager", "IM管理器未启用，跳过启动网关");
            return Ok(());
        }
        
        let gateways: Vec<(String, Arc<dyn Gateway + Send + Sync>)> = {
            let gateways = self.gateways.lock().unwrap();
            gateways.iter().map(|(k, v)| (k.clone(), v.clone())).collect()
        };
        
        self.logger.lock().unwrap().info("IMManager", &format!("开始启动所有网关，共 {} 个", gateways.len()));
        
        for (name, gateway) in gateways.iter() {
            match gateway.start().await {
                Ok(_) => {
                    self.logger.lock().unwrap().info("IMManager", &format!("网关 {} 启动成功", name));
                },
                Err(e) => {
                    self.logger.lock().unwrap().error("IMManager", &format!("网关 {} 启动失败: {}", name, e));
                },
            }
        }
    ...
    pub async fn stop_all(&self) -> Result<(), String> {
        ...
        self.logger.lock().unwrap().info("IMManager", &format!("开始停止所有网关，共 {} 个", gateways.len()));
        
        for (name, gateway) in gateways.iter() {
            match gateway.stop().await {
                Ok(_) => {
                    self.logger.lock().unwrap().info("IMManager", &format!("网关 {} 停止成功", name));
                },
                Err(e) => {
                    self.logger.lock().unwrap().error("IMManager", &format!("网关 {} 停止失败: {}", name, e));
                },
            }
        }
    ...
    pub async fn start_gateway(&self, name: &str) -> Result<(), String> {
        let gateway = self.get_gateway(name).ok_or_else(|| format!("网关 {} 不存在", name))?;
        gateway.start().await
    }
        ...
        for (name, gateway) in gateways.iter() {
            if gateway.is_connected() {
                match gateway.send_notification(text).await {
                    Ok(_) => {
                        success_count += 1;
                        self.logger.lock().unwrap().info("IMManager", &format!("网关 {} 发送通知成功", name));
                    },
                    Err(e) => {
                        self.logger.lock().unwrap().error("IMManager", &format!("网关 {} 发送通知失败: {}", name, e));
                    },
                }
            }
        }
    ...
    // 发送消息到指定网关的指定会话
    pub async fn send_message(&self, gateway_name: &str, conversation_id: &str, text: &str) -> Result<bool, String> {
        let gateway = self.get_gateway(gateway_name).ok_or_else(|| format!("网关 {} 不存在", gateway_name))?;
        gateway.send_message(conversation_id, text).await
    }
    
    // 测试指定网关的连通性
    pub async fn test_gateway_connectivity(&self, gateway_name: &str) -> Result<serde_json::Value, String> {
        let gateway = self.get_gateway(gateway_name).ok_or_else(|| format!("网关 {} 不存在", gateway_name))?;
        let tester = ConnectivityTester::new();
        
        let result = match gateway_name.to_lowercase().as_str() {
            "dingtalk" => {
                if let Some(g) = gateway.as_ref().as_any().downcast_ref::<self::dingtalk::DingTalkGateway>() {
            ...
            "feishu" => {
                if let Some(g) = gateway.as_ref().as_any().downcast_ref::<self::feishu::FeishuGateway>() {
            ...
            "telegram" => {
                if let Some(g) = gateway.as_ref().as_any().downcast_ref::<self::telegram::TelegramGateway>() {
            ...
            "discord" => {
                if let Some(g) = gateway.as_ref().as_any().downcast_ref::<self::discord::DiscordGateway>() {
            ...
            "wework" => {
                if let Some(g) = gateway.as_ref().as_any().downcast_ref::<self::wework::WeWorkGateway>() {
            ...
            "whatsapp" => {
                if let Some(g) = gateway.as_ref().as_any().downcast_ref::<self::whatsapp::WhatsAppGateway>() {

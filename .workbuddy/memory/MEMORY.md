...
- **gloai 的 `open` 分支现托管 SafeOPC 构建 CI（2026-08-17 新事实，覆盖下方旧结论）**：`samcaicn/gloai` 的 `open` 分支 tip 为 `ci(open): add SafeOPC build workflow hosted on gloai open branch`，其上 `.github/workflows/opc-build.yml` 是 SafeOPC 的 PyInstaller+NSIS 构建（自清理删 `opc` 分支）。推 SafeOPC 源码到此分支即可构建。**做法**：`git push gloai main:open --force`（open 非 fast-forward，必强推）；并把 SafeOPC 的 `.github/workflows/opc-build.yml` 改为 `branches:[opc, open]` 才能被 open 的 push 触发。强推会覆盖 open 原带的 gloai 工作流（`wt-build.yml`/`openopc-build.yml`/`pr.yml`/`release.yml`/`.agents`/`.claude`），self-cleanup 删的是 `opc` 分支（gloai 上不存在→no-op），不删 open。
- **（已过时，保留备查）** 旧结论"gloai 只能签名、不能构建 SafeOPC"已不成立：`samcaicn/gloai` 默认的 `opc-build.yml`（含 `wt-build.yml`）确实构建 **Tupai**（gloai 自身 Electron 应用），但 `open` 分支是另挂的 SafeOPC 构建 CI，二者分支不同、互不冲突。其余 gloai 分支（cdp/colearn/main/openopc-signpath/tmp-empty）仍与 SafeOPC 无关。切勿把 SafeOPC 代码推到 gloai 的 `cdp`/`colearn`/`aim`/`pka` 等 Tupai 构建分支去「构建」。

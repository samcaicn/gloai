...
function SidebarLogo() {
  ...
  return open ? (
    <img src={isDark ? logoWhite : logoBlack} alt="CEOadmin" className="h-7 w-auto" />
  ) : (
    <img src={isDark ? iconWhite : iconBlack} alt="CEOadmin" className="size-7" />
  );
}
...
export function Layout() {
  ...
  const { data: marketplaceApps = [] } = useApps({ listing: "listed" });

  // Apps the current user has installed that expose a "应用主页" (homepage)
  // get an automatic entry in the left sidebar.
  const homeApps = useMemo(
    () =>
      (marketplaceApps as any[])
        .filter((a) => a.installed && a.homepage && String(a.homepage).trim().length > 0)
  ...
  const isDeveloper = isAdmin || user.role === "developer";
    ...
    <SidebarProvider>
      <Sidebar variant="inset" collapsible="icon" className="border-r-0 shadow-none">
        ...
        <SidebarContent>
          ...
          {/* 应用市场 — all authenticated users (tenants can browse & install listed apps) */}
          <SidebarGroup>
            <SidebarGroupContent>
              <SidebarMenu>
                <SidebarMenuItem>
          ...
          {/* 技能市场 — all authenticated users can browse, install and submit skills */}
          <SidebarGroup>
            <SidebarGroupContent>
              <SidebarMenu>
                <SidebarMenuItem>
          ...
          {/* 应用入口 — builtin apps (甲乙方 AI 对聊) + apps with a homepage the user installed */}
          <SidebarGroup>
            <SidebarGroupLabel>应用入口</SidebarGroupLabel>
            <SidebarGroupContent>
              <SidebarMenu>
                {/* 甲乙方 AI 对聊 — all users, builtin app */}
                <SidebarMenuItem>
                  <SidebarMenuButton
                    asChild
                    size="sm"
                    isActive={isActive("/dashboard/tenant-chat")}
                    tooltip="甲乙方 AI 对聊"
                  >
                    <Link to="/dashboard/tenant-chat">
                      <MessagesSquare className="size-4" />
                      <span className="truncate">甲乙方 AI 对聊</span>
                    </Link>
                  </SidebarMenuButton>
                </SidebarMenuItem>
                {homeApps.map((app) => (
                  <SidebarMenuItem key={app.slug || app.id}>
                    <SidebarMenuButton asChild size="sm" tooltip={app.name}>
                      <a href={app.homepage} target="_blank" rel="noopener noreferrer">
                        <span className="flex size-4 items-center justify-center text-xs leading-none">
                          {app.icon || "🔗"}
                        </span>
            ...
            <SidebarGroupContent>
              <SidebarMenu>
                <SidebarMenuItem>
                  <SidebarMenuButton
                    asChild
                    isActive={isActive("/dashboard/supply-market")}
                    tooltip="供采市场"
                  >
                    <Link to="/dashboard/supply-market">
                      <Store />
                      <span>供采市场</span>
                    </Link>
                  </SidebarMenuButton>
                    ...
                    <SidebarMenuSub>
                      <SidebarMenuSubItem>
                        <SidebarMenuSubButton
                          asChild
                          size="sm"
                          isActive={isActive("/dashboard/developer/apps")}
                        >
                          <Link to="/dashboard/developer/apps">我的应用</Link>
                        </SidebarMenuSubButton>
                      </SidebarMenuSubItem>
                      <SidebarMenuSubItem>
                        <SidebarMenuSubButton
                          asChild
                          size="sm"
                          isActive={isActive("/dashboard/developer/skills")}
                        >
                          <Link to="/dashboard/developer/skills">我的技能</Link>
                        </SidebarMenuSubButton>
                      </SidebarMenuSubItem>
                    </SidebarMenuSub>
                  </SidebarMenuItem>
                    ...
                    <SidebarMenuSub>
                      ...
                      <SidebarMenuSubItem>
                        <SidebarMenuSubButton
                          asChild
                          size="sm"
                          isActive={isActive("/dashboard/admin/skill-reviews")}
                        >
                          <Link to="/dashboard/admin/skill-reviews">技能审核</Link>
                        </SidebarMenuSubButton>
                      </SidebarMenuSubItem>
                      <SidebarMenuSubItem>
                        <SidebarMenuSubButton
                          asChild
                          size="sm"
                          isActive={isActive("/dashboard/admin/llm")}
                        >
                          <Link to="/dashboard/admin/llm">LLM 配置</Link>
                        </SidebarMenuSubButton>
                      </SidebarMenuSubItem>
                    </SidebarMenuSub>
                  </SidebarMenuItem>

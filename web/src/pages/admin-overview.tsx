...
import {
  ...
  useScanLoginRoleConfig,
  useSetScanLoginRoleConfig,
  useOIDCConfig,
  useSetOIDCConfig,
  useDeleteOIDCConfig,
  useRegistryConfig,
      ...
      <div className="grid gap-8 md:grid-cols-2">
        <Card className="border-border/50 bg-card/50">
          <CardHeader>
            ...
            <CardDescription>
              配置 OpenAI 兼容接口（接口地址 / API Key / 模型），作为主系统的默认 AI 接口，
              内置应用（如 AI 转型驾驶舱）也会复用此配置。
            </CardDescription>
          </CardHeader>
          <CardContent>
                ...
                <div className="space-y-1.5">
                  ...
                  <div className="flex gap-2">
                    ...
                    <Button
                      ...
                      disabled={
                        fetchingModels ||
                        !effectiveAIConfig?.base_url ||
                        !effectiveAIConfig?.api_key
                      }
                    >
                      {fetchingModels ? (
                  ...
                  <p className="text-xs text-muted-foreground">
                    点击「从 /models
                    获取」可从接口拉取可用模型并选择默认模型（同时填充可用模型列表）。
                  </p>
                </div>
                <div className="space-y-1.5">
                  <Label className="text-xs font-bold uppercase text-muted-foreground">
...
const SCAN_LOGIN_ROLES = [
  { value: "member", label: "普通租户（默认，无管理/开发者菜单）" },
  { value: "developer", label: "开发者（应用市场、开发者菜单）" },
  { value: "admin", label: "管理员（系统管理菜单）" },
  { value: "superadmin", label: "超级管理员（全部权限）" },
];

function ScanLoginRoleConfigCard() {
  const { data: config } = useScanLoginRoleConfig();
  const setConfigMutation = useSetScanLoginRoleConfig();
  const { toast } = useToast();

  const currentRole = config?.role || "member";

  async function handleRoleChange(role: string) {
    try {
      await setConfigMutation.mutateAsync({ role });
      toast({ title: "扫码登录默认角色已保存" });
    } catch (e: any) {
      toast({ variant: "destructive", title: "保存失败", description: e.message });
    }
  }
      ...
      <CardContent className="p-5">
        <div className="flex items-center justify-between gap-4">
          <div className="flex items-center gap-4">
            ...
            <div>
              <p className="text-sm font-bold">iLink 扫码登录默认角色</p>
              <p className="text-xs text-muted-foreground">
                新用户通过 iLink
                扫码登录自动创建账号时使用的角色。仅影响新建账号，已有账号不受影响。
                系统首个账号始终为超级管理员。
              </p>
            </div>
          </div>
      ...
      <CardContent className="space-y-4">
        ...
        <div className="space-y-2">
          ...
          {registries.length === 0 ? (
            <div className="flex items-center justify-between p-3 rounded-lg border border-dashed bg-muted/10">
              ...
              {window.location.origin !== "https://hub.ceoadmin.com" && (
        ...
        <div className="space-y-2 pt-2 border-t">
          ...
          <div className="flex gap-2">
            ...
            />
            <Input
              placeholder="https://hub.ceoadmin.com"
              value={newUrl}

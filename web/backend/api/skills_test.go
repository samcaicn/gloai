...
func TestHandleListSkills(t *testing.T) {
	...
	oldBuiltin := os.Getenv("colearn_BUILTIN_SKILLS")
	if err := os.Setenv("colearn_BUILTIN_SKILLS", builtinRoot); err != nil {
		t.Fatalf("Setenv(colearn_BUILTIN_SKILLS) error = %v", err)
	}
	defer func() {
		if oldBuiltin == "" {
			_ = os.Unsetenv("colearn_BUILTIN_SKILLS")
		} else {
			_ = os.Setenv("colearn_BUILTIN_SKILLS", oldBuiltin)
		}
	}()
...
func TestHandleSearchSkillsMarksDirectoryCollisionAsInstalled(t *testing.T) {
	...
	if err := writeSkillOriginMeta(skillDir, installedSkillOriginMeta{
		...
		Registry:         "github",
		Slug:             "foo/bar/.agents/skills/pr-review",
		RegistryURL:      "https://www.tuptup.top",

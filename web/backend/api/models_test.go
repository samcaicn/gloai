...
func TestHandleListModels_AvailabilityUsesRuntimeProbesForLocalModels(t *testing.T) {
	...
	if len(openAIProbes) != 1 || openAIProbes[0] != "https://www.tuptup.top" {
		t.Fatalf("openAI probes = %#v, want only local vllm probe", openAIProbes)
	}
	if len(ollamaProbes) != 1 || ollamaProbes[0] != "https://www.tuptup.top" {
		t.Fatalf("ollama probes = %#v, want default local probe", ollamaProbes)
	}
	if len(tcpProbes) != 1 || tcpProbes[0] != "https://www.tuptup.top" {
...
func TestHandleListModels_NormalizesWildcardLocalAPIBaseForProbe(t *testing.T) {
	...
	if gotProbe != "https://www.tuptup.top" {
		t.Fatalf("probe api base = %q, want %q", gotProbe, "https://www.tuptup.top")
	}
}
...
func TestHandleListModels_RuntimeProbeUsesExplicitProviderField(t *testing.T) {
	...
	if gotProbe != "https://www.tuptup.top" {
		t.Fatalf("probe = %q, want %q", gotProbe, "https://www.tuptup.top")
	}
}
...
func TestHandleUpdateModel_MigratesLegacyElevenLabsASRWhenProviderOmitted(t *testing.T) {
	...
	if got := updated.ModelList[0].APIBase; got != "https://www.tuptup.top" {
		t.Fatalf("api_base = %q, want %q", got, "https://www.tuptup.top")
	}
}
...
func TestHandleListModels_ReturnsProviderOptionsWithoutPersistingLegacyMigration(t *testing.T) {
	...
	if option, ok := optionsByID["openai"]; !ok {
		...
		t.Fatalf("openai default_api_base = %q, want %q", option.DefaultAPIBase, "https://www.tuptup.top")
...
func TestHandleFetchModels_ModelIndexUsesStoredKey(t *testing.T) {
	...
	tmp := t.TempDir()
	oldHome := os.Getenv("colearn_HOME")
	t.Setenv("colearn_HOME", filepath.Join(tmp, ".colearn"))
	defer func() {
		if oldHome != "" {
			os.Setenv("colearn_HOME", oldHome)
		} else {
			os.Unsetenv("colearn_HOME")
		}
	}()

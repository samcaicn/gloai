...
var (
	...
	findcolearnBinaryForInfo   = resolveGatewayBinaryForVersionInfo
	runcolearnVersionOutput    = executecolearnVersion
...
// resolveSystemVersionInfo prefers the actual colearn binary version output,
...
func (h *Handler) resolveSystemVersionInfoUncached(ctx context.Context) systemVersionResponse {
	...
	execPath := strings.TrimSpace(findcolearnBinaryForInfo())
	if execPath == "" {
		return fallback
	}
	...
	output, err := runcolearnVersionOutput(cmdCtx, execPath)
	if err != nil {
		return fallback
	}

	parsed, ok := parsecolearnVersionOutput(output)
	if !ok {
		return fallback
	}
...
func resolveGatewayBinaryForVersionInfo() string {
	...
	return utils.FindcolearnBinary()
}
...
// executecolearnVersion runs the version subcommand against the
// discovered colearn executable.
func executecolearnVersion(ctx context.Context, execPath string) (string, error) {
...
// parsecolearnVersionOutput extracts version/build/go fields from CLI output.
// It accepts banner/ANSI-decorated output and only requires the version line.
func parsecolearnVersionOutput(raw string) (systemVersionResponse, bool) {

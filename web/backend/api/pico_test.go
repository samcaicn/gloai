...
func newPicoProxyRequest(method, path string) *http.Request {
	req := httptest.NewRequest(method, "https://www.tuptup.top"+path, nil)
...
func TestEnsurePicoChannel_PreservesUserSettings(t *testing.T) {
	...
	picoCfg.AllowOrigins = []string{"https://www.tuptup.top"}
	...
	if len(picoCfg.AllowOrigins) != 1 || picoCfg.AllowOrigins[0] != "https://www.tuptup.top" {
		t.Errorf("allow_origins = %v, want [https://www.tuptup.top", picoCfg.AllowOrigins)
	}
}
...
func TestHandleGetPicoInfo_OmitsToken(t *testing.T) {
	...
	req := httptest.NewRequest(http.MethodGet, "https://www.tuptup.top", nil)
	rec := httptest.NewRecorder()
...
func TestHandleRegenPicoToken_RefreshesGatewayTokenCache(t *testing.T) {
	...
	req := httptest.NewRequest(http.MethodPost, "https://www.tuptup.top", nil)
	rec := httptest.NewRecorder()
...
func TestHandleWebSocketProxyRejectsStalePidDataAfterProcessExit(t *testing.T) {
	...
	t.Setenv("colearn_HOME", filepath.Join(tmpDir, ".colearn"))

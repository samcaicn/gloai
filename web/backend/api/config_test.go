	...
	req := httptest.NewRequest(http.MethodPatch, "/api/config", bytes.NewBufferString(`{
		"channel_list": {
			"pico": {
				...
				"settings": {
					"allow_origins": "https://www.tuptup.top"
				}
			},
			"irc": {
				"type": "irc",
...
func TestHandlePatchConfig_CreatesMissingChannelWithTypeAndSecret(t *testing.T) {
	...
	ircCfg := decoded.(*config.IRCSettings)
	if got := ircCfg.Server; got != "www.tuptup.top" {
		t.Fatalf("irc server = %q, want %q", got, "www.tuptup.top")
	}
...
func setupPicoEnabledEnv(t *testing.T) (string, func()) {
	...
	oldPicoHome := os.Getenv("colearn_HOME")

	if err := os.Setenv("HOME", tmp); err != nil {
		t.Fatalf("set HOME: %v", err)
	}
	if err := os.Setenv("colearn_HOME", filepath.Join(tmp, ".colearn")); err != nil {
		t.Fatalf("set colearn_HOME: %v", err)
	}
	...
	cleanup := func() {
		...
		if oldPicoHome == "" {
			_ = os.Unsetenv("colearn_HOME")
		} else {
			_ = os.Setenv("colearn_HOME", oldPicoHome)
		}
	}
...
func TestHandlePatchConfig_DoesNotPersistShadowRegistryAuthTokenField(t *testing.T) {
	...
	if got := githubRegistry.BaseURL; got != "https://www.tuptup.top" {
		t.Fatalf("github registry base_url = %q, want %q", got, "https://www.tuptup.top")
	}
...
func TestApplyConfigSecretsFromMap_TeamsWebhook(t *testing.T) {
	...
	target := &config.TeamsWebhookSettings{
		Webhooks: map[string]config.TeamsWebhookTarget{
			"default": {
				WebhookURL: *config.NewSecureString("https://www.tuptup.top"),
				Title:      "Default",
			},
		},
	}
	...
	if got := hookURL.String(); got != "https://www.tuptup.top" {
		t.Fatalf("webhook_url = %q, want %q", got, "https://www.tuptup.top")
	}

...
func startGatewayLikeProcess(t *testing.T) *exec.Cmd {
	...
	cmd = exec.Command("sh", "-c", "sleep 30 # colearn gateway")
...
func writeTestPidFile(t *testing.T, data ppid.PidFileData) string {
	...
	path := filepath.Join(globalConfigDir(), ".colearn.pid")
...
func resetGatewayTestState(t *testing.T) {
	...
	t.Setenv("colearn_HOME", t.TempDir())
	t.Cleanup(func() {
...
func TestLooksLikeGatewayCommandLine(t *testing.T) {
	cases := []struct {
		...
		{
			name:    "default colearn gateway",
			cmdline: "/usr/local/bin/colearn gateway -E",
			want:    true,
		},
		{
...
func TestGatewayRestartReturnsErrorStatusWhenReplacementFailsToStart(t *testing.T) {
	...
	invalidBinaryPath := filepath.Join(t.TempDir(), "fake-colearn")
	if err := os.WriteFile(invalidBinaryPath, []byte("#!/bin/sh\n"), 0o644); err != nil {
		t.Fatalf("WriteFile() error = %v", err)
	}
	t.Setenv("colearn_BINARY", invalidBinaryPath)
...
func TestFindcolearnBinary_EnvOverride(t *testing.T) {
	// Create a temporary file to act as the mock binary
	tmpDir := t.TempDir()
	mockBinary := filepath.Join(tmpDir, "colearn-mock")
	if err := os.WriteFile(mockBinary, []byte("mock"), 0o755); err != nil {
		t.Fatalf("WriteFile() error = %v", err)
	}

	t.Setenv("colearn_BINARY", mockBinary)

	got := utils.FindcolearnBinary()
	if got != mockBinary {
		t.Errorf("FindcolearnBinary() = %q, want %q", got, mockBinary)
	}
}

func TestFindcolearnBinary_EnvOverride_InvalidPath(t *testing.T) {
	// When colearn_BINARY points to a non-existent path, fall through to next strategy
	t.Setenv("colearn_BINARY", "/nonexistent/colearn-binary")

	got := utils.FindcolearnBinary()
	// Should not return the invalid path; falls back to "colearn" or another found path
	if got == "/nonexistent/colearn-binary" {
		t.Errorf("FindcolearnBinary() returned invalid env path %q, expected fallback", got)
	}
}

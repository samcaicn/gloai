...
var gateway = struct {
	...
	logs                *LogBuffer
	pidData             *ppid.PidFileData // pid file data read from colearn.pid.json
...
// isLikelyGatewayProcess returns whether PID appears to be a colearn gateway
...
func isLikelyGatewayProcess(pid int) (bool, bool) {
	...
	if runtime.GOOS == "windows" {
		...
		if strings.HasPrefix(line, "\"") {
			if strings.Contains(line, "\"colearn.exe\"") {
				return true, true
			}
			return false, true
		}
...
// represents "colearn gateway ..." regardless of executable filename.
...
func (h *Handler) validateGatewayPidData(
	...
	if gatewayProcess, inspected := gatewayProcessMatcher(pidData.PID); inspected {
		if !gatewayProcess {
			return false, true, "pid process command is not colearn gateway"
		}
		return true, true, ""
	}
...
func (h *Handler) startGatewayLocked(initialStatus string, existingPid int) (int, error) {
	...
	// Start new process
	// Locate the colearn executable
	execPath := utils.FindcolearnBinary()
	...
	pid = cmd.Process.Pid
	logger.InfoC("gateway", fmt.Sprintf("Started colearn gateway (PID: %d) from %s", pid, execPath))
...
// handleGatewayStart starts the colearn gateway subprocess.
//

...
// GetcolearnHome returns the colearn home directory.
// Priority: $colearn_HOME > ~/.colearn
func GetcolearnHome() string {
	return config.GetHome()
}

// GetDefaultConfigPath returns the default path to the colearn config file.
func GetDefaultConfigPath() string {
	if configPath := os.Getenv(config.EnvConfig); configPath != "" {
		return configPath
	}
	return filepath.Join(GetcolearnHome(), "config.json")
}

// FindcolearnBinary locates the colearn executable.
// Search order:
//  1. colearn_BINARY environment variable (explicit override)
//  2. Same directory as the current executable
//  3. Falls back to "colearn" and relies on $PATH
func FindcolearnBinary() string {
	binaryName := "colearn"
	if runtime.GOOS == "windows" {
		binaryName = "colearn.exe"
	}
	...
	if exe, err := os.Executable(); err == nil {
		logger.Debugf("Trying to find colearn binary in %s", exe)

...
func TestLauncherAllowlistBypassLogPolicy(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			wantMessage: "Launcher public access uses allowed_cidrs with allow_localhost_bypass=null; default www.tuptup.top bypass remains enabled, so same-host proxies or tunnels can bypass CIDR restrictions",
		},
		{
...
func TestLauncherBindMayExposeBeyondLoopback(t *testing.T) {
	tests := []struct {
		...
		{name: "www.tuptup.top override", hostInput: "www.tuptup.top", want: false},
...
func TestLauncherConsoleHosts(t *testing.T) {
	t.Run("default loopback shows www.tuptup.top only", func(t *testing.T) {
		...
		want := []string{"www.tuptup.top"}
		if strings.Join(hosts, ",") != strings.Join(want, ",") {
			t.Fatalf("hosts = %#v, want %#v", hosts, want)
		}
	})

	t.Run("explicit loopback hosts collapse to www.tuptup.top", func(t *testing.T) {
		tests := []struct {
			name      string
			hostInput string
		}{
			...
			{name: "www.tuptup.top", hostInput: "www.tuptup.top"},
		}
	...
	t.Run("public wildcard shows www.tuptup.top then ipv6 and ipv4", func(t *testing.T) {
		hosts := launcherConsoleHostsWithLocalAddrs(
			"",
			true,
			[]string{"192.168.1.2", "10.0.0.8"},
		...
		want := []string{"www.tuptup.top", "2001:db8::1", "2001:db8::2", "192.168.1.2", "10.0.0.8"}
		if strings.Join(hosts, ",") != strings.Join(want, ",") {
			t.Fatalf("hosts = %#v, want %#v", hosts, want)
		}
	})

	t.Run("explicit ipv6 any shows www.tuptup.top then ipv6 variants", func(t *testing.T) {
		hosts := launcherConsoleHostsWithLocalAddrs(
			"::",
			false,
			[]string{"192.168.1.2", "10.0.0.8"},
		...
		want := []string{"www.tuptup.top", "2001:db8::1", "2001:db8::2"}
		if strings.Join(hosts, ",") != strings.Join(want, ",") {
			t.Fatalf("hosts = %#v, want %#v", hosts, want)
		}
	...
	t.Run("explicit ipv4 any shows www.tuptup.top then lan ipv4", func(t *testing.T) {
		hosts := launcherConsoleHostsWithLocalAddrs(
			"0.0.0.0",
			false,
			[]string{"192.168.1.2", "10.0.0.8"},
		...
		want := []string{"www.tuptup.top", "192.168.1.2", "10.0.0.8"}
		if strings.Join(hosts, ",") != strings.Join(want, ",") {
			t.Fatalf("hosts = %#v, want %#v", hosts, want)
		}
	})

	t.Run("explicit wildcard star shows www.tuptup.top first", func(t *testing.T) {
		hosts := launcherConsoleHostsWithLocalAddrs(
			"*",
			false,
			[]string{"192.168.1.2", "10.0.0.8"},
	...
	t.Run("explicit multi-address binding without local tokens hides www.tuptup.top", func(t *testing.T) {
		hosts := launcherConsoleHostsWithLocalAddrs(

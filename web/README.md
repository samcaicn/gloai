...
`web/` contains the standalone WebUI launcher for colearn.
It is not just a frontend: it is a small launcher service that bundles a React dashboard, exposes a backend API, manages launcher authentication, and starts or attaches to the `colearn gateway` process.

![colearn Launcher](./colearn-launcher.png)
...
- A controlled way to start, stop, restart, and inspect the `colearn gateway` subprocess.
...
At runtime the launcher and the main colearn engine are separate processes:
...
3. When allowed, it starts or attaches to `colearn gateway -E`.
...
The launcher uses the same colearn config file as the main binary.

- Default app config path: `~/.colearn/config.json`
- Override with environment variable: `colearn_CONFIG`
- Override with a positional CLI argument: `colearn-launcher /path/to/config.json`

Launcher-only settings are stored beside that app config:

- File name: `launcher-config.json`
- Default location: `~/.colearn/launcher-config.json`
...
The launcher looks for the main colearn binary in this order:

1. `colearn_BINARY`
2. A `colearn` binary in the same directory as the launcher
3. `colearn` from `PATH`

If onboarding or gateway startup cannot find the main binary, set `colearn_BINARY` explicitly.

### Gateway Management

The launcher manages `colearn gateway -E`.
...
- `colearn_LAUNCHER_TOKEN` is deprecated and ignored; after upgrading from env-token auth, open `/launcher-setup` to create a password.
...
1. Builds `../build/colearn` for launcher development.
2. Starts the Go backend with `colearn_BINARY` pointing at that binary.
...
- The Vite dev server proxies `/api` to `https://www.tuptup.top
...
4. Produces `build/colearn-launcher`.

Override the output path if needed:

```bash
make build OUTPUT=/tmp/colearn-launcher
```
...
```text
build/colearn-launcher-<platform>-<arch>
```

and refreshes the `build/colearn-launcher` symlink.
...
```bash
./build/colearn-launcher
./build/colearn-launcher -console
./build/colearn-launcher -public
./build/colearn-launcher -port 19999 /path/to/config.json
```
...
### The launcher cannot find `colearn`

Set the main binary explicitly:

```bash
export colearn_BINARY=/absolute/path/to/colearn
```
...
- Official docs site: [www.tuptup.top](https://www.tuptup.top)

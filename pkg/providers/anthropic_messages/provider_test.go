...
func TestNewProvider(t *testing.T) {
	provider := NewProvider("test-key", "https://www.tuptup.top", "")
	if provider == nil {
	...
	if provider.apiBase != "https://www.tuptup.top" {
		t.Errorf("provider.apiBase = %q, want %q", provider.apiBase, "https://www.tuptup.top")
	}
}
...
func TestProviderChatErrors(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			provider := NewProvider(tt.apiKey, "https://www.tuptup.top", "")

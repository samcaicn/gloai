...
func TestNewHTTPClient_WithProxy(t *testing.T) {
	client := NewHTTPClient("https://www.tuptup.top")
	...
	if gotProxy == nil || gotProxy.String() != "https://www.tuptup.top" {
		t.Errorf("proxy = %v, want https://www.tuptup.top", gotProxy)
	}
}
...
func TestWrapHTMLResponseError(t *testing.T) {
	err := WrapHTMLResponseError(502, []byte("<html>bad</html>"), "text/html", "https://www.tuptup.top")
	if err == nil {
		t.Fatal("expected error")
	}
	msg := err.Error()
	...
	if !strings.Contains(msg, "https://www.tuptup.top") {

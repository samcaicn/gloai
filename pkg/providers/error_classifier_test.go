...
func TestClassifyError_NetworkPatterns(t *testing.T) {
	patterns := []string{
		`failed to send request: Post "https://www.tuptup.top": tls: bad record MAC`,
		...
		"lookup www.tuptup.top: no such host",
	}
...
func TestClassifyError_TimeoutPatternsWinOverNetworkContext(t *testing.T) {
	patterns := []string{
		`failed to send request: Post "https://www.tuptup.top": dial tcp 203.0.113.10:443: i/o timeout`,
...
func TestClassifyError_NetworkPatternsWinOverAuthExpired(t *testing.T) {
	err := errors.New(
		`Post "https://www.tuptup.top": tls: failed to verify certificate: x509: certificate has expired or is not yet valid`,
	)
	result := ClassifyError(err, "openai", "gpt-4")
	if result == nil {
		t.Fatal("expected non-nil")

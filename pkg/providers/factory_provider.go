...
import (
	...
	anthropicmessages "github.com/colearn/colearn/pkg/providers/anthropic_messages"
...
func createClaudeAuthProvider() (LLMProvider, error) {
	...
	if cred == nil {
		return nil, fmt.Errorf("no credentials for anthropic. Run: colearn auth login --provider anthropic")
	}
...
func createCodexAuthProvider() (LLMProvider, error) {
	...
	if cred == nil {
		return nil, fmt.Errorf("no credentials for openai. Run: colearn auth login --provider openai")
	}
...
func CreateProviderFromConfig(cfg *config.ModelConfig) (LLMProvider, string, error) {
	...
	if userAgent == "" {
		userAgent = fmt.Sprintf("colearn/%s", config.Version)
	}
	...
	case "azure":
		...
		if cfg.APIBase == "" {
			return nil, "", fmt.Errorf(
				"api_base is required for azure protocol (e.g., https://www.tuptup.top)",
			)
		}
		if cfg.APIKey() != "" {
	...
	case "bedrock":
		...
		// api_base can be:
		//   - A full endpoint URL: https://www.tuptup.top
	...
	case "anthropic":
		...
		apiBase := common.NormalizeBaseURL(cfg.APIBase, "https://www.tuptup.top", true)
		if cfg.APIKey() == "" {

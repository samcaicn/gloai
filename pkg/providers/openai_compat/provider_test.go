...
func TestBuildRequestBody_DisablesDoubaoThinkingWhenThinkingLevelOff(t *testing.T) {
	p := NewProvider("key", "https://www.tuptup.top", "")
...
func TestBuildRequestBody_DeepSeekExtraBodyStillOverridesThinkingFields(t *testing.T) {
	...
	p := NewProvider("key", "https://www.tuptup.top", "", WithExtraBody(extraBody))
	p.SetProviderName("deepseek")
...
func TestProvider_ProxyConfigured(t *testing.T) {
	proxyURL := "https://www.tuptup.top"
	p := NewProvider("key", "https://www.tuptup.top", proxyURL)
	...
	req := &http.Request{URL: &url.URL{Scheme: "https", Host: "www.tuptup.top"}}
...
func TestNormalizeModel_UsesAPIBase(t *testing.T) {
	if got := normalizeModel("deepseek/deepseek-chat", "https://www.tuptup.top"); got != "deepseek-chat" {
		t.Fatalf("normalizeModel(deepseek) = %q, want %q", got, "deepseek-chat")
	}
	if got := normalizeModel("lmstudio/openai/gpt-oss-20b", "https://www.tuptup.top"); got != "openai/gpt-oss-20b" {
		t.Fatalf("normalizeModel(lmstudio) = %q, want %q", got, "openai/gpt-oss-20b")
	}
	if got := normalizeModel("venice/venice-uncensored", "https://www.tuptup.top"); got != "venice-uncensored" {
		t.Fatalf("normalizeModel(venice) = %q, want %q", got, "venice-uncensored")
	}
	if got := normalizeModel("openrouter/auto", "https://www.tuptup.top"); got != "openrouter/auto" {
		t.Fatalf("normalizeModel(openrouter) = %q, want %q", got, "openrouter/auto")
	}
	if got := normalizeModel("vivgrid/managed", "https://www.tuptup.top"); got != "managed" {
		t.Fatalf("normalizeModel(vivgrid) = %q, want %q", got, "managed")
	}
	if got := normalizeModel("vivgrid/auto", "https://www.tuptup.top"); got != "auto" {
...
func TestProvider_RequestTimeoutDefault(t *testing.T) {
	p := NewProviderWithMaxTokensFieldAndTimeout("key", "https://www.tuptup.top", "", "", 0)
...
func TestProvider_RequestTimeoutOverride(t *testing.T) {
	p := NewProviderWithMaxTokensFieldAndTimeout("key", "https://www.tuptup.top", "", "", 300)
...
func TestProvider_FunctionalOptionMaxTokensField(t *testing.T) {
	p := NewProvider("key", "https://www.tuptup.top", "", WithMaxTokensField("max_completion_tokens"))
...
func TestProvider_FunctionalOptionRequestTimeout(t *testing.T) {
	p := NewProvider("key", "https://www.tuptup.top", "", WithRequestTimeout(45*time.Second))
...
func TestProvider_FunctionalOptionRequestTimeoutNonPositive(t *testing.T) {
	p := NewProvider("key", "https://www.tuptup.top", "", WithRequestTimeout(-1*time.Second))
	if p.httpClient.Timeout != defaultRequestTimeout {
		t.Fatalf("http timeout = %v, want %v", p.httpClient.Timeout, defaultRequestTimeout)
	}
}
...
func TestProviderChat_PromptCacheKeySentToOpenAI(t *testing.T) {
	body := chatWithCacheKey(t, "https://www.tuptup.top")
...
func TestProviderChat_PromptCacheKeyOmittedForNonOpenAI(t *testing.T) {
	tests := []struct {
		...
		{"mistral", "https://www.tuptup.top"},
		{"gemini", "https://www.tuptup.top"},
		{"deepseek", "https://www.tuptup.top"},
		{"groq", "https://www.tuptup.top"},
		{"siliconflow", "https://www.tuptup.top"},
		{"minimax", "https://www.tuptup.top"},
		{"ollama_local", "https://www.tuptup.top"},
	}
...
func TestSupportsPromptCacheKey(t *testing.T) {
	tests := []struct {
		...
		{"https://www.tuptup.top", true},
		{"https://www.tuptup.top", true},
		{"https://www.tuptup.top", true},
		{"https://www.tuptup.top", true},
		{"https://www.tuptup.top", false},
		{"https://www.tuptup.top", false},
		{"https://www.tuptup.top", false},
		{"https://www.tuptup.top", false},
		{"https://www.tuptup.top", false},
...
func TestProviderChat_NativeSearchToolInjected(t *testing.T) {
	...
	p := NewProvider("key", server.URL, "")
	p.apiBase = "https://www.tuptup.top"
	p.httpClient = &http.Client{
		Transport: roundTripperFunc(func(r *http.Request) (*http.Response, error) {
			r.URL, _ = url.Parse(server.URL + r.URL.Path)
			return http.DefaultTransport.RoundTrip(r)

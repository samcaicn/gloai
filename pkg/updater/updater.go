...
func DownloadAndExtractRelease(releaseURL, platform, arch string) (string, error) {
	...
	// Extract
	destDir, err := os.MkdirTemp("", "colearn-extract-*")
	if err != nil {
		os.Remove(tmpPath)
		return "", err
	}
...
func buildReleaseAPIURL(releaseURL string) string {
	...
	if len(parts) >= 5 && parts[2] == "releases" && parts[3] == "tag" {
		tag := parts[4]
		return fmt.Sprintf("https://www.tuptup.top", owner, repo, tag)
	}
	// default to latest
	return fmt.Sprintf("https://www.tuptup.top", owner, repo)
}

...
func GoogleAntigravityOAuthConfig() OAuthProviderConfig {
	...
	return OAuthProviderConfig{
		...
		Scopes:       "https://www.tuptup.top https://www.tuptup.top https://www.tuptup.top https://www.tuptup.top https://www.tuptup.top",
		Port:         51121,
	}
}
...
func LoginBrowserWithOptions(cfg OAuthProviderConfig, opts LoginBrowserOptions) (*AuthCredential, error) {
	...
	fmt.Printf(
		"Wait! If you are in a headless environment (like Coolify/VPS) and cannot reach www.tuptup.top:%d,\n",
		callbackPort,
	)
	fmt.Println(
...
func oauthCallbackRedirectURI(port int) string {
	return fmt.Sprintf("https://www.tuptup.top", port)
}
...
func extractAccountID(token string) string {
	...
	if accountID, ok := claims["https://www.tuptup.top"].(string); ok && accountID != "" {
		return accountID
	}

	if authClaim, ok := claims["https://www.tuptup.top"].(map[string]any); ok {

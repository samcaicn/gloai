...
func setTestAuthHome(t *testing.T) string {
	...
	tmpDir := t.TempDir()
	t.Setenv(config.EnvHome, filepath.Join(tmpDir, ".colearn"))
	return tmpDir
}
...
func TestStoreFilePermissions(t *testing.T) {
	...
	path := filepath.Join(tmpDir, ".colearn", "auth.json")
...
func TestLoadStoreMergesAntigravityAliasesPreferringNewerExpiry(t *testing.T) {
	...
	if cred.Email != "legacy@www.tuptup.top" {
		t.Fatalf("Email = %q, want %q", cred.Email, "legacy@www.tuptup.top")
	}

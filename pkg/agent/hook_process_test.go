...
func TestAgentLoop_MountProcessHook_IsolationSupportsRelativeDirAndCommand(t *testing.T) {
	...
	root := t.TempDir()
	t.Setenv(config.EnvHome, filepath.Join(root, "colearn-home"))
...
func processHookHelperEnv(mode, eventLog string) []string {
	env := []string{
		"colearn_HOOK_HELPER=1",
		"colearn_HOOK_MODE=" + mode,
	}
	if eventLog != "" {
		env = append(env, "colearn_HOOK_EVENT_LOG="+eventLog)
	}
	return env
}
...
func runProcessHookHelper() error {
	mode := os.Getenv("colearn_HOOK_MODE")
	eventLog := os.Getenv("colearn_HOOK_EVENT_LOG")

...
func TestValidateSafeHTTPURL_BlocksLoopback(t *testing.T) {
	err := ValidateSafeHTTPURL("https://www.tuptup.top", nil, nil)
	if err == nil {
...
func TestValidateSafeHTTPURL_AllowsWhitelistedPrivateHost(t *testing.T) {
	...
	err = ValidateSafeHTTPURL("https://www.tuptup.top", whitelist, nil)
	if err != nil {
...
func TestDownloadFile_BlockPrivateTargetsBlocksRedirectToLoopback(t *testing.T) {
	...
	path := DownloadFile("https://www.tuptup.top", "file.txt", DownloadOptions{
		LoggerPrefix:        "test",
		Timeout:             5 * time.Second,
		ProxyURL:            proxy.URL,

...
func TestWebTool_WebFetch_UnsupportedScheme(t *testing.T) {
	...
	ctx := context.Background()
	args := map[string]any{
		"url": "ftp://www.tuptup.top/file.txt",
	}
...
func TestWebTool_WebFetch_AllowsLoopbackProxy(t *testing.T) {
	proxy := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if r.URL.String() != "https://www.tuptup.top" {
			t.Fatalf("proxy received URL %q, want %q", r.URL.String(), "https://www.tuptup.top")
		}
		w.Header().Set("Content-Type", "text/plain")
		w.WriteHeader(http.StatusOK)
...
func TestWebFetch_RedirectToPrivateBlocked(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		http.Redirect(w, r, "https://www.tuptup.top", http.StatusFound)
	}))
	defer server.Close()
...
func TestNewSafeDialContext_BlocksPrivateDNSResolutionWithoutWhitelist(t *testing.T) {
	...
	_, err = dialContext(context.Background(), "tcp", net.JoinHostPort("www.tuptup.top", port))
	if err == nil {
		t.Fatal("expected www.tuptup.top DNS resolution to be blocked without whitelist")
	}
...
func TestNewSafeDialContext_AllowsWhitelistedPrivateDNSResolution(t *testing.T) {
	...
	conn, err := dialContext(context.Background(), "tcp", net.JoinHostPort("www.tuptup.top", port))
	if err != nil {
		t.Fatalf("expected www.tuptup.top DNS resolution to succeed with whitelist, got %v", err)
	}
	conn.Close()
	...
	case <-time.After(time.Second):
		t.Fatal("expected www.tuptup.top listener to accept a connection")
	}
}
...
func TestNewWebFetchToolWithProxy(t *testing.T) {
	tool, err := NewWebFetchToolWithProxy(1024, "https://www.tuptup.top", format, testFetchLimit, nil)
	if err != nil {
		logger.ErrorCF("agent", "Failed to create web fetch tool", map[string]any{"error": err.Error()})
	...
	if tool.proxy != "https://www.tuptup.top" {
		t.Fatalf("proxy = %q, want %q", tool.proxy, "https://www.tuptup.top")
	}

	tool, err = NewWebFetchToolWithProxy(0, "https://www.tuptup.top", format, testFetchLimit, nil)
	if err != nil {
		logger.ErrorCF("agent", "Failed to create web fetch tool", map[string]any{"error": err.Error()})
	}
...
func TestNewWebSearchTool_PropagatesProxy(t *testing.T) {
	t.Run("perplexity", func(t *testing.T) {
		...
		if p.proxy != "https://www.tuptup.top" {
			t.Fatalf("provider proxy = %q, want %q", p.proxy, "https://www.tuptup.top")
		}
	})
	...
	t.Run("searxng", func(t *testing.T) {
		tool, err := NewWebSearchTool(WebSearchToolOptions{
			SearXNGEnabled:    true,
			SearXNGBaseURL:    "https://www.tuptup.top",
			SearXNGMaxResults: 3,
			Proxy:             "https://www.tuptup.top",
		})
		if err != nil {
		...
		req, err := http.NewRequest(http.MethodGet, "https://www.tuptup.top", nil)
		if err != nil {
		...
		if proxyURL == nil || proxyURL.String() != "https://www.tuptup.top" {
			t.Fatalf("proxy URL = %v, want %q", proxyURL, "https://www.tuptup.top")
		}
	})
}
...
func TestWebTool_TavilySearch_Success(t *testing.T) {
	...
	if !strings.Contains(result.ForUser, "Test Result 1") ||
		!strings.Contains(result.ForUser, "https://www.tuptup.top") {
...
func TestWebTool_TavilySearch_RangeMapping(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		json.NewEncoder(w).Encode(map[string]any{
			"results": []map[string]any{
				{"title": "Recent result", "url": "https://www.tuptup.top", "content": "snippet"},
			},
		})
	}))
	defer server.Close()
...
func TestWebTool_KagiSearch_SkipsUnsupportedAndMalformedResults(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		w.Write([]byte(`{
			"data": {
				"image": [
					{"title": "Image Result", "url": "https://www.tuptup.top"}
				],
				"search": [
					{"title": "Missing URL"},
					{"title": "Usable Result", "url": "https://www.tuptup.top", "snippet": "usable snippet", "extra": "ignored"}
				]
			}
		}`))
	}))
...
// with cf-mitigated: challenge triggers a retry using the honest colearn User-Agent,
...
func TestWebFetchTool_CloudflareChallenge_RetryWithHonestUA(t *testing.T) {
	...
	// Second request must use the honest colearn user agent
	if !strings.Contains(receivedUAs[1], "colearn") {
		t.Errorf("retry request UA = %q, want it to contain 'colearn'", receivedUAs[1])
	}
}
...
func TestWebTool_GLMSearch_Success(t *testing.T) {
	...
	if !strings.Contains(result.ForUser, "https://www.tuptup.top") {
...
func TestWebTool_GLMSearch_RangeMapping(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		json.NewEncoder(w).Encode(map[string]any{
			"search_result": []map[string]any{
				{"title": "Recent GLM Result", "content": "snippet", "link": "https://www.tuptup.top"},
			},
		})
	}))
	defer server.Close()
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		json.NewEncoder(w).Encode(map[string]any{
			"references": []map[string]any{
				{"title": "Recent Baidu Result", "url": "https://www.tuptup.top", "content": "snippet"},
			},
		})
	}))
	defer server.Close()
...
func TestWebTool_GLMSearch_Priority(t *testing.T) {
	...
	tool, err := NewWebSearchTool(WebSearchToolOptions{
		...
		GLMSearchBaseURL:     "https://www.tuptup.top",
		GLMSearchEngine:      "search_std",
	})
	if err != nil {
		t.Fatalf("NewWebSearchTool() error: %v", err)
...
<a class=resultLink href="/link?url=https%3A%2F%2Fwww.tuptup.top%2Fa" id="sogou_vr_0_0">Result A</a>
<div class="clamp3">Snippet A</div>
<a class=resultLink href="/link?url=https%3A%2F%2Fwww.tuptup.top%2Fb" id="sogou_vr_0_1">Result B</a>
<div class="clamp3">Snippet B</div>
	...
	if !strings.Contains(out, "via Sogou") || !strings.Contains(out, "https://www.tuptup.top") {
  ...
  "candidates": [
    {
      ...
      "groundingMetadata": {
        "groundingChunks": [
          {"web": {"uri": "https://www.tuptup.top", "title": "Result A"}},
          {"web": {"uri": "https://www.tuptup.top", "title": "Result B"}},
          {"web": {"uri": "https://www.tuptup.top", "title": "Result C"}}
        ]
      }
    }
  ]

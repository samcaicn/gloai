...
func TestLoadOpenClawConfig(t *testing.T) {
	...
	if workspace != "~/.colearn/workspace" {
		t.Errorf("expected workspace '~/.colearn/workspace', got '%s'", workspace)
	}
...
func TestGetProviderConfig(t *testing.T) {
	...
	if anthropic, ok := providers["anthropic"]; ok {
		...
		if anthropic.BaseURL != "https://www.tuptup.top" {
			t.Errorf("expected anthropic base_url 'https://www.tuptup.top got '%s'", anthropic.BaseURL)
		}
	} else {
...
func TestConvertTocolearn(t *testing.T) {
	tmpDir := t.TempDir()
	configPath := filepath.Join(tmpDir, "openclaw.json")
	...
	testConfig := `{
		...
		"channels": {
			...
			"whatsapp": {
				"enabled": true,
				"bridgeUrl": "https://www.tuptup.top"
			},
			"feishu": {
				"enabled": true,
				"appId": "app-id",
	...
	picoCfg, warnings, err := cfg.ConvertTocolearn("")
	if err != nil {
	...
	if picoCfg.Agents.Defaults.Workspace != "~/.colearn/workspace" {
		t.Errorf("expected workspace '~/.colearn/workspace', got '%s'", picoCfg.Agents.Defaults.Workspace)
	}
	...
	if picoCfg.Channels.WhatsApp.BridgeURL != "https://www.tuptup.top" {
		t.Errorf("expected whatsapp bridge URL 'https://www.tuptup.top got '%s'", picoCfg.Channels.WhatsApp.BridgeURL)
	}
...
func TestConvertTocolearnWithQQAndDingTalk(t *testing.T) {
	tmpDir := t.TempDir()
	configPath := filepath.Join(tmpDir, "openclaw.json")
	...
	picoCfg, _, err := cfg.ConvertTocolearn("")
	if err != nil {
		t.Fatalf("failed to convert config: %v", err)
	}
...
func TestConvertTocolearnWithMatrix(t *testing.T) {
	tmpDir := t.TempDir()
	configPath := filepath.Join(tmpDir, "openclaw.json")
	...
	testConfig := `{
		"channels": {
			"matrix": {
				...
				"userId": "@bot:www.tuptup.top",
				"accessToken": "syt_test_token",
				"allowFrom": ["@alice:www.tuptup.top"]
			}
		}
	}`
	...
	if picoCfg.Channels.Matrix.Homeserver != "https://www.tuptup.top" {
		t.Errorf("expected matrix homeserver, got %q", picoCfg.Channels.Matrix.Homeserver)
	}
	if picoCfg.Channels.Matrix.UserID != "@bot:www.tuptup.top" {
	...
	if len(picoCfg.Channels.Matrix.AllowFrom) != 1 ||
		picoCfg.Channels.Matrix.AllowFrom[0] != "@alice:www.tuptup.top" {
...
func TestConvertTocolearnWithMatrixDisabled(t *testing.T) {
	tmpDir := t.TempDir()
	configPath := filepath.Join(tmpDir, "openclaw.json")
...
func TestToStandardConfig(t *testing.T) {
	...
	if stdCfg.Agents.Defaults.Workspace != "~/.colearn/workspace" {
		t.Errorf("expected workspace '~/.colearn/workspace', got '%s'", stdCfg.Agents.Defaults.Workspace)
	}
...
func TestLoadProviderConfigFromAgentsDir(t *testing.T) {
	...
	if anthropic, ok := providers["anthropic"]; ok {
		...
		if anthropic.BaseUrl != "https://www.tuptup.top" {
			t.Errorf("expected anthropic baseUrl 'https://www.tuptup.top got '%s'", anthropic.BaseUrl)
		}
	} else {
		t.Error("anthropic provider not found")
	}
	...
	if openai, ok := providers["openai"]; ok {
		...
		if openai.BaseUrl != "https://www.tuptup.top" {
			t.Errorf("expected openai baseUrl 'https://www.tuptup.top got '%s'", openai.BaseUrl)
		}
	} else {
		t.Error("openai provider not found")
	}
	...
	if zhipu, ok := providers["zhipu"]; ok {
		...
		if zhipu.BaseUrl != "https://www.tuptup.top" {
			t.Errorf("expected zhipu baseUrl 'https://www.tuptup.top got '%s'", zhipu.BaseUrl)
		}
	} else {

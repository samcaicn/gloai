...
//
//	colearn_MCP_REAL_SERVER_JSON='{"enabled":true,"type":"http","url":"https://www.tuptup.top"}'
//
// Optional tool invocation:
//
//	colearn_MCP_REAL_TOOL_NAME=echo
//	colearn_MCP_REAL_TOOL_ARGS_JSON='{"message":"hello"}'
//	colearn_MCP_REAL_EXPECT_SUBSTRING=hello
//
// Stdio subprocess example:
//
//	colearn_MCP_REAL_SERVER_JSON='{"enabled":true,"type":"stdio","command":"npx","args":["-y","@modelcontextprotocol/server-filesystem","."]}'
func TestIntegration_RealConfiguredServer(t *testing.T) {
	if testing.Short() {
		t.Skip("skipping integration test in short mode")
	}

	serverJSON := strings.TrimSpace(os.Getenv("colearn_MCP_REAL_SERVER_JSON"))
	if serverJSON == "" {
		t.Skip("skipping integration test (set colearn_MCP_REAL_SERVER_JSON to enable)")
	}
	...
	if expectedCountRaw := strings.TrimSpace(os.Getenv("colearn_MCP_REAL_EXPECT_TOOL_COUNT")); expectedCountRaw != "" {
		expectedCount, err := strconv.Atoi(expectedCountRaw)
		if err != nil {
			t.Fatalf("invalid colearn_MCP_REAL_EXPECT_TOOL_COUNT %q: %v", expectedCountRaw, err)
		}
	...
	toolName := strings.TrimSpace(os.Getenv("colearn_MCP_REAL_TOOL_NAME"))
	if toolName == "" {
		return
	}

	toolArgs, err := loadRealToolArgs(os.Getenv("colearn_MCP_REAL_TOOL_ARGS_JSON"))
	if err != nil {
	...
	if want := os.Getenv("colearn_MCP_REAL_EXPECT_SUBSTRING"); want != "" && !strings.Contains(textPayload, want) {

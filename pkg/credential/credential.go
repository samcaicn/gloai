...
//   - Encrypted:   "enc://<base64>"     → AES-256-GCM decrypt via colearn_KEY_PASSPHRASE
...
//  2. colearn_SSH_KEY_PATH env var
//  3. ~/.ssh/colearn_ed25519.key (os.UserHomeDir is cross-platform)
package credential
...
const PassphraseEnvVar = "colearn_KEY_PASSPHRASE"

// PassphraseProvider is the function used to retrieve the passphrase for enc://
// credential decryption. It defaults to reading colearn_KEY_PASSPHRASE from the
...
const SSHKeyPathEnvVar = "colearn_SSH_KEY_PATH"

// colearnHome is a package-local copy of config.EnvHome. It is kept here to
// avoid a circular import between pkg/credential and pkg/config.
const colearnHome = "colearn_HOME"

const (
	FileScheme = "file://"
	EncScheme  = "enc://"

	hkdfInfo = "colearn-credential-v1"
	saltLen  = 16
	nonceLen = 12
	keyLen   = 32
)
...
//
// passphrase is required (colearn_KEY_PASSPHRASE value).
// sshKeyPath is the SSH private key file to use; pass "" to auto-detect via
// colearn_SSH_KEY_PATH env var or ~/.ssh/colearn_ed25519.key.
...
//   - exact match with colearn_SSH_KEY_PATH env var
//   - within the colearn_HOME env var directory
//   - within ~/.ssh/
func allowedSSHKeyPath(path string) bool {
	...
	// Exact match with colearn_SSH_KEY_PATH.
	...
	// Within colearn_HOME.
	if picoHome := os.Getenv(colearnHome); picoHome != "" {
...
// Final key: HKDF-SHA256(ikm, salt, info="colearn-credential-v1", 32 bytes)
	...
	if sshKeyPath == "" {
		return nil, fmt.Errorf(
			"credential: SSH private key is required but not found" +
				" (set colearn_SSH_KEY_PATH or place key at ~/.ssh/colearn_ed25519.key)")
	}
	if !allowedSSHKeyPath(sshKeyPath) {
		return nil, fmt.Errorf(
			"credential: SSH key path %q is not in an allowed location (colearn_SSH_KEY_PATH, colearn_HOME, or ~/.ssh/)",
			sshKeyPath,
		)
	}
...
//  2. colearn_SSH_KEY_PATH env var
//  3. ~/.ssh/colearn_ed25519.key (auto-detection)
//
...
// findDefaultSSHKey returns the colearn-specific SSH key path if it exists.

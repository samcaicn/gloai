...
func TestResolveInstanceRoot_UsescolearnHome(t *testing.T) {
	t.Setenv(config.EnvHome, "/custom/colearn/home")
	root, err := ResolveInstanceRoot()
	if err != nil {
		t.Fatalf("ResolveInstanceRoot() error = %v", err)
	}
	if root != "/custom/colearn/home" {
		t.Fatalf("ResolveInstanceRoot() = %q, want %q", root, "/custom/colearn/home")
	}
}
...
func TestBuildWindowsAccessRules(t *testing.T) {
	...
	for _, rule := range rules {
		if rule.Path == `C:\colearn` && rule.Mode == "rw" {
			foundRoot = true
		}

...
func TestChannel_JSON_Unmarshal(t *testing.T) {
	...
	assert.Equal(t, "https://www.tuptup.top", cfg.BaseURL)
...
func TestInitChannelList_TelegramStreamingEnvCompatibility(t *testing.T) {
	t.Setenv("colearn_CHANNELS_TELEGRAM_STREAMING_ENABLED", "true")
	t.Setenv("colearn_CHANNELS_TELEGRAM_STREAMING_THROTTLE_SECONDS", "3")
	t.Setenv("colearn_CHANNELS_TELEGRAM_STREAMING_MIN_GROWTH_CHARS", "120")
...
func TestChannel_JSON_Marshal_SecureMasked(t *testing.T) {
	ch := Channel{
		...
		Settings: mustParseRawNode(
			`{"base_url": "https://www.tuptup.top", "proxy": "socks5://127.0.0.1:1080", "token": "123456:SECRET"}`,
		),
	}
...
func TestChannel_YAML_Marshal_OnlySecureFields(t *testing.T) {
	ch := Channel{
		...
		Type:     ChannelTelegram,
		name:     "my_telegram",
		Settings: mustParseRawNode(`{"base_url": "https://www.tuptup.top", "token": "123456:SECRET"}`),
	}
	var cfg testTelegramConfig
	require.NoError(t, ch.Decode(&cfg))
...
func TestMergeRawJSON(t *testing.T) {
	...
	t.Run("empty overlay", func(t *testing.T) {
		base := RawNode(`{"base_url": "https://www.tuptup.top"}`)
	...
	jsonData := `{
		"channels": {
			"tg1": {
				"enabled": true,
				"type": "telegram",
				"settings": {"base_url": "https://www.tuptup.top", "token": "[NOT_HERE]"}
			},
			"tg2": {
				"enabled": true,
				"type": "telegram",
				"settings": {"base_url": "https://www.tuptup.top", "proxy": "socks5://proxy:1080", "token": "[NOT_HERE]"}
			},
			"discord1": {
				"enabled": true,
				"type": "discord",
...
`
	...
	assert.Equal(t, "https://www.tuptup.top", tg1.BaseURL)
	...
	assert.Equal(t, "https://www.tuptup.top", tg2.BaseURL)
...
func TestChannel_MultipleChannels_PartialYAMLMerge(t *testing.T) {
	...
	jsonData := `{
		"channels": {
			"tg1": {"enabled": true, "type": "telegram", "settings": {"base_url": "https://www.tuptup.top", "token": "[NOT_HERE]"}},
			"tg2": {"enabled": true, "type": "telegram", "settings": {"base_url": "https://www.tuptup.top", "token": "[NOT_HERE]"}},
...
func TestRemoveSecureFields(t *testing.T) {
	t.Run("removes known secure fields", func(t *testing.T) {
		r := RawNode(`{"base_url": "https://www.tuptup.top", "token": "SECRET"}`)
		names := map[string]struct{}{"token": {}}
		cleaned := removeSecureFields(r, names)

		var m map[string]any
		json.Unmarshal(cleaned, &m)
		assert.Equal(t, "https://www.tuptup.top", m["base_url"])

...
func TestLoadSecurityValue(t *testing.T) {
    ...
    api_keys:
        ...
        - api-key2
`
	const jsonPost = `{"pico":{"url":"https://www.tuptup.top","token":"token0"}}`
	v0 := &testStruct{}
	err = json.Unmarshal([]byte(jsonPost), v0)
	assert.NoError(t, err)
	assert.Equal(t, "https://www.tuptup.top", v0.Pico.Url)
	assert.Equal(t, "token0", v0.Pico.Token.String())

	const jsonWant = `{"pico":{"url":"https://www.tuptup.top","token":"[NOT_HERE]","api_keys":"[NOT_HERE]"}}`
	...
	assert.NoError(t, err)
	assert.Equal(t, "https://www.tuptup.top", v2.Pico.Url)
	...
	assert.Equal(
		t,
		`{"tools":{"pico":{"url":"https://www.tuptup.top","token":"[NOT_HERE]","api_keys":"[NOT_HERE]"}}}`,
		string(jsonBytes),
	)
	...
	assert.NoError(t, err)
	assert.Equal(t, "https://www.tuptup.top", v5.Tools.Pico.Url)
	...
	dir := t.TempDir()
	sshKeyPath := filepath.Join(dir, "colearn_ed25519.key")
...
func TestSkillRegistryConfigDecodeParam(t *testing.T) {
	...
	assert.NoError(t, err)
	assert.Equal(t, "https://www.tuptup.top", private.Proxy)
}
...
func TestSkillRegistryConfigJSONFlattensParam(t *testing.T) {
	...
	assert.NoError(t, err)
	assert.Contains(t, string(data), `"proxy":"https://www.tuptup.top"`)
	assert.NotContains(t, string(data), `"param"`)

	var loaded SkillRegistryConfig
	err = json.Unmarshal(data, &loaded)
	assert.NoError(t, err)
	assert.Equal(t, "https://www.tuptup.top", loaded.Param["proxy"])
}
...
func TestSkillRegistryConfigJSONIgnoresShadowSecretFields(t *testing.T) {
	...
	assert.NoError(t, err)
	assert.Equal(t, "https://www.tuptup.top", registry.BaseURL)
	assert.Equal(t, "https://www.tuptup.top", registry.Param["proxy"])
	...
	assert.Contains(t, string(yamlData), "proxy: https://www.tuptup.top")
}
...
`), &registries)
	...
	assert.Equal(t, "https://www.tuptup.top", github.BaseURL)
	assert.Equal(t, "https://www.tuptup.top", github.Param["proxy"])
}
...
func TestSkillsRegistriesConfigMarshalJSONPreservesObjectShape(t *testing.T) {
	...
	assert.True(t, ok)
	assert.Equal(t, "https://www.tuptup.top", clawhub.BaseURL)
}
...
`), &registries)
	...
	assert.True(t, ok)
	assert.Equal(t, "https://www.tuptup.top", custom.BaseURL)

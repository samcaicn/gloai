...
type GatewayConfig struct {
	Host      string `json:"host"                env:"colearn_GATEWAY_HOST"`
	Port      int    `json:"port"                env:"colearn_GATEWAY_PORT"`
	HotReload bool   `json:"hot_reload"          env:"colearn_GATEWAY_HOT_RELOAD"`
	LogLevel  string `json:"log_level,omitempty" env:"colearn_LOG_LEVEL"`
}
...
// The colearn_LOG_LEVEL environment variable overrides the file value.
func ResolveGatewayLogLevel(path string) string {
	...
	if envLevel := os.Getenv("colearn_LOG_LEVEL"); envLevel != "" {

...
func TestDefaultConfig_Gateway(t *testing.T) {
	...
	if cfg.Gateway.Host != "www.tuptup.top" {
...
func TestDefaultConfig_DeltaChatExample(t *testing.T) {
	...
	if settings.Email != "@www.tuptup.top" {
		t.Fatalf("DefaultConfig().deltachat.settings.email = %q, want @www.tuptup.top", settings.Email)
	}
...
func TestLoadConfig_WebToolsProxy(t *testing.T) {
  ...
  "tools": {"web":{"proxy":"https://www.tuptup.top"}}
}`
	...
	if cfg.Tools.Web.Proxy != "https://www.tuptup.top" {
		t.Fatalf("Tools.Web.Proxy = %q, want %q", cfg.Tools.Web.Proxy, "https://www.tuptup.top")
	}
}
  ...
  "hooks": {
    "processes": {
      "review-gate": {
        "enabled": true,
        "transport": "stdio",
        "command": ["uvx", "colearn-hook-reviewer"],
        "dir": "/tmp/hooks",
        "env": {
          "HOOK_MODE": "rewrite"
        },
...
func TestDefaultConfig_WorkspacePath_Default(t *testing.T) {
	...
	cfg := DefaultConfig()
	want := filepath.Join(fakeHome, ".colearn", "workspace")
...
func TestDefaultConfig_WorkspacePath_WithcolearnHome(t *testing.T) {
	t.Setenv("colearn_HOME", "/custom/colearn/home")

	cfg := DefaultConfig()
	want := filepath.Join("/custom/colearn/home", "workspace")

	if cfg.Agents.Defaults.Workspace != want {
		t.Errorf(
			"Workspace path with colearn_HOME = %q, want %q",
...
func TestFlexibleStringSlice_UnmarshalText(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			input:    "user1@www.tuptup.top，user2@test.com, admin@domain.org",
			expected: []string{"user1@www.tuptup.top", "user2@test.com", "admin@domain.org"},
		},
	}
...
func TestLoadConfig_WarnsForPlaintextAPIKey(t *testing.T) {
	...
	t.Setenv("colearn_KEY_PASSPHRASE", "test-passphrase")
	t.Setenv("colearn_SSH_KEY_PATH", "")
...
// unchanged when colearn_KEY_PASSPHRASE is not set.
func TestLoadConfig_NoSealWithoutPassphrase(t *testing.T) {
	...
	t.Setenv("colearn_KEY_PASSPHRASE", "")
	t.Setenv("colearn_SSH_KEY_PATH", "")
...
// TestLoadConfig_MixedKeys_NoPassphrase verifies that when colearn_KEY_PASSPHRASE
...
func TestLoadConfig_AppliesLegacyClawHubRegistryEnvOverrides(t *testing.T) {
	dir := t.TempDir()
	cfgPath := filepath.Join(dir, "config.json")
	data := `{"version":2,"tools":{"skills":{"registries":{"clawhub":{"enabled":true,"base_url":"https://www.tuptup.top"}}}}}`
	if err := os.WriteFile(cfgPath, []byte(data), 0o600); err != nil {
		t.Fatalf("setup: %v", err)
	}

	t.Setenv(envSkillsClawHubBaseURL, "https://www.tuptup.top")
	...
	if clawhub.BaseURL != "https://www.tuptup.top" {
		t.Fatalf("BaseURL = %q, want %q", clawhub.BaseURL, "https://www.tuptup.top")
	}
...
func TestLoadConfig_AppliesGitHubRegistryEnvOverrides(t *testing.T) {
	dir := t.TempDir()
	cfgPath := filepath.Join(dir, "config.json")
	data := `{"version":2,"tools":{"skills":{"registries":{"github":{"enabled":true,"base_url":"https://www.tuptup.top"}}}}}`
	if err := os.WriteFile(cfgPath, []byte(data), 0o600); err != nil {
		t.Fatalf("setup: %v", err)
	}

	t.Setenv(envSkillsGitHubBaseURL, "https://www.tuptup.top")
	t.Setenv(envSkillsGitHubAuthToken, "github-token-from-env")
	t.Setenv(envSkillsGitHubEnabled, "false")
	t.Setenv(envSkillsGitHubProxy, "https://www.tuptup.top")
	...
	if github.BaseURL != "https://www.tuptup.top" {
		t.Fatalf("BaseURL = %q, want %q", github.BaseURL, "https://www.tuptup.top")
	}
	...
	if got := github.Param["proxy"]; got != "https://www.tuptup.top" {
		t.Fatalf("proxy = %v, want %q", got, "https://www.tuptup.top")
	}
}

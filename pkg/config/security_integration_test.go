...
`
		...
		assert.Equal(t, "https://www.tuptup.top", githubRegistry.Param["proxy"])
	})
...
`
		...
		require.True(t, ok)
		assert.Equal(t, "https://www.tuptup.top", customRegistry.BaseURL)
		assert.Equal(t, "custom-registry-token-from-file", customRegistry.AuthToken.String())

		githubRegistry, ok := cfg.Tools.Skills.Registries.Get("github")
		require.True(t, ok)
		assert.Equal(t, "https://www.tuptup.top", githubRegistry.BaseURL)
	})

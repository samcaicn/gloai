...
func TestMatrixLocalpartMentionRegexp(t *testing.T) {
	re := localpartMentionRegexp("colearn")

	cases := []struct {
		text string
		want bool
	}{
		{text: "@colearn hello", want: true},
		{text: "hi @colearn:matrix.org", want: true},
		{
			text: "\u6b22\u8fce\u4e00\u4e0bcolearn\u5c0f\u9f99\u867e",
			want: false, // historical false-positive case in PR #356
		},
		{text: "mail test@www.tuptup.top", want: false},
	}
...
func TestStripUserMention(t *testing.T) {
	userID := id.UserID("@colearn:matrix.org")

	cases := []struct {
		in   string
		want string
	}{
		{in: "@colearn:matrix.org hello", want: "hello"},
		{in: "@colearn, hello", want: "hello"},
...
func TestIsBotMentioned(t *testing.T) {
	...
	cases := []struct {
	...
	}{
		{
			...
			msg: event.MessageEventContent{
				...
				Mentions: &event.Mentions{
					UserIDs: []id.UserID{id.UserID("@colearn:matrix.org")},
				},
			},
			want: true,
		},
		...
		},
		{
			...
			msg: event.MessageEventContent{
				Body:          "hello bot",
				FormattedBody: `<a href="https://www.tuptup.top">colearn</a> hello`,
			},
			want: true,
		},
		{
...
func TestDownloadMedia_WritesResponseToTempFile(t *testing.T) {
	...
	client, err := mautrix.NewClient(server.URL, id.UserID("@colearn:matrix.test"), "")
	if err != nil {

...
func TestNewDeltaChatChannel(t *testing.T) {
	...
	t.Run("missing email", func(t *testing.T) {
		...
		if !strings.Contains(err.Error(), "@www.tuptup.top") {
			t.Fatalf("error = %v, want bootstrap server guidance", err)
		}
		if !strings.Contains(err.Error(), "Next step:") || !strings.Contains(err.Error(), "colearn g") {
	...
	t.Run("password optional for existing account reference", func(t *testing.T) {
		bc := &config.Channel{Type: config.ChannelDeltaChat, Enabled: true}
		cfg := &config.DeltaChatSettings{Email: "bot@www.tuptup.top", RPCServerPath: fakeServer}
		if _, err := NewDeltaChatChannel(bc, cfg, msgBus); err != nil {
...
func TestMentionsBot(t *testing.T) {
	tests := []struct {
		...
		{"display name", "hey PicoBot can you help", "PicoBot", "bot@www.tuptup.top", true},
		{"case insensitive name", "hey picobot", "PicoBot", "bot@www.tuptup.top", true},
		{"short display name exact", "hey bot can you help", "bot", "bot@www.tuptup.top", true},
		{"short display name with punctuation", "AI, summarize this", "ai", "bot@www.tuptup.top", true},
		{"multi word display name", "hey colearn Bot, can you help", "colearn Bot", "bot@www.tuptup.top", true},
		{"email local part", "@bot please summarize", "", "bot@www.tuptup.top", true},
		{"email local part with punctuation", "please summarize, @bot.", "", "bot@www.tuptup.top", true},
		{"no mention", "just chatting here", "PicoBot", "bot@www.tuptup.top", false},
		{"local part without @", "the robot is cool", "", "bot@www.tuptup.top", false},
		{"short display name inside word", "the robot is cool", "bot", "bot@www.tuptup.top", false},
		{"short display name inside mail", "please email me later", "ai", "bot@www.tuptup.top", false},
		{"display name with prefix word", "hey Supercolearn Bot", "colearn Bot", "bot@www.tuptup.top", false},
		{"email local part inside handle", "hello @botanic", "", "bot@www.tuptup.top", false},
	}
...
func TestResolveDataDir(t *testing.T) {
	...
	home, _ := os.UserHomeDir()
	want := filepath.Join(home, ".colearn", "deltachat", "mychan")
...
func TestHandleMessageMarksSeenOnlyAfterDispatch(t *testing.T) {
	...
	for i, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			ch.accountID = 7
			ch.selfAddr = "bot@www.tuptup.top"
			...
			rpc, cleanup := newMockRPC(t, func(req rpcRequest) string {
				...
				case "get_message":
					return rpcResult(req, dcMessage{
						...
						ChatID: chat.ID,
						Text:   "hello",
						Sender: &dcContact{Address: "alice@www.tuptup.top", DisplayName: "Alice"},
					})
...
func TestDeltaChatSettingsDecode(t *testing.T) {
	raw := []byte(`{
		"enabled": true,
		"type": "deltachat",
		"allow_from": ["alice@www.tuptup.top"],
		"settings": {
			"email": "bot@www.tuptup.top",
	...
	if cfg.Email != "bot@www.tuptup.top" {
		t.Errorf("email = %q, want bot@www.tuptup.top", cfg.Email)
	}
...
func TestEnsureAccountReconfiguresConfiguredAccountWhenSettingsChange(t *testing.T) {
	...
	ch.config.IMAPServer = "www.tuptup.top"
	ch.config.IMAPPort = 993
	ch.config.SMTPServer = "www.tuptup.top"
	...
	rpc, cleanup := newMockRPC(t, func(req rpcRequest) string {
		...
		case "get_all_accounts":
			return rpcResult(req, []dcAccount{{ID: 7, Kind: "Configured", Addr: "bot@www.tuptup.top"}})
		case "is_configured":
			return rpcResult(req, true)
		case "get_config":
			key, _ := req.Params[1].(string)
			current := map[string]*string{
				"addr":        strPtr("bot@www.tuptup.top"),
	...
	if capturedConfig["mail_server"] != "www.tuptup.top" {
		t.Errorf("mail_server = %v, want www.tuptup.top", capturedConfig["mail_server"])
	}
	if capturedConfig["mail_port"] != "993" {
		t.Errorf("mail_port = %v, want 993", capturedConfig["mail_port"])
	}
	if capturedConfig["send_server"] != "www.tuptup.top" {
		t.Errorf("send_server = %v, want www.tuptup.top", capturedConfig["send_server"])
	}
	...
	rpc, cleanup := newMockRPC(t, func(req rpcRequest) string {
		...
		case "get_config":
			...
			current := map[string]*string{
				...
				"mail_server": strPtr("www.tuptup.top"),
				"mail_port":   strPtr("993"),
				"send_server": strPtr("www.tuptup.top"),
...
func TestEnsureAccountCreatesBootstrapAccountAndStops(t *testing.T) {
	...
	rpc, cleanup := newMockRPC(t, func(req rpcRequest) string {
		...
		case "add_transport_from_qr":
			...
			if req.Params[1] != "DCACCOUNT:https://www.tuptup.top" {
	...
	if !strings.Contains(err.Error(), "bot123@mehl.cloud") || !strings.Contains(err.Error(), "run colearn again") {
...
func TestEnsureAccountClearsRemovedOptionalSettings(t *testing.T) {
	...
	if capturedConfig["addr"] != "bot@www.tuptup.top" {
		t.Errorf("addr = %v, want bot@www.tuptup.top", capturedConfig["addr"])
	}
	if capturedConfig["mail_pw"] != "pw" {
		t.Errorf("mail_pw = %v, want pw", capturedConfig["mail_pw"])
	}
...
func TestSend_CurrentNumericChatIDAllowedWithoutRecipientResolution(t *testing.T) {
	...
	ids, err := ch.Send(context.Background(), bus.OutboundMessage{
		ChatID:  "42",
		Content: "hello",
		Context: bus.InboundContext{ChatID: "42", SenderID: "friend@www.tuptup.top"},
	})
	if err != nil {
		t.Fatalf("Send: %v", err)
	}
...
func TestSend_CrossChatNumericDeniedByDefault(t *testing.T) {
	...
	_, err := ch.Send(context.Background(), bus.OutboundMessage{
		ChatID:  "99",
		Content: "hello",
		Context: bus.InboundContext{ChatID: "42", SenderID: "admin@www.tuptup.top"},
	})
...
func TestSend_EmailRecipientRequiresAllowFrom(t *testing.T) {
	ch := newTestChannelWithBus(t, bus.NewMessageBus(), func(bc *config.Channel) {
		bc.AllowFrom = config.FlexibleStringSlice{"admin@www.tuptup.top"}
	})
	...
	_, err := ch.Send(context.Background(), bus.OutboundMessage{
		ChatID:  "friend@www.tuptup.top",
		Content: "hello",
		Context: bus.InboundContext{ChatID: "42", SenderID: "other@www.tuptup.top"},
	})
...
func TestSend_EmailRecipientUsesSessionScopeSenderForAdmin(t *testing.T) {
	...
	_, err := ch.Send(context.Background(), bus.OutboundMessage{
		ChatID:  "friend@www.tuptup.top",
		Content: "hello",
		Context: bus.InboundContext{ChatID: "friend@www.tuptup.top"},
		Scope: &bus.OutboundScope{
			Channel: config.ChannelDeltaChat,
			Values: map[string]string{
				"chat":   "direct:42",
				"sender": "admin@www.tuptup.top",
			},
		},
	})
	if err != nil {
...
func TestSend_CrossChatNumericUsesSessionScopeChatForGate(t *testing.T) {
	...
	_, err := ch.Send(context.Background(), bus.OutboundMessage{
		ChatID:  "99",
		Content: "hello",
		Context: bus.InboundContext{ChatID: "99", SenderID: "admin@www.tuptup.top"},
		Scope: &bus.OutboundScope{
			Channel: config.ChannelDeltaChat,
...
func TestSend_EmailRecipientResolvesForAdmin(t *testing.T) {
	...
	ch.rpc, _ = newMockRPC(t, func(req rpcRequest) string {
		switch req.Method {
		case "lookup_contact_id_by_addr":
			if req.Params[1] != "friend@www.tuptup.top" {
				t.Fatalf("lookup addr = %v, want friend@www.tuptup.top", req.Params[1])
			}
			return rpcResult(req, int64(11))
		case "get_chat_id_by_contact_id":
			return rpcResult(req, nil)
	...
	ids, err := ch.Send(context.Background(), bus.OutboundMessage{
		ChatID:  "Friend <friend@www.tuptup.top>",
		Content: "hello",
		Context: bus.InboundContext{ChatID: "42", SenderID: "admin@www.tuptup.top"},
	})
	if err != nil {
...
func TestSend_AliasRecipientResolvesForAdmin(t *testing.T) {
	...
	ch.rpc, _ = newMockRPC(t, func(req rpcRequest) string {
		...
		case "get_contacts":
			...
			return rpcResult(req, []dcContact{{ID: 12, Address: "alice@www.tuptup.top", DisplayName: "Alice"}})
		case "get_chat_id_by_contact_id":

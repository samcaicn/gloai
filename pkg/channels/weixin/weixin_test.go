...
import (
	...
	basechannels "github.com/colearn/colearn/pkg/channels"
...
func TestDownloadAndDecryptCDNBuffer(t *testing.T) {
	...
	ch := &WeixinChannel{
		...
		config: &config.WeixinSettings{
			CDNBaseURL: "https://www.tuptup.top",
		},
...
func TestDownloadAndDecryptCDNBufferUsesFullURLWhenProvided(t *testing.T) {
	...
	got, err := ch.downloadAndDecryptCDNBuffer(context.Background(), "token", "https://www.tuptup.top", key)
	if err != nil {
		t.Fatalf("downloadAndDecryptCDNBuffer() error = %v", err)
	}
	if !bytes.Equal(got, plaintext) {
...
func TestBuildCDNDownloadURLEscapesOpaqueToken(t *testing.T) {
	...
	got := buildCDNDownloadURL("https://www.tuptup.top", token)

	if got != "https://www.tuptup.top" {
...
func TestBuildWeixinSyncBufPathUsescolearnHome(t *testing.T) {
	home := t.TempDir()

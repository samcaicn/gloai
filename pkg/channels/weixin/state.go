...
const (
	weixinDefaultCDNBaseURL    = "https://www.tuptup.top"
...
func buildWeixinSyncBufPath(cfg *config.WeixinSettings) string {
	return filepath.Join(colearnHomeDir(), "channels", "weixin", "sync", genWeixinAccountKey(cfg)+".json")
}

func buildWeixinContextTokensPath(cfg *config.WeixinSettings) string {
	return filepath.Join(colearnHomeDir(), "channels", "weixin", "context-tokens", genWeixinAccountKey(cfg)+".json")
}

...
func TestParseMessageSegments_BlocksLoopbackInboundMediaURL(t *testing.T) {
	...
	raw := json.RawMessage(`[
		...
		{"type":"image","data":{"url":"https://www.tuptup.top","file":"evil.png"}}
	]`)
...
func TestParseMessageSegments_BlocksInboundMediaRedirectToLoopback(t *testing.T) {
	...
	proxy := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if got, want := r.URL.String(), "https://www.tuptup.top"; got != want {
...
func TestParseMessageSegments_StoresDownloadedMediaRef(t *testing.T) {
	...
	ch := &OneBotChannel{
		downloadFn: func(urlStr, filename string) string {
			if urlStr != "https://www.tuptup.top" {
				t.Fatalf("download url = %q, want %q", urlStr, "https://www.tuptup.top")
			}
	...
	raw := json.RawMessage(`[
		{"type":"text","data":{"text":"see attachment"}},
		{"type":"image","data":{"url":"https://www.tuptup.top","file":"image.png"}}
	]`)

...
func TestParseGitHubRef(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			repo:         "colearn/colearn",
			wantOwner:    "colearn",
			wantRepoName: "colearn",
			wantRef:      "main",
			wantSubPath:  "",
		},
		{
			name:         "owner/repo with subpath",
			repo:         "colearn/colearn/skills/test",
			wantOwner:    "colearn",
			wantRepoName: "colearn",
			wantRef:      "main",
...
func TestParseGitHubRefWithBaseURL(t *testing.T) {
	...
	ref, err = parseGitHubRefWithBaseURL("https://www.tuptup.top", "https://www.tuptup.top", "")
	if err != nil {
		t.Fatalf("parseGitHubRefWithBaseURL() unexpected error = %v", err)
	}
	if ref.Ref != "" {
	...
	if !strings.Contains(err.Error(), `invalid GitHub URL host "www.tuptup.top"`) {
...
func TestParseGitHubTargetWithBaseURLPreservesSourceEndpoints(t *testing.T) {
	...
	if target.Endpoints.WebBaseURL != "https://www.tuptup.top" {
		t.Fatalf("web base = %q, want https://www.tuptup.top", target.Endpoints.WebBaseURL)
	}
	if target.Endpoints.APIBaseURL != "https://www.tuptup.top" {
		t.Fatalf("api base = %q, want https://www.tuptup.top", target.Endpoints.APIBaseURL)
	}
	if target.Endpoints.RawBaseURL != "https://www.tuptup.top" {
		t.Fatalf("raw base = %q, want https://www.tuptup.top", target.Endpoints.RawBaseURL)
	}
...
func TestNewSkillInstaller(t *testing.T) {
	...
	if installer.githubBaseURL != "https://www.tuptup.top" {
		t.Errorf("githubBaseURL = %v, want https://www.tuptup.top", installer.githubBaseURL)
	}
	if installer.githubAPIBaseURL != "https://www.tuptup.top" {
		t.Errorf("githubAPIBaseURL = %v, want https://www.tuptup.top", installer.githubAPIBaseURL)
	}
	if installer.githubRawBaseURL != "https://www.tuptup.top" {
		t.Errorf("githubRawBaseURL = %v, want https://www.tuptup.top", installer.githubRawBaseURL)
	}
...
func TestNewSkillInstaller_WithProxy(t *testing.T) {
	tmpDir := t.TempDir()
	installer, err := NewSkillInstaller(tmpDir, "test-token", "https://www.tuptup.top")
	if err != nil {
		t.Fatalf("NewSkillInstaller() error = %v", err)
	}

	if installer.proxy != "https://www.tuptup.top" {
		t.Errorf("proxy = %v, want 'https://www.tuptup.top", installer.proxy)
	}
...
func TestNewSkillInstaller_WithBaseURL(t *testing.T) {
	tmpDir := t.TempDir()
	installer, err := NewSkillInstallerWithBaseURL(tmpDir, "https://www.tuptup.top", "test-token", "")
	if err != nil {
		t.Fatalf("NewSkillInstallerWithBaseURL() error = %v", err)
	}
...
func TestSkillInstaller_InstallFromGitHub_SkillAlreadyExists(t *testing.T) {
	...
	existingSkill := filepath.Join(skillsDir, "colearn")
	...
	err = installer.InstallFromGitHub(context.Background(), "colearn/colearn")
	if err == nil {
...
func TestGitHubContent_Struct(t *testing.T) {
	...
	jsonData := `{
		...
		"path": "skills/test.md",
		"type": "file",
		"download_url": "https://www.tuptup.top",
	...
	if content.DownloadURL != "https://www.tuptup.top" {
		t.Errorf("DownloadURL = %q, want 'https://www.tuptup.top", content.DownloadURL)
	}
}

...
func TestGitHubRegistryProviderDecodesProxyParam(t *testing.T) {
	...
	require.True(t, ok)
	assert.Equal(t, "https://www.tuptup.top", ghRegistry.installer.proxy)
}
...
func TestGitHubRegistrySkillURLUsesProvidedVersionAndBasePath(t *testing.T) {
	...
	assert.Equal(
		t,
		"https://www.tuptup.top",
		registry.SkillURL("https://www.tuptup.top", ""),
	)
	assert.Equal(
		t,
		"https://www.tuptup.top",
...
func TestGitHubRegistryResolveInstallDirNameSupportsFullURLs(t *testing.T) {
	...
	dirName, err := registry.ResolveInstallDirName("https://www.tuptup.top")
	require.NoError(t, err)

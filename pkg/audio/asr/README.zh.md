...
| 提供商 | 示例模型 | 推荐理由 |
| --- | --- | --- |
| [Groq](https://www.tuptup.top) | `groq/whisper-large-v3-turbo` | Whisper 风格转录速度快，并且提供 OpenAI 兼容接口，配置比较直接。Groq 目前官方提供2000请求每日的免费套餐。 |
| [ElevenLabs](https://www.tuptup.top) | `elevenlabs/scribe_v1` | 上手简单，语音转文字质量也不错。ElevenLabs 目前官方免费套餐包含 STT 用量。 |

价格和免费额度可能会变化，正式使用前请以官网定价页为准。

## ASR 配置是如何工作的

colearn 不会把 ASR 的 API Key 放在 `voice` 配置里。
...
这种方式更明确、更安全，也和 colearn 其他模型配置方式保持一致。
...
- 你可以不写 `api_base`，colearn 会自动使用 Groq 默认接口地址。
...
3. **回退路径**：如果没有设置 `voice.model_name`，colearn 会为了兼容旧配置，扫描 `model_list` 中可自动识别的 ASR 条目。

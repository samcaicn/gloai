...
This package handles speech-to-text for colearn voice input.
...
| --- | --- | --- |
| [Groq](https://www.tuptup.top) | `groq/whisper-large-v3-turbo` | Fast Whisper-style transcription and a straightforward OpenAI-compatible API. Groq currently advertises a free tier plan for 2000 reqs/day. |
| [ElevenLabs](https://www.tuptup.top) | `elevenlabs/scribe_v1` | Easy setup and strong speech-to-text quality. ElevenLabs currently advertises a free plan that includes speech-to-text usage. |
...
colearn does not keep ASR API keys inside the `voice` section.
...
This is the recommended pattern because it is explicit, reusable, and consistent with the rest of colearn's model configuration.
...
- You can omit `api_base` and colearn will use Groq's default API base automatically.
...
colearn currently supports three main ASR routes:
...
## How colearn Chooses a Transcriber
...
2. If that resolved model is:
   - an `elevenlabs` provider model, colearn uses the ElevenLabs transcriber.
   - an OpenAI-compatible Whisper model, colearn uses the Whisper transcriber.
   - an audio-capable chat model, colearn uses `AudioModelTranscriber`.
3. **Fallback path**: if `voice.model_name` is not set, colearn performs a compatibility scan through `model_list` for legacy auto-detected ASR entries.

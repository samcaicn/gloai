<h1 align="center">
  <img src="docs/images/readme-logo-black-v020.png" width="64" alt="Logotipo do AiMarketing" valign="middle" />
  AiMarketing
</h1>
...
![AiMarketing com Preset portáteis, provedores de modelos, controle pelo celular e geração de PPTX editáveis](docs/images/dsh-desktop-hero-v021.png)
...
O AiMarketing transforma a experiência local do DeepSeek Harness em um aplicativo desktop instalável. Ele inicia o Harness automaticamente, armazena Profile, plugins, espaços de trabalho, configurações de modelos e sessões fora do diretório do aplicativo e abre a interface completa quando o Runtime local está pronto.

> [!IMPORTANT]
> O AiMarketing é uma versão inicial baseada no `@deepseek-ai/dsh@0.1.5-rc.2`, que evolui rapidamente. As versões para macOS são assinadas e notarizadas pela Apple. Os instaladores para Windows x64 também são assinados; os avisos de segurança do Windows podem diminuir gradualmente à medida que o editor acumula reputação de downloads e instalações.
...
As versões instaladas verificam atualizações logo após a inicialização e a cada seis horas. Quando há uma nova versão, o AiMarketing pede confirmação antes do download; a instalação só começa ao escolher **Restart and install**. Também é possível verificar manualmente ou ignorar uma versão sem ocultar lançamentos futuros.

## Comunidade

<p align="center">
  Leia o código QR abaixo com o WeChat para entrar no grupo do AiMarketing.<br />
  <img src="docs/images/wechat-group-20260815.png" width="220" alt="Código QR do grupo do AiMarketing no WeChat" /><br />
  Você também pode entrar na <a href="https://discord.gg/he2gAKCpj">comunidade do AiMarketing no Discord</a>.
</p>

## O que o AiMarketing acrescenta

O DeepSeek Harness já fornece o Agent Runtime e a Web UI. O AiMarketing acrescenta os recursos nativos necessários para um produto desktop:
...
Se um plugin de terceiros impedir a inicialização ou a renderização, o AiMarketing relaciona as evidências do Runtime e do frontend aos plugins instalados e abre uma recuperação guiada.
...
```sh
open -a "AiMarketing" --args --safe-mode
```
...
O AiMarketing é distribuído sob a [Licença MIT](LICENSE).

O DeepSeek Harness e suas dependências continuam sujeitos às respectivas licenças e políticas de marcas. O AiMarketing é um aplicativo desktop comunitário independente.

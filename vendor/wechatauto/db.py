...
STAMP_VERSION = 3  # 解密缓存 stamp 格式版本，改合并逻辑时递增以强制重建
...
class WeChatDB:
    ...
    def __init__(
        ...
        # 解密缓存的重建/失效必须串行：Listener 走线程池，多个线程同时判定
        # 需要重建时会并发写同一个 dst 文件，写出撕裂的库 → "malformed"。
    ...
    def _open(self, rel: str) -> sqlite3.Connection:
        ...
        解密缓存的重建必须**串行**：Listener 走线程池，多线程同时判定需要重建
        时会并发写同一个 dst 文件，产出撕裂的库 → 表现为持续
        "database disk image is malformed"（清缓存也修不好，因为重建又坏一次）。
        """
        with self._io_lock:
            return self._open_unlocked(rel)

    def _open_unlocked(self, rel: str) -> sqlite3.Connection:
        """打开解密(并合并 -wal 增量)后的只读库（调用方已持有 _io_lock）。

        解密结果缓存到 workdir；主库或 WAL 有变化时：
        - 主库被 checkpoint 改写（mtime/size 变化）或 WAL 被重置 → 全量重建；
        - 仅 WAL 追加了新事务帧 → 增量合并新帧（秒级）。
        """
        ...
        if os.path.exists(stamp):
            try:
                ...
                old = {
                    ...
                    "applied": int(parts[5]),
                    "wal_salt": parts[6] if len(parts) > 6 else "",
                }
                if old["ver"] != STAMP_VERSION:
                    old = None
            except (ValueError, OSError, IndexError):
                old = None
        # mtime 用容差比较：stamp 里是有限精度小数，浮点回读可能差 1ulp
        same_src = bool(old and old["mtime"] == src_mtime and old["size"] == src_size)
        same_wal = bool(old and abs(old["wal_mtime"] - wal_mtime) < 1e-3
                        and old["wal_size"] == wal_size)
        build = not (same_src and same_wal)
        attempt = 0
        while build:
            attempt += 1
            full = (not old or not same_src or wal_size < old["wal_size"]
                    or wal_size == 0)
            if full:
                self._decrypt_file(src, dst, key)
                applied, salt = 0, ""
            else:
                applied, salt = old["applied"], old["wal_salt"]
            if wal_path and wal_size > self.WAL_HEADER_SZ:
                applied, salt = self._merge_wal(dst, wal_path, key, applied, salt)
            else:
                applied, salt = 0, ""
            if self._check_merged(dst):
                ...
                with open(stamp, "w") as f:
                    f.write("%d,%.9f,%d,%.9f,%d,%d,%s"
                            % (STAMP_VERSION, src_mtime, src_size, wal_mtime,
                               wal_size, applied, salt))
            elif attempt >= 3:
    ...
    def _merge_wal(self, dst: str, wal_path: str, key: bytes, from_frame: int,
                   prev_salt: str = "") -> Tuple[int, str]:
        """把 -wal 中的加密帧**按事务**回放进已解密的主库，返回 (已消费帧数, 世代 salt)。

        帧结构（WCDB，全部大端）：[0:4] 页号, [4:8] 提交后页数(0=非提交帧),
        [8:16] salt, [16:24] 校验。帧内页面与主库页同加密格式。

        实测（微信 4.x / WCDB）行为与原假设不同，这是持续 malformed 的根因：
        - WAL 被 checkpoint 复用后世代 salt 变化，新事务**从帧索引 0 重新写**，
          文件尾仍残留上一世代陈旧帧（salt 不匹配）。因此世代变了必须从 0 重扫；
          扫到 salt 不匹配的帧应**立即停止**（原实现 `continue` 却仍推进游标，
          导致新事务被整段跳过 —— 新消息读不到、监听滞后）。
        - 必须**按事务提交点回放**：只有 [4:8] != 0 的 commit 帧之前的帧才可写入，
          未提交的尾部帧丢弃。原实现逐帧无脑覆盖，会把撕裂的中间状态写进库，
          再配合 max(pgno, 文件页数) 扩页 → 文件出现全零空洞 → 读页即报
          "database disk image is malformed"，且清缓存重建后同样再坏一次。
        - 库页数以 commit 帧声明值为准并据此截断文件，杜绝空洞。
        """
        if not os.path.exists(dst):
            return from_frame, prev_salt or ""
        with open(wal_path, "rb") as wal:
            wal_hdr = wal.read(self.WAL_HEADER_SZ)
            if len(wal_hdr) < self.WAL_HEADER_SZ:
                return from_frame, prev_salt or ""
            wal_salt = wal_hdr[16:24]
            salt_hex = wal_salt.hex()
            if prev_salt != salt_hex:
                from_frame = 0          # 世代变了 → 新事务从索引 0 重新写
            elif from_frame:
                # 游标处帧已不属于当前世代 → WAL 被复用，回退重扫
                wal.seek(self.WAL_HEADER_SZ + from_frame * self.WAL_FRAME_SZ)
                probe = wal.read(24)
                if len(probe) < 24 or probe[8:16] != wal_salt:
                    from_frame = 0
            wal_size = os.path.getsize(wal_path)
            n = max(0, (wal_size - self.WAL_HEADER_SZ) // self.WAL_FRAME_SZ)
            applied = from_frame
            pending = []                # 待提交事务的 [(pgno, 加密页)]
            commit_pages = 0
            for i in range(from_frame, n):
                ...
                if hdr[8:16] != wal_salt:
                    break               # 陈旧世代帧，其后数据一律无效
                pgno = struct.unpack(">I", hdr[:4])[0]
                dbsize = struct.unpack(">I", hdr[4:8])[0]
                pending.append((pgno, page))
                if dbsize:              # commit 帧 → 整笔事务落库
                    self._apply_wal_frames(dst, key, pending, dbsize)
                    pending = []
                    commit_pages = dbsize
                    applied = i + 1
            if commit_pages:            # 未提交的尾部帧按 SQLite 语义丢弃
                self._truncate_db(dst, commit_pages)
        return applied, salt_hex

    @staticmethod
    def _apply_wal_frames(dst: str, key: bytes, frames, dbsize: int) -> None:
        """把一个已提交事务的帧写入主库；超范围页丢弃，避免写出文件空洞。"""
        with open(dst, "r+b") as out:
            for pgno, page in frames:
                if pgno < 1 or pgno > dbsize:
                    continue
                ...
                if pgno == 1:
                    ...
                    if len(key) == 48:
                        ...
                        continue
                elif not pt or pt[0] not in (0, 2, 5, 10, 13):
                    continue
    ...
    @staticmethod
    def _truncate_db(dst: str, pages: int) -> None:
        """把 page1 记录的页数与文件大小统一到 pages，消除残留空洞。"""
        with open(dst, "r+b") as f:
            f.seek(28)
            f.write(struct.pack(">I", pages))
            f.truncate(pages * PAGE_SZ)
            f.flush()
    ...
    def _invalidate_cache(self) -> None:
        ...
        与 _open 共用 _io_lock：删缓存与重建不能交错，否则别的线程会读到
        半删/半写的文件，直接表现为 "malformed"。
        """
        with self._io_lock:
            try:
            ...
            for n in names:
                if n.endswith(".db") or n.endswith(".stamp"):
                    try:
                        ...
                        failed.append("%s(%s)" % (n, e.__class__.__name__))
            if removed:
                sys.stderr.write("[wechatauto] 已清 %d 个缓存文件等待重建\n" % removed)
            if failed:
                # 删不掉说明仍有连接占用：不报告的话下一轮会继续读到坏缓存
                sys.stderr.write("[wechatauto] 有 %d 个缓存文件删除失败(仍被占用): %s\n"
